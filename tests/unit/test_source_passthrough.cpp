...
TEST_CASE("blue source recovery replaces brown neural foreground under alpha",
          "[unit][passthrough][regression]") {
    ImageBuffer src_buf(3, 1, 3);
    ImageBuffer fg_buf(3, 1, 3);
    ImageBuffer alpha_buf(3, 1, 1);
    ...
    ColorUtils::State state;
    source_passthrough(src, fg, alpha, 0, 0, state, 0.0F);
    despill(fg, 1.0F, SpillMethod::Average, /*screen_channel=*/2);

    CHECK(fg(0, 0, 0) == Catch::Approx(0.72F));
    CHECK(fg(0, 0, 1) == Catch::Approx(0.52F));
    CHECK(fg(0, 0, 2) == Catch::Approx(0.38F));

    CHECK(fg(0, 1, 0) > 0.50F);
    CHECK(fg(0, 1, 1) > 0.45F);
    CHECK(fg(0, 1, 2) < 0.30F);

    CHECK(fg(0, 2, 0) == Catch::Approx(0.30F));
    CHECK(fg(0, 2, 1) == Catch::Approx(0.24F));
    CHECK(fg(0, 2, 2) == Catch::Approx(0.12F));
}
