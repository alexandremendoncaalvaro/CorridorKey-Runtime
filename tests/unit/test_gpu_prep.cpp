...
TEST_CASE("GpuInputPrep device path accepts the CUDA default stream handle", "[unit][core]") {
    core::GpuInputPrep prep;
    if (!prep.available()) {
        SKIP("GPU input prep not available on this host");
    }
    ...
    auto res = prep.prepare_inputs_device_on_stream(rgb, hint, model_w, model_h, mean, inv_stddev,
                                                    nullptr, record_stage);
    REQUIRE(res.has_value());
    REQUIRE(res->planar_device != nullptr);
    REQUIRE(res->ready_event != nullptr);
    CHECK(res->ready_event_on_current_stream);
