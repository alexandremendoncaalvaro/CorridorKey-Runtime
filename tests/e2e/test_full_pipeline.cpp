...
TEST_CASE("End-to-End: Full Video Pipeline Sanity Check", "[e2e][video]") {
    ...
    std::filesystem::path model_path = std::filesystem::path(PROJECT_ROOT) / "models" / "corridorkey_int8_512.onnx";
    ...
    if (!std::filesystem::exists(model_path)) {
        ...
        SUCCEED("Model file not found (" + model_path.string() + "), skipping real inference E2E test.");
        return;
    }
