...
TEST_CASE("generated Windows shared frame transport uses named memory",
          "[integration][ofx][runtime][regression]") {
    const auto transport_path = next_ofx_shared_frame_path();

    auto created = SharedFrameTransport::create(transport_path, 16, 8);
    REQUIRE(created.has_value());

    auto opened = SharedFrameTransport::open(transport_path);
    REQUIRE(opened.has_value());
    CHECK(opened->width() == 16);
    CHECK(opened->height() == 8);

#ifdef _WIN32
    CHECK_FALSE(std::filesystem::exists(transport_path));
#else
    std::error_code error;
    std::filesystem::remove(transport_path, error);
#endif
