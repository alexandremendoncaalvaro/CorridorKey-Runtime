...
enum class SpillMethod : int { Average = 0, DoubleLimit = 1, Neutral = 2, ScreenOnly = 3 };

// Removes screen-color spill from an RGB image. Average, DoubleLimit, and
// Neutral redistribute the removed energy into the non-screen channels. The
// screen channel index defaults to 1 (green) so existing callers keep their
// semantics; pass 2 for a blue screen.
//
...
// Routing dedicated blue-trained weights can target a different screen
// channel directly. ScreenOnly subtracts the screen channel
// without adding warm fill to the non-screen channels.
