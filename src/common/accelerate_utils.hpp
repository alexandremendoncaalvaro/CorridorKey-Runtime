...
inline void accelerate_vmul(const float* src1, std::ptrdiff_t src1_stride, const float* src2,
     ...
     */
    inline void accelerate_normalize_and_pack_4ch(
        const float* rgb, int rgb_stride, const float* hint, int hint_stride, float* dst, int count,
        ...
        // Formula: (val - mean) / stddev  => val * (1/stddev) + (-mean/stddev)
        for (int c = 0; c < 3; ++c) {
            float m = means[c];
        ...
        // Copy hint channel
        vDSP_mmov(hint, dst + 3, 1, count, hint_stride, 4);
#else
