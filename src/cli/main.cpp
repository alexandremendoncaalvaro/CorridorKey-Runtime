...
void print_info() {
    ...
    for (const auto& d : devices) {
        ...
        switch (d.backend) {
            case Backend::CPU:
                backend_name = "CPU (Universal)";
                break;
            case Backend::CoreML:
                backend_name = "CoreML (Apple Silicon NPU/GPU)";
                break;
            case Backend::TensorRT:
                backend_name = "TensorRT (NVIDIA RTX)";
                break;
            case Backend::CUDA:
                backend_name = "CUDA (NVIDIA)";
                break;
            case Backend::DirectML:
                backend_name = "DirectML (Windows GPU)";
                break;
            default:
                backend_name = "Unknown";
                break;
        }
        std::cout << " - " << std::left << std::setw(30) << d.name << " [" << backend_name << "] "
...
int main(int argc, char* argv[]) {
    ...
    options.add_options()("command",
                          "Sub-command to run (info, process, download, doctor, benchmark)",
    ...
    try {
        ...
        if (!result.count("command")) {
            std::cout << "Error: No command specified. Use 'info', 'download', 'process', 'doctor' "
                         "or 'benchmark'."
                      << std::endl;
            return 1;
        }
