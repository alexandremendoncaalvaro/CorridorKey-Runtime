...
4. **Curated Platform Tracks.** The official product tracks are Apple Silicon
   through MLX and Windows RTX through dynamic TorchTRT artifacts on NVIDIA
...
- The Windows RTX track resolves `green` and `blue` to one dynamic TorchTRT
  artifact each. `Blue-Green Channel Swap` resolves to the green artifact with
...
- **Inference:** Backend adapters and session management for the official MLX
  and TorchTRT product paths, the experimental DirectML and Linux CUDA EP
