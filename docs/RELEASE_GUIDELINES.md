...
The OFX product name inside staged bundle folders is `CorridorKey_OFX` (not
...
host. Windows RTX installers use `CorridorKey_v<label>_Windows_online_Setup.exe`.
...
- Windows RTX: `CorridorKey_v<label>_Windows_online_Setup.exe`

Public Windows RTX publication uses the online installer. Offline packaging is
reserved for explicit private no-network validation and is not attached to
GitHub releases.
...
| Inno Setup 6.4+ | default install at `C:\Program Files (x86)\Inno Setup 6\` | auto-discovered |
...
- **Models:** the Windows RTX release expects the dynamic TorchTRT green and
...
Windows has one canonical release entrypoint for OFX release packaging:
...
as a fallback path. The Windows RTX release flow packages the dynamic TorchTRT
runtime contract and fails when asked to emit DirectML or combined OFX
installers.
...
That canonical command generates the official Windows `RTX` online installer
by default.
...
- `certify-rtx-artifacts`
  - certify an already existing Windows RTX model set and write the certified
    artifact manifest without regenerating model files from the checkpoint
- `package-ofx`
  - package Windows installers from an already certified model set. Windows
    RTX packaging uses Inno Setup; `online` is the default flavor and
    `offline` must be requested explicitly for private no-network validation.
- `release`
...
to `online` for Windows RTX when no flavor is provided. The `offline` flavor is
produced only when validating the no-network installer path or serving an
environment that cannot download packs at install time. Windows GitHub releases
publish the online flavor only; the release pipeline rejects `-PublishGithub`
...
already matches the manifest, upgrades preserve it and skip the download or
copy while still refreshing plugin binaries. When a selected pack file is
missing or invalid, the installer replaces that file from the verified source.
Packs that are not selected are removed from the install tree.
...
The wrapper stages and validates the OFX bundle, then produces the chosen Inno
Setup installer. Windows RTX local validation and publication must use the
setup executable emitted by `scripts/installer/build_installer.ps1`.
...
2. Run the canonical Windows release flow:
   ...
   ```
Scripts validate the runtime root and packaged runtime contract before emitting
the installer.
...
2. You already have a certified RTX artifact set and only want the installers:
   ```powershell
   .\scripts\windows.ps1 -Task package-ofx -Version X.Y.Z -Track rtx -Flavor online
   ```
...
- `Windows RTX` packages the dynamic green TorchTRT artifact, the optional
  dynamic blue TorchTRT artifact, and the TorchTRT runtime pack. ONNX Runtime
...
- The Windows `RTX` artifact is the supported Windows track for NVIDIA RTX 30
  series and newer.
- The canonical public Windows release produces the Windows RTX online
  installer only.
...
- Do not include a Windows DirectML OFX installer in the Windows RTX release
  flow. A separate Windows DirectML product track requires a support-matrix
  contract update before publication.
