...
- TensorRT RTX Lite: `CorridorKey_Resolve_vX.Y.Z_Windows_RTX_Lite_Installer.exe`
...
- `CorridorKey_Resolve_vX.Y.Z_Windows_RTX_Lite.zip`
...
That canonical command generates the Windows `RTX Lite` and `RTX Full`
...
- `RTX Lite` packages the validated FP16 and INT8 ladder through `1024px`.
...
- The Windows `RTX Lite` and `RTX Full` artifacts are the supported Windows
  tracks for NVIDIA RTX 30 series and newer.
- `RTX Lite` is the conservative installer through `1024px`.
