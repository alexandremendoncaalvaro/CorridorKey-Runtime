...
Public Windows RTX publication uses the online installer. Offline installers
are local validation artifacts and are not attached to GitHub releases.
...
- `vendor\onnxruntime-windows-rtx` - curated Windows RTX runtime support
...
- `package-ofx`
  - package Windows installers from an already certified model set. Modern
    Windows RTX packaging uses Inno Setup and an explicit
    `-Flavor online|offline`; see "Modern installer (Inno Setup)" below for
    the flavor semantics.
- `release`
