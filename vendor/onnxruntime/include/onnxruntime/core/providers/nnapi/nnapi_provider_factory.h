...
// To generate an uint32_t nnapi_flags for using with OrtSessionOptionsAppendExecutionProvider_Nnapi
// below,
...
enum NNAPIFlags {
    ...
    //
    // NNAPI is more efficient using GPU or NPU for execution, and NNAPI might fall back to its own
    // CPU implementation for operations not supported by GPU/NPU. The CPU implementation of NNAPI
    // (which is called nnapi-reference) might be less efficient than the optimized versions of the
    // operation of ORT. It might be advantageous to disable the NNAPI CPU fallback and handle
    // execution using ORT kernels.
    //
    // For some models, if NNAPI would use CPU to execute an operation, and this flag is set, the
    // execution of the model may fall back to ORT kernels.
    //
    // This option is only available after Android API level 29, and will be ignored for Android API
    // level 28-
    //
    // For NNAPI device assignments, see
    // https://developer.android.com/ndk/guides/neuralnetworks#device-assignment For NNAPI CPU
    // fallback, see https://developer.android.com/ndk/guides/neuralnetworks#cpu-fallback
    //
