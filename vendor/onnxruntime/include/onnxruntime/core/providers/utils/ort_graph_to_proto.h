...
/*
 SUMMARY:
   Utilities to serialize an OrtGraph into an ONNX GraphProto or ModelProto. Can be used by
 execution provider implementations that need to convert an OrtGraph instance into an ONNX protobuf
 model.

   Users may copy this file and modify as needed.

 USAGE:
   This is a header-only implementation that includes both the function declarations and
 definitions. Copy this file into a project that links with both ONNX Runtime and ONNX.

   Define the ORT_EP_UTILS_ORT_GRAPH_TO_PROTO_IMPL preprocessor macro before the #include statement
 in exactly one C++ file to define the implementation. Example:

     #define ORT_EP_UTILS_ORT_GRAPH_TO_PROTO_IMPL
     #include "ort_graph_to_proto.h"

   Other compilation units that depend on these utilities should include this file without defining
 the preprocessor macro.

   Example program snippets are shown below. Refer to the function declarations for detailed usage
 information.
 ...
 EXAMPLE SNIPPET (large initializers stored in external file):
   ...
   OrtStatus* ORT_API_CALL GetCapabilityImpl(OrtEp* this_ptr, const OrtGraph* ort_graph,
     ...
     auto handle_initializer_data = [&external_file_path, &out_file](const OrtValueInfo* value_info,
                                                                     ...
                                                                     bool& is_external, std::string&
 location, int64_t& offset) -> Ort::Status {
 ...
 EXAMPLE SNIPPET (external initializers that point to data in memory, not officially supported by
 ONNX spec):

   This example stores initializers externally. However, instead of storing the initializers in a
 separate file, the onnx::TensorProto objects point directly to memory addresses. This requires
 setting the initializer's location to a special tag like "_MEM_ADDR_" (instead of a file path). The
 offset is set to the pointer to the initializer's data in memory (instead of an offset into a
 file).

   Because this is not standard ONNX, such a onnx::GraphProto should not be saved as an ONNX file.
   However, it allows custom tools that operate directly on a onnx::GraphProto to get the
 initializer data if it has already been loaded into memory.
   ...
   OrtStatus* ORT_API_CALL GetCapabilityImpl(OrtEp* this_ptr, const OrtGraph* ort_graph,
     ...
     auto handle_initializer_data = [](const OrtValueInfo* value_info,
       ...
       location = "_MEM_ADDR_";  // Some special location tag that indicates the offset is a
 pointer. is_external = true;  // True if is external initializer return Ort::Status{nullptr};
     }
...
/// <summary>
/// Signature of user-provided function to handle initializer data. Called by OrtGraphToProto() for
/// every initializer.
///
/// If the function sets the `is_external` output parameter to false, OrtGraphToProto() stores
/// initializer data within the TensorProto as raw_data.
///
/// Otherwise, if the function sets `is_external` to true, OrtGraphToProto() assumes that this
/// function stores the initializer data in a file. In this case, OrtGraphToProto() configures the
/// corresponding TensorProto to point the location and offset returned via the `location` and
/// `offset` output parameters.
///
/// It is recommended to keep small initializers with byte size <= 127 stored inline the TensorProto
/// to ensure ONNX shape inference works correctly with the serialized ONNX model.
/// </summary>
/// <param name="value_info">OrtValueInfo for the initializer. Can be used to query name, type,
/// shape,
...
/// <param name="is_external">Output parameter set to true if the initializer data is stored
/// externally. The
///                           implementer is responsible for writing the initializer data to file.
///                           If set to false, the initializer will be stored within the
///                           TensorProto.</param>
/// <param name="location">Output parameter set to the location (e.g., file) into which the
/// initializer is stored
///                        by the implementer of this function. Ignored if `is_external` is set to
///                        false.</param>
/// <param name="offset">Output parameter set to the offset (e.g., file offset) into which the
/// initializer is stored
///                      by the implementer of this function. Ignored if `is_external` is set to
///                      false.</param>
/// <returns>An Ort::Status indicating success or an error. Serialization exits if this returns an
/// error.</returns>
using HandleInitializerDataFunc =
    std::function<Ort::Status(const OrtValueInfo* value_info, const void* data, size_t size,
...
/// within a TensorProto, written to a file, or remain as an in-memory external initializer (not
/// valid ONNX).
/// </summary>
/// <param name="ort_graph">OrtGraph instance to serialize.</param>
/// <param name="graph_proto">Destination GraphProto into which to serialize the input
/// OrtGraph.</param> <param name="handle_initializer_data_func">Optional function called to allow
...
Ort::Status OrtGraphToProto(const OrtGraph& ort_graph, onnx::GraphProto& graph_proto,
...
/// </summary>
/// <param name="ort_graph">OrtGraph instance to serialize.</param>
/// <param name="model_proto">Destination ModelProto into which to serialize the input
/// OrtGraph.</param> <param name="handle_initializer_data_func">Optional function called to allow
/// the user to determine
///                                            where the initializer data is stored.</param>
/// <returns>An Ort::Status indicating success or an error.</returns>
Ort::Status OrtGraphToProto(const OrtGraph& ort_graph, onnx::ModelProto& model_proto,
                            HandleInitializerDataFunc handle_initializer_data_func = nullptr);
/// <summary>
...
static Ort::Status GetOrtValueInfoTensorTypeShape(Ort::ConstValueInfo vi, bool get_symbolic_dims,
...
static Ort::Status GetTensorElementSize(const ONNXTensorElementDataType& element_type,
                                        size_t& element_size);
...
Ort::Status OrtGraphToProto(const OrtGraph& graph, onnx::GraphProto& graph_proto,
    ...
    try {
        ...
        for (const auto& ort_value_info : graph_inputs) {
            ...
            ORT_EP_UTILS_CXX_RETURN_IF_ERROR(
                OrtValueInfoToProto(ort_value_info, *value_info_proto));
        }
        ...
        // Use std::maps to store OrtValueInfos for GraphProto.value_info and
        // GraphProto.initializer. A std::map maintains its elements in a stable ordering.
        std::map<std::string, Ort::ConstValueInfo> value_infos;  // For GraphProto.value_info
        std::map<std::string, Ort::ConstValueInfo>
            initializer_value_infos;  // For GraphProto.initializer

        // Helper function to collect an OrtValueInfo into `value_infos` or
        // `initializer_value_infos`. Optionally returns the OrtValueInfo name to the caller.
        auto collect_value_info = [&value_infos, &initializer_value_infos](
                                      Ort::ConstValueInfo ort_value_info,
            ...
            if (value_infos.count(value_name) != 0 ||
                initializer_value_infos.count(value_name) != 0) {
            ...
            // Do add initializers (constant and non-constant) to GraphProto's list of initializer
            // tensors.
            if (is_from_outer_scope) {
                ...
                value_infos.emplace(value_name,
                                    ort_value_info);  // This is an internal OrtValueInfo.
            }
        };

        std::vector<Ort::ConstNode> nodes = ort_graph.GetNodes();
        // Loop through all nodes (topological order): add NodeProto instances to GraphProto and
        // track OrtValueInfos that will be stored in GraphProto.value_info and
        ...
        // Iterating through all nodes to collect initializer value info is therefore not
        // sufficient, initializers must also be obtained from ort_graph.GetInitializers(). Add
        // those missing initializers and skip the ones that already in `initializer_value_infos`
        ...
        for (const auto& [initializer_name, initializer_value_info] : initializer_value_infos) {
            ...
            ONNXTensorElementDataType initializer_elem_type =
                ONNX_TENSOR_ELEMENT_DATA_TYPE_UNDEFINED;
            bool has_shape = false;
            ORT_EP_UTILS_CXX_RETURN_IF_ERROR(GetOrtValueInfoTensorTypeShape(
                initializer_value_info, /*get_sym_dims*/ false, initializer_elem_type,
                initializer_dims, initializer_sym_dims, has_shape));
            ...
            if (handle_initializer_data_func != nullptr) {
                ORT_EP_UTILS_CXX_RETURN_IF_ERROR(
                    handle_initializer_data_func(initializer_value_info, data, data_bytes,
...
Ort::Status OrtGraphToProto(const OrtGraph& graph, onnx::ModelProto& model_proto,
                            HandleInitializerDataFunc handle_initializer_data_func) {
    try {
        Ort::ConstGraph ort_graph{&graph};
        ...
        ORT_EP_UTILS_C_RETURN_IF(op_sets.empty(),
                                 "OrtGraph should have at least one operator set.");
        ...
        ORT_EP_UTILS_CXX_RETURN_IF_ERROR(
            OrtGraphToProto(*ort_graph, *graph_proto, handle_initializer_data_func));
...
static Ort::Status GetOrtValueInfoTensorTypeShape(Ort::ConstValueInfo vi, bool get_symbolic_dims,
    ...
    try {
        ...
        ORT_EP_UTILS_C_RETURN_IF(ort_onnx_type != ONNX_TYPE_TENSOR,
                                 "Expected OrtValueInfo to represent a Tensor");
...
static Ort::Status OrtValueInfoToProto(Ort::ConstValueInfo ort_value_info,
    ...
    // We currently only support ONNX tensors. Support for other types (e.g., ONNX_TYPE_SEQUENCE)
    // can be added later.
    bool has_shape = false;
    ORT_EP_UTILS_CXX_RETURN_IF_ERROR(GetOrtValueInfoTensorTypeShape(
        ort_value_info, /*get_sym_dims*/ true, ort_elem_type, ort_dims, ort_dim_syms, has_shape));

    value_info_proto.set_name(ort_value_info.GetName());

    onnx::TypeProto_Tensor* type_proto_tensor =
        value_info_proto.mutable_type()->mutable_tensor_type();
    ...
    if (has_shape) {
        ...
        for (size_t dim_idx = 0; dim_idx < ort_dims.size(); dim_idx++) {
            ...
            if (ort_dims[dim_idx] >= 0) {
                ...
                // If dim_param is empty, leave dim_proto with neither the dim_value or dim_param
                // set, which represents an unknown dimension.
                if (!dim_param.empty()) {
        ...
        switch (attr_type) {
            ...
            case OrtOpAttrType::ORT_OP_ATTR_TENSOR: {
                ...
                switch (element_type) {
                    ...
                    default: {
                        std::string err_msg = "Unexpected ONNXTensorElementDataType with value " +
            ...
            default: {
                std::string err_msg = "Unexpected OrtOpAttrType with value " +
                                      std::to_string(static_cast<int>(attr_type));
                return Ort::Status(err_msg.c_str(), ORT_FAIL);
            }
        }
    } catch (const Ort::Exception& ex) {
...
Ort::Status ConvertExternalData(const OrtValueInfo* value_info, void* data, size_t bytes) {
    ...
    bool has_shape{false};
    ORT_EP_UTILS_CXX_RETURN_IF_ERROR(
        GetOrtValueInfoTensorTypeShape(ort_value_info, false, initializer_elem_type,
                                       initializer_dims, initializer_sym_dims, has_shape));
    GetTensorElementSize(initializer_elem_type, element_size);
    if (element_size != 1) {
