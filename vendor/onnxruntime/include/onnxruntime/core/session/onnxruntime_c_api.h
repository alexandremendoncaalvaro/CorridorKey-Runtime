...
typedef enum ONNXTensorElementDataType {
    ...
    ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT8E5M2,  // Non-IEEE floating-point format based on IEEE754
                                               // single-precision
    ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT8E5M2FNUZ,  // Non-IEEE floating-point format based on
                                                   // IEEE754 single-precision
    ...
    ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT4E2M1,  // maps to a pair of packed float4 values (size == 1
                                               // byte)
...
typedef struct OrtAllocator {
     ...
     *
     * \return pointer to an allocated block of `size` bytes. nullptr if size was 0 or allocation
     * failed.
     *
     * \since 1.18
     */
     ...
     *
     * Return a pointer to the OrtKeyValuePairs structure that contains the statistics of the
     * allocator. The user should call OrtApi::ReleaseKeyValuePairs when done.
     *
     ...
     *       If the OrtAllocator is wrapping an internal ORT allocator that does not implement
     * GetStats the returned OrtKeyValuePairs instance will be empty.
     *
     * \since 1.23
     */
     ...
     * \since 1.23
     */
    void*(ORT_API_CALL* AllocOnStream)(struct OrtAllocator* this_, size_t size,
...
typedef enum OrtDeviceMemoryType {
    ...
    OrtDeviceMemoryType_HOST_ACCESSIBLE =
        5,  ///< Shared/pinned memory for transferring between CPU and the device
} OrtDeviceMemoryType;
...
/** \brief These are the default EP selection policies used by ORT when doing automatic EP
 * selection.
 */
 ...
 *
 * This delegate is called by the EP selection code to allow the user to provide custom device
 selection logic.
 ...
 * \param selected Pre-allocated array to populate with selected OrtEpDevice pointers from
 ep_devices.
 * \param max_selected The maximum number of devices that can be selected in the pre-allocated
 array. Currently the maximum is 8.
 * \param num_selected The number of selected devices.
 * \param state Opaque pointer. Required to use the delegate from other languages like C# and
 python.
 *
...
typedef OrtStatus*(ORT_API_CALL* EpSelectionDelegate)(
    ...
    _In_ const OrtKeyValuePairs* model_metadata, _In_opt_ const OrtKeyValuePairs* runtime_metadata,
    _Inout_ const OrtEpDevice** selected, _In_ size_t max_selected, _Out_ size_t* num_selected,
    _In_ void* state);

/** \brief Function called by ORT to write a buffer to a custom destination (e.g., file, stream,
 * etc.).
 *
...
/** \brief Function called by ORT to allow user to specify how an initializer should be saved, that
 * is, either written to an external file or stored within the model. ORT calls this function for
 * every initializer when generating a model.
 *
 * If the function implementation sets the `new_external_info` output parameter to NULL, ORT stores
 * the initializer data within the generated model.
 *
 * Otherwise, if the function implementation sets `new_external_info` to a valid
 * OrtExternalInitializerInfo instance, ORT assumes that this function stores the initializer data
 * in a file. In this case, ORT configures the model's initializer to point to the location
 * specified by the `new_external_info` output parameter.
 *
 ...
 * \param[in] external_info If the initializer is originally stored in an external file,
 * `external_info` contains the file path, file offset, and the data's byte size within the file.
 * Otherwise, `external_info` is NULL if the initializer is not originally stored in a file.
 * \param[out] new_external_info Output parameter set to a new OrtExternalInitializerInfo instance
 * indicating the location where the function implementation stored the initializer data. The
 * function implementation must use `OrtApi::CreateExternalInitializerInfo()` to create the
 * instance. If the function implementation sets `new_external_info` to NULL, ORT stores the
 * initializers within the model.
 *
...
typedef struct OrtMIGraphXProviderOptions {
    ...
    int migraphx_save_compiled_model;  // migraphx save compiled model. Default 0 = false, nonzero =
                                       // true
 ...
 *  \brief This Struct is frozen since ORT 1.13.0. Its maintained part of Legacy API for
 * compatibility. \brief For latest OpenVINO Provider Options update to the ProviderOptions map.
 ...
 * document.</a> \endhtmlonly \see OrtApi::SessionOptionsAppendExecutionProvider()
 */
 ...
 * nullptr \param[out] num_outputs Number of outputs, on error, the value will be zero \param[out]
 * status On error, status will provide details
 */
...
typedef enum OrtExternalMemoryHandleType {
    ...
    ORT_EXTERNAL_MEMORY_HANDLE_TYPE_D3D12_HEAP =
        1, /**< Shared HANDLE from ID3D12Device::CreateSharedHandle(heap) */
...
typedef enum OrtExternalSemaphoreType {
    ORT_EXTERNAL_SEMAPHORE_D3D12_FENCE =
        0, /**< Shared HANDLE from ID3D12Device::CreateSharedHandle(fence) */
...
typedef struct OrtEnvCreationOptions {
    ...
    /** \brief The logging severity level for the environment. Must be set to a value from
     * OrtLoggingLevel.
     *
     * \note Logging messages which are less severe than the `logging_severity_level` are not
     * emitted.
     *
     * \note Serves as the default logging severity level for session creation and runs.
     *       Use OrtApi::SetSessionLogSeverityLevel to set a logging severity level for the creation
     * of specific session. Use OrtApi::RunOptionsSetRunLogSeverityLevel to set a logging severity
     * level for a specific session run.
     *
     * \since Version 1.24.
     */
     ...
     *
     * \note The OrtEnvCreationOptions::custom_logging_param is provided as the first argument to
     * this logging function. This allows passing custom state into the logging function.
     *
     * \note This function is only called when a message's severity meets or exceeds the set logging
     * severity level.
     *
     * \since Version 1.24.
     */
    ...
    /** \brief Optional threading options for creating an environment with global thread pools
     * shared across sessions. May be set to NULL.
     *
     ...
     *
     * \note Use this in conjunction with OrtApi::DisablePerSessionThreads or else the session will
    ...
    /** \brief Optional environment configuration entries represented as string key-value pairs. May
     * be set to NULL.
     *
     * \note The OrtKeyValuePairs instance is copied by ORT.
     *
     * \note Refer to onnxruntime_env_config_keys.h for common config entry keys and their supported
     * values.
     *
     * \note An application provides environment-level configuration options for execution provider
     * libraries by using keys with the prefix 'ep_factory.\\<ep_name\\>.'. Ex: the key
     * 'ep_factory.my_ep.some_ep_key' represents a key named 'some_ep_key' that is meant to be
     * consumed by an execution provider named 'my_ep'. Refer to the specific execution provider's
     * documentation for valid keys and values.
     *
     * \note An application may separately set session-level configuration options for execution
     * providers via other APIs such as SessionOptionsAppendExecutionProvider_V2, which store
     * configuration entries within OrtSessionOptions. If an environment-level configuration
     * conflicts with a session-level configuration, then precedence is determined by the execution
     * provider library itself.
     *
     * \since Version 1.24.
     */
...
struct OrtApi {
     ...
     * will be ignored. \param[in] log_severity_level The log severity level. \param[in] logid The
     * log identifier. \param[out] out Returned newly created OrtEnv. Must be freed with
     * OrtApi::ReleaseEnv
     *
     ...
     * the SetUserLoggingFunction API instead. \param[in] logging_function A pointer to a logging
     * function. \param[in] logger_param A pointer to arbitrary data passed as the
     * ::OrtLoggingFunction `param` parameter to `logging_function`. This parameter is optional.
     ...
     *
     * The allocator wraps the internal allocator from the OrtSession and becomes invalid when the
     * session does.
     *
     * \param[in] session
     ...
     * default. "max_power_of_two_extend_bytes": The maximum extend size if arena strategy is
     * `kNextPowerOfTwo`. It is not an allocation limit, it is only a limit for extension when
     ...
     * num_initializers Number of elements in the initializer_names and initializers
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.12.
     ...
     * \param[in] len Number of bytes stored in data for ORT_OP_ATTR_STRING.
                      Number of elements if data represents an array (e.g., ORT_OP_ATTR_INTS).
     Otherwise, set to 1.
     ...
     *
     * QNN supported keys:
     *   "backend_type": Type of QNN backend. Specifies a backend path that is the associated QNN
     *backend library file name. E.g., given backend type "htp", on Windows, the backend path would
     *be "QnnHtp.dll", and on other platforms, it would be "libQnnHtp.so". Mutually exclusive with
     *"backend_path". Available options:
     *      -# "cpu"
     *      -# "gpu"
     *      -# "htp": Default.
     *      -# "saver"
     ...
     *   "dump_qnn_ir_dlc": Use the QnnIr backend library to write .dlc files for each subgraph
     *dispatched to QNN. When enabled, inference results will be incorrect. Use only for debugging.
     *      -# "0": Default: disabled
     *      -# "1": enabled
     *   "dump_qnn_ir_dlc_dir": Set the directory into which QnnIr will be configured to write QNN
     *graphs as .dlc files. Default is current working directory. "qnn_ir_backend_path": File path
     *to the QnnIr backend library. If "dump_qnn_ir_dlc" is enabled, use this path instead of
     *looking for the Ir backend in the standard location. "qnn_saver_path": File path to the QNN
     *Saver backend library. If specified, QNN Saver will be enabled and will dump QNN API calls to
     *disk for replay/debugging. QNN Saver produces incorrect model inference results and may alter
     *model/EP partitioning. Use only for debugging. "qnn_context_priority": QNN context priority.
     *      Available options:
     *      -# "low"
     *      -# "normal": Default.
     *      -# "normal_high"
     ...
     *documentation for valid values. Defaults to "0" (unknown). "htp_arch": The minimum HTP
     *architecture the driver will use to select compatible QNN operators. Available options:
     *      -# "0": Default (none).
     *      -# "68"
     *      -# "69"
     *      -# "73"
     ...
     *single device). "enable_htp_fp16_precision": Used for float32 model for HTP backend. Enable
     *the float32 model to be inferenced with fp16 precision. Otherwise, it will be fp32 precision.
     *      -# "0": With fp32 precision.
     *      -# "1": Default. With fp16 precision.
     *   "offload_graph_io_quantization": Offload graph input quantization and graph output
     *dequantization to another execution provider (typically CPU EP).
     *      -# "0": Disabled. QNN EP will handle quantization and dequantization of graph I/O.
     *      -# "1": Enabled. This is the default value.
     *   "enable_htp_spill_fill_buffer": Enable HTP spill fill buffer setting. The flag is used
     *while generating context binary.
     *      -# "0": Default. Disabled.
     *      -# "1": Enabled.
     *   "enable_htp_shared_memory_allocator": Enable the QNN HTP shared memory allocator. Requires
     *libcdsprpc.so/dll to be available.
     *      -# "0": Default. Disabled.
     *      -# "1": Enabled.
     *   "dump_json_qnn_graph": Set to "1" to dump QNN graphs generated by QNN EP as JSON files.
     *Each graph partition assigned to QNN EP is dumped to a separate file. "json_qnn_graph_dir":
     *Directory in which to dump QNN JSON graphs. If not specified, QNN graphs are dumped in the
     ...
     *     where op_type is the name of the operation, op_package_path is the path to the op package
     *shared library, interface is the symbol name to register the op life cycle functions, and
     *target is the backend type. For more details, refer to:
     *https://docs.qualcomm.com/bundle/publicresource/topics/80-63442-50/op_packages.html [Advanced]
     *"skip_qnn_version_check": Set to "1" to allow a different version of QNN to be used than what
     *was compiled into ONNX Runtime. Differences in operator support, accuracy, performance, and
     *QNN's ABI may lead to crashes, inaccurate results, and poor performance. Use with caution and
     *test thoroughly.
     *
     ...
     * that enable training with onnxruntime. \note A NULL pointer will be returned and no error
     * message will be printed if the training api is not supported with this build. A NULL pointer
     * will be returned and an error message will be printed if the provided version is unsupported,
     * for example when using a runtime older than the version created with this header file.
     *
     * \param[in] version Must be ::ORT_API_VERSION
     ...
     * used to allocate continuous buffers for output strings and lengths. \param[out] ptr is a
     * UTF-8 null terminated string allocated using 'allocator'. The caller is responsible for using
     ...
     * to the registration function. \param[in] registration_func_name Name of registration function
     * to use.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.14
     ...
     *
     * If the `size` parameter is greater than or equal to the name string's size and `out` is not
     * nullptr, the value of `size` is set to the true size of the string (including
     * null-terminator), the provided memory is filled with the string's contents, and a success
     * status is returned.
     *
     ...
     * out-of-bounds. \param[out] out Memory location into which to write the UTF-8 null-terminated
     * string representing the input's name. \param[in,out] size Pointer to the size of the `out`
     ...
     * out-of-bounds. \param[out] out Memory location into which to write the UTF-8 null-terminated
     * string representing the output's name. \param[in,out] size Pointer to the size of the `out`
     * buffer. See above comments for details.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.14
     ...
     * be stored. \param[in,out] size Pointer to the size of the `config_value` buffer. See above
     ...
     * representing the name. \param[in,out] size Pointer to the size of the `out` buffer. See above
     * comments for details.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.15
     */
    ORT_API2_STATUS(KernelInfo_GetNodeName, _In_ const OrtKernelInfo* info, _Out_opt_ char* out,
                    _Inout_ size_t* size);
     ...
     * value of ORT_FILE. \param[in] line_number The file line number in which the message is
     * logged. Usually the value of __LINE__. \param[in] func_name The name of the function in which
     * the message is logged. Usually the value of __FUNCTION__.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.15
     */
     ...
     *
     * Can be used in a custom operator to get the logging severity level of the ::OrtLogger
     ...
     * registered. \param[in] env OrtEnv instance \param[in] provider_type ExecutionProvider type
     *  \param[in] mem_info OrtMemoryInfo instance
     ...
     * zero, there is no limit \param[in] usr_data User data to be passed back to fn
     *
     * \since Version 1.17.
     */
    ...
    /** \brief Get scratch buffer from the corresponding allocator under the specific OrtMemoryInfo
     * object. NOTE: callers are responsible to release this scratch buffer from the corresponding
     * allocator \param[in] context OrtKernelContext instance \param[in] mem_info OrtMemoryInfo
     * instance \param[in] count_or_bytes How many bytes is this scratch buffer \param[out] out A
     ...
     * the file names which holds the external initializers. \param[in]
     * external_initializer_file_buffer_array Array of pointers to the buffer of the file content.
     ...
     * content \param[in] num_external_initializer_files Number of external files
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.18.
     ...
     *
     * The function attempts to locate file specified by adapter_file_path, read it and create an
     * OrtLoraAdapter instance. The adapter_file_path should be a valid path to a file that contains
     * a valid Lora Adapter format. The function attempts to validate the format at load time. The
     * file will always be memory mapped, unless the platform does not support memory mapping, in
     ...
     *            data is copied to the device at some point before Run() is invoked. If nullptr,
     * data stays on CPU. The data would still be copied to device if required by the model at
     * inference time. \param[out] out A pointer to a newly created OrtLoraAdapter instance. Must be
     * released with OrtApi::ReleaseLoraAdapter.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.20.
     ...
     * \since Version 1.20.
     */
    ORT_API2_STATUS(CreateLoraAdapterFromArray, _In_ const void* bytes, size_t num_bytes,
                    _In_ OrtAllocator* allocator, _Outptr_ OrtLoraAdapter** out);
     ...
     *
     * The function adds the Lora Adapter to the list of active adapters. The Lora Adapter must be
     * created with OrtApi::CreateLoraAdapter or FromArray. The Lora Adapter will be used by the
     * session to run the model. The instance of the OrtRunOptions can then be used to customize the
     * Run() calls. More than one OrtLoraAdapter can be active at the same time. Lora Parameters
     * that belong to different Lora adapters that will be active at the same time must not overlap.
     ...
     * \since Version 1.20.
     */
    ORT_API2_STATUS(RunOptionsAddActiveLoraAdapter, _Inout_ OrtRunOptions* options,
     ...
     * `include\onnxruntime\core\session\onnxruntime_session_options_config_keys.h` Look for
     ...
     *
     * ORT will take ownership of the memory and free it using the provided deleter when no longer
     * in use.
     *
     * \param[in] deleter OrtAllocator instance that will be used to free the memory.
     *                    Only the OrtAllocator:Info and OrtAllocator::Release functions are
     * required. The OrtMemoryInfo returned by OrtAllocator::Info must match the location of p_data.
     * \param[in] p_data Pointer to the memory that will be used by the Tensor. ORT will take
     * ownership of the memory. \param[in] p_data_len Length of the memory in bytes. \param[in]
     * shape Dimensions of the Tensor. All values should be > 0. \param[in] shape_len Number of
     * dimensions in the shape array. \param[in] type Data type of the Tensor. \param[out] out Newly
     * created ::OrtValue. Must be freed with OrtApi::ReleaseValue
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.22.
    ...
    ORT_API2_STATUS(CreateTensorWithDataAndDeleterAsOrtValue, _In_ OrtAllocator* deleter,
                    _In_ void* p_data, size_t p_data_len, _In_ const int64_t* shape,
                    size_t shape_len, ONNXTensorElementDataType type, _Outptr_ OrtValue** out);
     ...
     *            attempt to cancel the loading process. If cancellation is successful,
     * CreateSession() CreateSessionFromArray() or any other session creation API that take session
     * options as an argument will return an OrtStatus indicating that session loading was canceled
     * at user request, error code ORT_MODEL_LOAD_CANCELED. The APIs above would not return any
     * valid Session instance. This is the best case effort and the result is not guaranteed. The
     * session may have already been created and initialized before the cancellation request was
     * issued.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     ...
     *
     * Get the Compile API instance to compile ONNX models. Execution providers that support
     * compilation fuse a subgraph into an EPContext node that wraps a provider-specific binary
     * representation of the subgraph. For more details about the EPContext design, refer to:
     *  \htmlonly
     *  <a href="https://onnxruntime.ai/docs/execution-providers/EP-Context-Design.html">EPContext
     * design document.</a> \endhtmlonly
     *
    ...
    void(ORT_API_CALL* AddKeyValuePair)(_In_ OrtKeyValuePairs* kvps, _In_ const char* key,
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(RegisterExecutionProviderLibrary, _In_ OrtEnv* env,
                    _In_ const char* registration_name, _In_ const ORTCHAR_T* path);

    /** \brief Unregister an execution provider library with ORT.
     *
     * ORT will call ReleaseEpFactory for all factories created by the library, and unload the
     * library.
     *
     * You <b>MUST</b> ensure there are no Session instances using execution providers created by
     * the library before calling this function.
     *
     ...
     * \since Version 1.22.
     */
    ORT_API2_STATUS(UnregisterExecutionProviderLibrary, _In_ OrtEnv* env,
                    _In_ const char* registration_name);

    /** \brief Get the list of available OrtEpDevice instances.
     *
     * Each OrtEpDevice instance contains details of the execution provider and the device it will
     * use.
     *
    ...
    /** \brief Append the execution provider that is responsible for the selected OrtEpDevice
     * instances to the session options.
     *
     ...
     *                       Obtain from GetEpDevices. All OrtEpDevice instances must be from the
     * same execution provider. It is only necessary to provide multiple OrtEpDevices if you want to
     * use the same execution provider for multiple devices. e.g. the EP is capable of running on
     * GPU and NPU. \param[in] num_ep_devices Number of OrtEpDevice instances. \param[in]
     * ep_option_keys Optional keys to configure the execution provider. \param[in] ep_option_vals
     * Optional values to configure the execution provider. \param[in] num_ep_options Number of
     * execution provide options to add.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.22.
    ...
    ORT_API2_STATUS(SessionOptionsAppendExecutionProvider_V2,
                    _In_ OrtSessionOptions* session_options, _In_ OrtEnv* env,
     ...
     *
     * Allows users to specify a device selection policy for automatic execution provider (EP)
     * selection. If custom selection is required please use
     ...
     *
     * Allows users to provide a custom device selection policy for automatic execution provider
     * (EP) selection.
     *
     * \param[in] session_options The OrtSessionOptions instance.
     * \param[in] delegate Delegate callback for custom selection.
     * \param[in] delegate_state Optional state that will be passed to the delegate callback.
     * nullptr if not required.
     *
     * \since Version 1.22
     */
    ORT_API2_STATUS(SessionOptionsSetEpSelectionPolicyDelegate,
                    _In_ OrtSessionOptions* session_options, _In_ EpSelectionDelegate delegate,
     ...
     *
     * \note This is not a unique identifier. It identifies the hardware type when combined with
     * vendor id. \since Version 1.22.
     */
     ...
     *
     * \param[in] ep_device The OrtEpDevice instance to query.
     * \return An OrtKeyValuePairs instance containing the execution provider options for the
     * device.
     *
     * \since Version 1.22.
     */
     ...
     *
     * Return a pointer to the OrtKeyValuePairs structure that contains the statistics of the
     * allocator and the user should call OrtApi::ReleaseKeyValuePairs.
     *
     * NOTE: If the allocator does not implement this function, the OrtKeyValuePairs instance will
     * be empty.
     *
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(AllocatorGetStats, _In_ const OrtAllocator* ort_allocator,
                    _Outptr_ OrtKeyValuePairs** out);
     ...
     * \param[in] device_id Device ID if there are multiple devices of the same type. e.g. 2 GPU
     * devices. \param[in] mem_type Memory type. Use OrtDeviceMemoryType_DEFAULT for device memory,
     * and OrtDeviceMemoryType_HOST_ACCESSIBLE (if applicable) for memory used to transfer between
     * the device and the CPU. Use the device_type and device_id of the GPU/NPU that the memory is
     * also accessible to. \param[in] alignment Alignment of the memory if required. Pass 0 for
     * default alignment. \param[in] allocator_type Allocator type. If
     * OrtAllocatorType::OrtArenaAllocator, the ORT arena will be used. Caveat: Support for
     * OrtArenaAllocator is currently limited to usage of internal ORT allocators via
     * CreateAllocator/CreateAndRegisterAllocator/CreateAndRegisterAllocatorV2. \param[out] out
    ...
    ORT_API2_STATUS(CreateMemoryInfo_V2, _In_ const char* name,
                    _In_ enum OrtMemoryInfoDeviceType device_type, _In_ uint32_t vendor_id,
                    _In_ int32_t device_id, _In_ enum OrtDeviceMemoryType mem_type,
     ...
     * \param[out] producer_output_index Optional output parameter set to the OrtNode instance's
     * output index that produces the value. Ignored if set to NULL. \snippet{doc} snippets.dox
     * OrtStatus Return Value \since Version 1.23.
     */
     ...
     *
     * Only nodes are considered "consumers" by this function. To check if an OrtValueInfo is a
     * graph output, call ValueInfo_IsGraphOutput().
     *
     * A single OrtNode may use a single value for more than one input (e.g., Mul(x, x)), so the
     * returned `num_consumers` may be larger than the number of unique OrtNode instances that
     * consume the value.
     *
     * \param[in] value_info The OrtValueInfo instance.
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(ValueInfo_GetValueNumConsumers, _In_ const OrtValueInfo* value_info,
                    _Out_ size_t* num_consumers);

    /** \brief Returns information (OrtNode and input index) for all consumer nodes that use the
     * value as an input.
     *
     * Only nodes are considered "consumers" by this function.
     *
     * Caller provides 2 pre-allocated arrays that will be filled with the OrtNode and input index
     * values. Use ValueInfo_GetValueNumConsumers() to get the number of consumers of the value.
     *
     * An OrtNode instance may appear multiple times if it uses the given value more than once.
     * Example: For a node MulNode(x, x) that consumes the value 'x' twice, the following is
     * returned:
     *   - nodes: [MulNode, MulNode]
     *   - input_indices: [0, 1]
     *
     * \param[in] value_info The OrtValueInfo instance.
     * \param[out] nodes Pre-allocated array of size `num_consumers` that is filled with OrtNode
     * instances. \param[out] input_indices Pre-allocated array of `num_consumers` elements that is
     * filled with input indices. Index is set to -1 for an "implicit" input to a consumer node that
     * contains a subgraph (e.g., If, Loop) with nodes that use the value internally. \param[in]
     * num_consumers The size of the `consumer_nodes` and `consumer_input_indices` arrays. Typical
     * usage sets this to the value of ValueInfo_GetValueNumConsumers(). An error status is returned
     * if `num_consumers` is less than the number of actual consumers.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     ...
     *
     * Sets the output parameter to NULL if the given OrtValueInfo does not represent an
     * initializer. Does not return an error status in this case.
     *
     ...
     * the initializer data on the first call to this function. If caller needs custom memory
     * mapping, use ValueInfo_GetExternalInitializerInfo to get the location of the initializer
     * data.
     *
     * \param[in] value_info The OrtValueInfo instance.
     * \param[out] initializer_value Output parameter set to the initializer value or NULL. Do not
     * cache the OrtValue as it is released when the owning OrtGraph is released.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
    ...
    /** \brief Get information about an external initializer (e.g., filepath, file offset, byte
     * size).
     *
     * Sets the output parameter `info` to NULL if the given OrtValueInfo does not represent an
     * initializer with external data. In this case, a NULL status (non-error) is returned.
     *
     * \param[in] value_info The OrtValueInfo instance.
     * \param[out] info Output parameter set to an OrtExternalInitializerInfo instance that can be
     * used to query file path, file offset, etc. ORT sets this to NULL if the OrtValueInfo does not
     * represent an external initializer. Must release with ReleaseExternalInitializerInfo.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     ...
     * \param[out] is_required_graph_input Output parameter set to true if the graph value is a
     ...
     *
     * Optional graph inputs were introduced in ONNX IR version 4. For ONNX IR version >=4, a graph
     * input with a matching initializer is an optional graph input with the initializer serving as
     * the default value. The matching initializer is also known as a non-constant initializer.
     *
     * \param[in] value_info The OrtValueInfo instance representing the graph value.
     * \param[out] is_optional_graph_input Output parameter set to true if the graph value is an
     ...
     *
     * \param[in] value_info The OrtValueInfo instance representing the graph value.
     * \param[out] is_graph_output Output parameter set to true if the graph value is a graph
     * output.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     */
    ORT_API2_STATUS(ValueInfo_IsGraphOutput, _In_ const OrtValueInfo* value_info,
     ...
     *
     * For ONNX IR version >=4, an initializer that serves as the default value for a matching graph
     * input is not a constant initializer.
     *
     * \param[in] value_info The OrtValueInfo instance representing the graph value.
     * \param[out] is_constant_initializer Output parameter set to true if the graph value is a
     ...
     *
     * \param[in] value_info The OrtValueInfo instance representing the graph value.
     * \param[out] is_from_outer_scope Output parameter set to true if the value is defined in an
     * outer scope (i.e., a parent graph).
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Graph_GetOnnxIRVersion, _In_ const OrtGraph* graph,
                    _Out_ int64_t* onnx_ir_version);

    /** \brief Returns the number of operator sets that the graph's model uses.
     *
     * \note An operator set is uniquely identified by the (domain, opset_version) pair. All models
     * must have at least one entry that specifies which entry of the ONNX operator set is used. The
     * ONNX domain is represented by an empty string.
     *
     * \param[in] graph The OrtGraph instance.
     * \param[out] num_operator_sets Output parameter set to the number of operator sets that the
     * graph's model uses.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Graph_GetNumOperatorSets, _In_ const OrtGraph* graph,
     ...
     * \param[out] opset_versions Pre-allocated array of `num_operator_sets` elements that is filled
     * with the opset version of the corresponding domain in the `domains` array. \param[in]
     * num_operator_sets The size of the `domains` and `opset_versions` arrays. Typical usage sets
     * this to the result of Graph_GetNumOperatorSets(). An error status is returned if
     * `num_operator_sets` is less than the actual number of operator sets.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
    ...
    ORT_API2_STATUS(Graph_GetOperatorSets, _In_ const OrtGraph* graph,
                    ...
                    _Out_writes_(num_operator_sets) int64_t* opset_versions,
     ...
     *
     * \param[in] graph The OrtGraph instance.
     * \param[out] inputs Pre-allocated array of `num_inputs` elements that is filled with the
     * graph's inputs. \param[in] num_inputs The size of the `inputs` array. Typical usage sets this
     * to the result of Graph_GetNumInputs(). An error status is returned if `num_inputs` is less
     * than the number of graph inputs.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     ...
     *
     * \param[in] graph The OrtGraph instance.
     * \param[out] outputs Pre-allocated array of `num_outputs` elements that is filled with the
     * graph's outputs. \param[in] num_outputs The size of the `outputs` array. Typical usage sets
     * this to the result of Graph_GetNumOutputs(). An error status is returned if `num_outputs` is
     * less than the number of graph outputs.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
    ...
    ORT_API2_STATUS(Graph_GetOutputs, _In_ const OrtGraph* graph,
                    _Out_writes_(num_outputs) const OrtValueInfo** outputs,
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Graph_GetNumInitializers, _In_ const OrtGraph* graph,
     ...
     * For ONNX IR version < 4, all initializers are constant.
     *
     * For ONNX IR version >= 4, an initializer with a name that matches a graph input is considered
     * a non-constant initializer.
     *
     * Call ValueInfo_GetInitializerValue to get the initializer's data.
     *
     * \param[in] graph The OrtGraph instance.
     * \param[out] initializers Pre-allocated array of `num_outputs` elements that is filled with
     * the initializers. \param[in] num_initializers The size of the `initializers` array. Typical
     * usage sets this to the result of Graph_GetNumInitializers(). An error status is returned if
     ...
     *
     * The nodes are sorted using a stable topological ordering. Callers are responsible for
     * maintaining their own node ordering if a different order is required.
     *
     * \param[in] graph The OrtGraph instance.
     * \param[out] nodes Pre-allocated array of `num_nodes` elements that is filled with the graph's
     * nodes. \param[in] num_nodes The size of the `nodes` array. Typical usage sets this to the
     ...
     *
     * \note The lifetime of "dst_graph" is tied to that of "src_graph", as they both internally
     * reference the same underlying graph. "dst_graph" preserves the input order of "src_graph",
     * and its output order corresponds to the outputs produced by the nodes in "nodes" with the
     * given order.
     *
     * \param[in] src_graph The source OrtGraph instance.
     * \param[in] nodes A subset of the nodes/OrtNodes in 'graph'.
     * \param[in] num_nodes Number of nodes.
     * \param[out] dst_graph An OrtGraph created from a given set of nodes. Must be released by
     * calling ReleaseGraph.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     ...
     *
     * \param[in] node The OrtNode instance.
     * \param[out] since_version The opset version in which the node's operator type was first
     * defined.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     ...
     * \param[in] node The OrtNode instance.
     * \param[out] inputs Pre-allocated array of `num_inputs` elements that is filled with the
     * node's inputs. \param[in] num_inputs The size of the `inputs` array. Typical usage sets this
     * to the result of Node_GetNumInputs(). An error status is returned if `num_inputs` is less
     * than the number of node inputs.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     ...
     * \param[in] node The OrtNode instance.
     * \param[out] outputs Pre-allocated array of `num_outputs` elements that is filled with the
     * node's outputs. \param[in] num_outputs The size of the `outputs` array. Typical usage sets
     * this to the result of Node_GetNumOutputs(). An error status is returned if `num_outputs` is
     * less than the number of node outputs.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     ...
     *
     * Certain operator types (e.g., If and Loop) contain nested subgraphs. The internal nodes
     * within the nested subgraphs may use values from the outer scope. Those "outer scope" values
     * are considered implicit inputs to the node that contains the subgraphs (e.g., the If or Loop
     * node).
     *
     * \param[in] node The OrtNode instance.
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Node_GetNumImplicitInputs, _In_ const OrtNode* node,
                    _Out_ size_t* num_implicit_inputs);

    /** \brief Get the implicit inputs, as OrtValueInfo instances, that are used within the given
     * node's subgraphs.
     *
     * \note Only certain operator types (e.g., If and Loop) contain nested subgraphs.
     * The internal nodes within the nested subgraphs may use values from the outer scope. Those
     * "outer scope" values are considered implicit inputs to the node that contains the subgraphs
     * (e.g., the If or Loop node).
     *
     * \param[in] node The OrtNode instance.
     * \param[out] implicit_inputs Pre-allocated array of `num_implicit_inputs` elements that is
     * filled the node's implicit inputs. \param[in] num_implicit_inputs The size of the
     * `implicit_inputs` array. Typical usage sets this to the result of
     * Node_GetNumImplicitInputs(). An error status is returned if `num_implicit_inputs` is less
     * than the number of node implicit inputs.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     ...
     *
     * \param[in] node The OrtNode instance.
     * \param[out] attributes Pre-allocated array of `num_attributes` elements that is filled with
     * the node's attributes. \param[in] num_attributes The size of the `num_attributes` array.
     *                           Typical usage sets this to the result of Node_GetNumAttributes().
     * An error status is returned if `num_attributes` is less than the number of node attributes.
     *
     * \note ONNX Runtime automatically sets optional (unset) attributes to their default values if
     * the default value is a constant expression that does not depend on other tensor/model
     * characteristics. Conv's 'kernel_shape' attribute is an example of an optional attribute that
     * does not have a constant default value. This function does not provide any unset optional
     * attributes without a constant default value.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
    ...
    ORT_API2_STATUS(Node_GetAttributes, _In_ const OrtNode* node,
                    _Out_writes_(num_attributes) const OrtOpAttr** attributes,
     ...
     * \param[out] attribute Output parameter set to the OrtOpAttr instance if an attribute by the
     * given name exists. For an unset optional attribute, `attribute` is set to NULL and a
     * non-error status is returned. For an invalid attribute name, `attribute` is set to NULL and
     * an error status with code ORT_NOT_FOUND is returned.
     *
     * \note ONNX Runtime automatically sets optional (unset) attributes to their default values if
     * the default value is a constant expression that does not depend on other tensor/model
     * characteristics. Conv's 'kernel_shape' attribute is an example of an optional attribute that
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Node_GetAttributeByName, _In_ const OrtNode* node,
     ...
     * \param[out] attr_tensor If successful, contains the 'TENSOR' attribute as a newly created
     OrtValue. Must be freed with OrtApi::ReleaseValue.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     ...
     *
     * \param[in] attribute The OrtOpAttr instance.
     * \param[out] name Output parameter set to the attribute's name. The name is a null-terminated
     * string.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     ...
     *
     * \note Only certain operator types (e.g., If and Loop) contain nested subgraphs. ONNX nodes
     * store subgraphs in their attributes, however, this function must be used to obtain subgraphs
     * from an OrtNode.
     *
     * \param[in] node The OrtNode instance.
     * \param[out] subgraphs Pre-allocated array of `num_subgraphs` elements that is filled with the
     * node's subgraphs. \param[in] num_subgraphs The size of the `num_subgraphs` array. Typical
     * usage sets this to the result of Node_GetNumSubgraphs(). An error status is returned if
     * `num_subgraphs` is less than the number of node subgraphs. \param[out] attribute_names
     * Optional pre-allocated array of `num_subgraphs` elements that is filled with the attribute
     * names that correspond to the subgraphs. Ignored if set to NULL.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
    ...
    ORT_API2_STATUS(Node_GetSubgraphs, _In_ const OrtNode* node,
                    _Out_writes_(num_subgraphs) const OrtGraph** subgraphs,
     ...
     * In another case, this API may also return NULL if `node` is obtained by calling
     * Graph_GetParentNode() on an OrtGraph that is a subgraph of a control-flow op, and the parent
     * graph has not been created yet, for example during ORT's GetCapability() when processing the
     * innermost subgraph.
     *
     * \param[in] node The OrtNode instance.
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Node_GetGraph, _In_ const OrtNode* node,
     ...
     * Caller can use Graph_GetModelPath to get the model's full path and construct the absolute
     * path to the external initializer file if necessary.
     *
     * \param[in] info The OrtExternalInitializerInfo instance.
     * \return The relative path to the file that stores the initializer's data. Do NOT free this
     * pointer.
     *
     * \since Version 1.23.
     */
     ...
     *
     * If a run configuration entry with key `config_key` doesn't exist, `config_value` will be set
     * to NULL.
     *
     ...
     * \return The configuration entry value. Either a null-terminated string if the entry was
     ...
     *         If memory_type is OrtDeviceMemoryType_DEFAULT and nullptr is returned the EP uses CPU
     * memory.
     *
     * \since Version 1.23
     */
     ...
     *
     * Both OrtDeviceMemoryType_DEFAULT and OrtDeviceMemoryType_HOST_ACCESSIBLE are optional for an
     * EP to provide. It is EP implementation dependent as to what is available.
     *
     * If a shared allocator already exists for the OrtEpDevice and OrtDeviceMemoryType, it is
     * replaced. This allows changing the shared allocator configuration from the default. e.g.
     * adding an arena.
     *
     ...
     * \param[in] allocator_type The type of allocator to create. Only OrtDeviceAllocator is valid
     * currently. \param[in] allocator_options Optional key-value pairs to configure the allocator.
     * If arena based, see include/onnxruntime/core/framework/allocator.h for the keys and values
     * that can be used. \param[out] allocator A pointer to the created shared allocator. Owned by
     * the OrtEnv instance.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     ...
     *
     * By default there is a shared allocator created for all OrtEpDevice instances, so if you get
     * the OrtMemoryInfo from the OrtEpDevice using EpDevice_MemoryInfo a shared allocator is
     * guaranteed to exist.
     *
     ...
     * \param[out] allocator A pointer to the shared allocator, or nullptr if no shared allocator
     * exists for the given memory info.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23
     ...
     *
     * The session must be fully initialized before calling this function as the input locations are
     * not known until this has occurred.
     *
     * \param[in] session The OrtSession instance.
     * \param[out] inputs_memory_info Pre-allocated array of size `num_inputs` that will be filled
     * with the OrtMemoryInfo* value for each input. The order is the same as returned by
     * SessionGetInputName. \param[in] num_inputs The number of inputs in the session. Must match
     ...
     * The user can pre-allocate an OrtValue using this information or use IOBinding to keep the
     * data on the device. ORT will copy the output to CPU otherwise.
     *
     * The session must be fully initialized before calling this function as the output locations
     * are not known until this has occurred.
     *
     * \param[in] session The OrtSession instance.
     * \param[out] outputs_memory_info Pre-allocated array of size `num_outputs` that will be filled
     * with OrtMemoryInfo* values for each output. The order is the same as returned by
     * SessionGetOutputName. \param[in] num_outputs The number of outputs in the session. Must match
     ...
     * or if the OrtEpDevice was manually added to the session using
     * SessionOptionsAppendExecutionProvider_V2.
     *
     * If an OrtEpDevice is not available for the input a nullptr is returned.
     *
     * The returned OrtEpDevice can be used to create an OrtSyncStream via
     * CreateSyncStreamForEpDevice to asynchronously provide input to the inference session Run.
     *
     * The session must be fully initialized before calling this function as the assigned EPs are
     * not known until this has occurred.
     *
     * \param[in] session The OrtSession instance.
     * \param[out] inputs_ep_devices Pre-allocated array of size `num_inputs` that will be filled
     * with OrtEpDevice* values for each input. The order is the same as returned by
     * SessionGetInputName. \param[in] num_inputs The number of inputs in the session. Must match
     * SessionGetInputCount.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     ...
     *
     * An error code of ORT_NOT_IMPLEMENTED will be returned if the EP does not support
     * OrtSyncStream.
     *
     ...
     *   - Use GetSharedAllocator to get the shared allocator for the OrtMemoryInfo if you need to
     * allocate memory on the device.
     *   - Use CreateTensorAsOrtValue, CreateTensorWithDataAsOrtValue or
     * CreateTensorWithDataAndDeleterAsOrtValue to create an OrtValue containing a tensor depending
     * on whether you have existing data or not, and whether you want ORT to free the existing data
     * once it is done with the OrtValue.
     *
     * \param[in] env The OrtEnv instance to use. The data transfer implementation is provided by an
     * execution provider that is registered in this OrtEnv. \param[in] src_tensors Array of
     * OrtValue instances containing the source tensors to copy. \param[in] dst_tensors Array of
     * OrtValue instances to copy the source tensors to. \param[in] stream Optional OrtSyncStream
     * that can be used to perform the copy asynchronously. May be nullptr. \param[in] num_tensors
     * The number of tensors to copy. The size of `src_tensors` and `dst_tensors` must match.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23
    ...
    ORT_API2_STATUS(CopyTensors, _In_ const OrtEnv* env,
                    ...
                    _In_opt_ OrtSyncStream* stream, _In_ size_t num_tensors);
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(Graph_GetModelMetadata, _In_ const OrtGraph* graph,
                    _Outptr_ OrtModelMetadata** out);
     ...
     * \param[in] compatibility_info The compatibility info string produced when the model was
     * compiled. \param[out] out_status The resulting compatibility status for the EP devices.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     ...
     *
     * \param[in] filepath The relative path to the file that stores the initializer's data. ORT
     * copies this path string. \param[in] file_offset The byte offset where the initializer's data
     * is stored within the file. \param[in] byte_size The size in bytes of the initializer's data
     * within the file. \param[out] out Output parameter set to the new OrtExternalInitializerInfo
     * instance. Must be released by calling ReleaseExternalInitializerInfo().
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
     */
    ORT_API2_STATUS(CreateExternalInitializerInfo, _In_ const ORTCHAR_T* filepath,
     ...
     * Config entries are set on the ::OrtSessionOptions and are accessible in custom operator
     * kernels.
     *
     * Used in the CreateKernel callback of an OrtCustomOp to access all session configuration
     * entries during kernel construction.
     *
     * \param[in] info An instance of ::OrtKernelInfo.
     * \param[out] out A pointer to a newly created OrtKeyValuePairs instance containing all config
     * entries. Note: the user should call OrtApi::ReleaseKeyValuePairs.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.24
     */
    ORT_API2_STATUS(KernelInfo_GetConfigEntries, _In_ const OrtKernelInfo* info,
                    _Outptr_ OrtKeyValuePairs** out);
     ...
     * string (including null-terminator), and a success status is returned.
     *
     * If the `size` parameter is greater than or equal to the string's size and `out` is not
     * nullptr, the value of `size` is set to the true size of the string (including
     * null-terminator), the provided memory is filled with the string's contents, and a success
     * status is returned.
     *
     ...
     * \param[in] info An instance of ::OrtKernelInfo.
     * \param[out] out Memory location into which to write the UTF-8 null-terminated string
     * representing the operator domain. \param[in,out] size Pointer to the size of the `out`
     * buffer. See above comments for details.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.24
     */
    ORT_API2_STATUS(KernelInfo_GetOperatorDomain, _In_ const OrtKernelInfo* info,
                    _Out_opt_ char* out, _Inout_ size_t* size);
     ...
     * \param[in] info An instance of ::OrtKernelInfo.
     * \param[out] out Memory location into which to write the UTF-8 null-terminated string
     * representing the operator type. \param[in,out] size Pointer to the size of the `out` buffer.
    ...
    /** \brief Get the opset version in which the given node's operator type was first defined from
     * ::OrtKernelInfo.
     *
     * \param[in] info An instance of ::OrtKernelInfo.
     * \param[out] since_version The opset version in which the node's operator type was first
     ...
     * This allows validation that outputs are placed on the expected device for external resource
     * sharing.
     *
     * The EP device for each output is determined by which execution provider will produce that
     * output during inferencing. This information is useful for:
     ...
     * \param[in] num_outputs The number of outputs in the session. Must match
     * SessionGetOutputCount.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     ...
     * Populates a user-provided array with pointers to OrtHardwareDevice instances. The caller is
     * responsible for allocating the array with sufficient space (use GetNumHardwareDevices() to
     * get the count).
     *
     * The returned pointers reference internal ORT data structures that are discovered once at
     * process startup and remain valid for the lifetime of the OrtEnv. The caller does not need to
     * release these pointers, but should not use them after calling ReleaseEnv().
     *
     * \param[in] env The OrtEnv instance where device discovery results are stored.
    ...
    /** \brief Check for known incompatibility issues between hardware device and a specific
     * execution provider.
     *
     ...
     * If returned incompatibility details have non-zero reasons, it indicates the device is not
     * compatible. However, if returned detail have reason == 0, it doesn't guarantee 100%
     * compatibility for all models, as models may have specific requirements.
     *
     ...
     * \param[in] ep_name The name of the execution provider to check. Required and cannot be null
     * or empty. \param[in] hw The hardware device to check for incompatibility. \param[out] details
     * Compatibility details including reasons for incompatibility if any. Must be freed with
    ...
    ORT_API2_STATUS(GetHardwareDeviceEpIncompatibilityDetails, _In_ const OrtEnv* env,
                    _In_ const char* ep_name, _In_ const OrtHardwareDevice* hw,
     ...
     *
     * \param[in] details The OrtDeviceEpIncompatibilityDetails instance to query.
     * \param[out] error_code Pointer to store the EP-specific error code. A value of 0 indicates no
     * error code was set.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     ...
     * provider from the model's metadata properties. This is only applicable to models that have
     * been precompiled for an EP (e.g., via OrtCompileApi). Standard ONNX models do not contain
     * this information.
     *
     ...
     * for scenarios where applications need to check compatibility before deciding whether to
     * proceed with session creation, such as providing early user feedback.
     *
     * \note This operation parses the full ONNX ModelProto from disk. For very large models,
     * consider using GetCompatibilityInfoFromModelBytes with a pre-loaded buffer if the model is
     * already in memory.
     *
     ...
     * \param[in] allocator Allocator to use for the output string. Use
     * OrtApi::GetAllocatorWithDefaultOptions. \param[out] compatibility_info Output pointer to the
     * compatibility info string. Returns nullptr if no compatibility info exists for the specified
     * EP. Caller must free with OrtApi::AllocatorFree when non-null.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.24.
     */
    ORT_API2_STATUS(GetCompatibilityInfoFromModel, _In_ const ORTCHAR_T* model_path,
                    _In_ const char* ep_type, _Inout_ OrtAllocator* allocator,
     ...
     * Useful when precompiled models are loaded from encrypted storage, network, or other non-file
     * sources.
     *
     * \note This API performs standalone model parsing, separate from session creation. This means
     * the protobuf parsing cost is incurred here and again during session creation. It is intended
     ...
     * \param[out] out Output parameter set to the new OrtEnv instance. Must be freed with
     * OrtApi::ReleaseEnv.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.24
     */
    ORT_API2_STATUS(CreateEnvWithOptions, _In_ const OrtEnvCreationOptions* options,
                    _Outptr_ OrtEnv** out);

    /** \brief Get information about the subgraphs assigned to each execution provider (EP) and the
     * nodes within.
     *
     * Each returned OrtEpAssignedSubgraph instance contains details of the subgraph/nodes assigned
     * to an execution provider, including the execution provider's name, and the name, domain, and
     * operator type for every node.
     *
     * For compiling execution providers, a single OrtEpAssignedSubgraph instance contains
     * information about the nodes that are fused and compiled within a single subgraph assigned to
     * the execution provider.
     *
     * For execution providers that use kernel registration (e.g., CPU EP), each node with a
     * registered kernel is contained in its own OrtEpAssignedSubgraph instance.
     *
     ...
     *       Refer to onnxruntime_session_options_config_keys.h. Otherwise, if not enabled, this
     * function returns a status with error code ORT_FAIL.
     *
     * \note The information reported by this function is obtained immediately after running basic
     * optimizations on the original graph if the session optimization level is set to
     * ORT_ENABLE_BASIC or higher. If the session optimization level is set to ORT_DISABLE_ALL, only
     * minimal/required optimizations are run before the information is collected.
     *
     * \param[in] session The OrtSession instance.
     * \param[out] ep_subgraphs Output parameter set to the array of OrtEpAssignedSubgraph
     * instances. \param[out] num_ep_subgraphs Output parameter set to the number of elements in the
     * `ep_subgraphs` array.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     ...
     * \param[out] out Output parameter set to the execution provider's name as a UTF-8
     * null-terminated string. Owned by the OrtEpAssignedSubgraph instance (do not free).
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.24.
     ...
     * \param[out] num_ep_nodes Output parameter set to the number of OrtEpAssignedNode instance
     * returned.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(EpAssignedNode_GetName, _In_ const OrtEpAssignedNode* ep_node,
                    _Outptr_ const char** out);
     ...
     * \since Version 1.24.
     */
    ORT_API2_STATUS(EpAssignedNode_GetDomain, _In_ const OrtEpAssignedNode* ep_node,
                    _Outptr_ const char** out);

    /** \brief Get the operator type of the node assigned to an execution provider.
     *
     * \param[in] ep_node The OrtEpAssignedNode instance.
     * \param[out] out Output parameter set to the node's operator type as a UTF-8 null-terminated
     * string. Owned by the OrtEpAssignedNode instance (do not free).
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.24.
     */
    ORT_API2_STATUS(EpAssignedNode_GetOperatorType, _In_ const OrtEpAssignedNode* ep_node,
                    _Outptr_ const char** out);
     ...
     * \since 1.24
     */
    ORT_API_T(void, RunOptionsSetSyncStream, _Inout_ OrtRunOptions* options,
              _In_ OrtSyncStream* sync_stream);

    /** \brief Get the element data type and shape for an OrtValue that represents a Tensor (scalar,
     * dense, or sparse).
     *
     * \note This function is an alternative to ::GetTensorTypeAndShape() that does not allocate a
     * new array for the shape data. The OrtValue instance's internal shape data is returned
     * directly.
     *
     ...
     * \param[out] shape_data Output parameter set to the OrtValue instance's internal shape data
     * array. For a scalar, `shape_data` is NULL and `shape_data_count` is 0. Must not be released
     * as it is owned by the OrtValue instance. This pointer becomes invalid when the OrtValue is
     * released or if the underlying shape data is updated or reallocated. \param[out]
     * shape_data_count Output parameter set to the number of elements in `shape_data`.
 ...
 */
struct OrtModelEditorApi {
    // Model building/editing requires a full build. We return nullptr from GetModelEditorApi if
    // this is a minimal build, so it doesn't matter if there are no function pointers in this
    // struct as a user will never get an OrtModelEditorApi instance. We do however need a dummy
    // field to avoid empty struct warning.
#if defined(ORT_MINIMAL_BUILD)
    const bool not_defined_in_this_build;
#else
    /** \brief Create an OrtTypeInfo instance for a Tensor.
     *
     * Create an OrtTypeInfo instance for a Tensor to use as graph inputs/outputs with the Model
     * Editor API.
     *
     ...
     *
     * Create an OrtTypeInfo instance for a SparseTensor to use as graph inputs/outputs with the
     * Model Editor API.
     *
     * User can release `tensor_info` after creating the OrtTypeInfo.
     *
     ...
     *
     * Create an OrtTypeInfo instance for a Map to use as graph inputs/outputs with the Model Editor
     * API.
     *
    ...
    ORT_API2_STATUS(CreateMapTypeInfo, ONNXTensorElementDataType map_key_type,
                    _In_ const OrtTypeInfo* map_value_type, _Outptr_ OrtTypeInfo** type_info);

    /** \brief Create an OrtTypeInfo instance for a Sequence.
     *
     * Create an OrtTypeInfo instance for a Sequence to use as graph inputs/outputs with the Model
     * Editor API.
     *
     ...
     *
     * Create an OrtTypeInfo instance for an Optional to use as graph inputs/outputs with the Model
     * Editor API.
     *
     ...
     * \param[in] type_info The type information for the input or output. The provided value is
     * copied. \param[out] value_info The OrtValueInfo instance.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.22.
     ...
     * The OrtGraph takes ownership of the OrtValueInfo instances and you should NOT call
     * ReleaseOrtValueInfo.
     *
     ...
     * The OrtGraph takes ownership of the OrtValueInfo instances provided and you should NOT call
     * ReleaseOrtValueInfo.
     *
     * \param[in] graph The OrtGraph instance to update.
     ...
     *
     * Pre-existing memory:
     *    Use CreateTensorWithDataAsOrtValue or CreateTensorWithDataAndDeleterAsOrtValue to create
     * an OrtValue with a tensor that contains a pointer to the existing data. Set
     * `data_is_external` to true.
     *
     *    The pointer must remain valid for the duration of the inference session.
     *    If using CreateTensorWithDataAsOrtValue you are responsible for freeing the memory after
     * the inference session is released. If using CreateTensorWithDataAndDeleterAsOrtValue, ORT
     * will free the memory using the provided deleter as soon as the OrtValue is no longer in use.
     *
     ...
     *
     *          ONNX shape inferencing does not support external data. An initializer involved in
     * shape inferencing is typically small (a single value or limited by the rank of a tensor) and
     * uses less than 128 bytes of memory, so this limit acts as a simple catch-all rule to avoid
     ...
     *
     * Add the node to the graph. The OrtGraph will take ownership of OrtNode and you should NOT
     * call ReleaseOrtNode.
     *
     * \param[in] graph The OrtGraph instance to update.
     ...
     *                           If augmenting an existing model add additional opset versions if
     * needed. \param[in] opset_entries_len The number of domain_names and opset_versions entries.
     ...
     * The OrtModel should have been populated with an OrtGraph containing nodes and initializers,
     * and SetGraphInputs and SetGraphOutputs must have been called. This will validate the model,
     * run optimizers, and prepare the session for inferencing.
     *
     ...
     *
     * Create an OrtSession with an existing model that will be augmented with additional nodes and
     * initializers. Nodes can be added before or after the existing nodes in the model. ONNX
     * Runtime will connect the nodes when the model is finalized.
     *
     * To add nodes and initializers to the existing model, first create an OrtModel using
     * CreateModel. Add nodes and initializers to the OrtModel using AddNodeToGraph and
     * AddInitializerToGraph. Graph inputs/outputs should be updated with SetGraphInputs and
     * SetGraphOutputs as needed to reflect changes made by the new nodes. The list of graph
     * inputs/outputs should be for the overall model and not just the new nodes.
     *
     * Add the new information from the OrtModel to the original model using ApplyModelToSession,
     * and prepare the session for inferencing by calling FinalizeModelEditorSession.
     *
     * \param{in} env The OrtEnv instance.
     ...
     *
     * When using the Model Editor API to augment a model, any new nodes must conform to the opset
     * version of the original model. To do that the user must be able to discover that opset
     * version. Returns an error if the domain is not used in the model.
     *
     ...
     *
     * Adds new nodes and updates graph inputs/outputs using `model` to augment the original ONNX
     * model in the session. All changes will be validated. Call FinalizeModelEditorSession to
     * prepare the session for inferencing.
     *
     * Existing input/outputs will only be updated if the OrtGraph inputs/outputs are set in the
     * OrtModel. i.e. you don't need to call SetGraphInputs/SetGraphOutputs if they are unchanged.
     *
     ...
     * CreateModelEditorSession[FromArray]. \param[in] model OrtModel containing new nodes, new
     * initializers, and updated graph input and/or output info.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.22.
     ...
     * \param[in] prepacked_weights_container Optional OrtPrepackedWeightsContainer to use for the
     session. Set to nullptr if not used.
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.22.
     */
    ORT_API2_STATUS(FinalizeModelEditorSession, _Inout_ OrtSession* session,
                    _In_ const OrtSessionOptions* options,
 ...
 *
 * Execution providers that support compilation fuse a subgraph into an EPContext node that wraps a
 * provider-specific binary representation of the subgraph. For more details about the EPContext
 * design, refer to: \htmlonly <a
 * href="https://onnxruntime.ai/docs/execution-providers/EP-Context-Design.html">EPContext design
 * document.</a> \endhtmlonly
 *
 ...
 *
 *   status = compile_api->CreateModelCompilationOptionsFromSessionOptions(env, session_options,
 * &compile_options); status =
 * compile_api->ModelCompilationOptions_SetInputModelPath(compile_options, ORT_TSTR("model.onnx"));
 *   status = compile_api->ModelCompilationOptions_SetOutputModelPath(compile_options,
 * ORT_TSTR("model.compiled.onnx")); status = compile_api->CompileModel(env, compile_options);
...
struct OrtCompileApi {
    ...
    /** \brief Creates an OrtModelCompilationOptions object from an existing OrtSessionOptions
     * object.
     *
     * An OrtModelCompilationOptions object contains the settings used to generate a compiled ONNX
     ...
     * \param[in] session_options The OrtSessionOptions instance from which to create the
     * OrtModelCompilationOptions. \param[out] out The created OrtModelCompilationOptions instance.
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.22.
     */
     ...
     * \param[in] input_model_path Null terminated string of the path (wchar on Windows, char
     * otherwise).
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.22.
     */
    ORT_API2_STATUS(ModelCompilationOptions_SetInputModelPath,
     ...
     *
     * If the output model's location is not set, ONNX Runtime will generate an output file with a
     * path based on the input model's file path. Examples: /Path/my_model.onnx ->
     * /Path/my_model_ctx.onnx /Path/my_model -> /Path/my_model_ctx.onnx
     *
     * \param[in] model_compile_options The OrtModelCompilationOptions instance.
     * \param[in] output_model_path Null terminated string of the path (wchar on Windows, char
     * otherwise).
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.22.
     */
    ORT_API2_STATUS(ModelCompilationOptions_SetOutputModelPath,
                    _In_ OrtModelCompilationOptions* model_compile_options,
                    _In_ const ORTCHAR_T* output_model_path);

    /** \brief Optionally sets the file that should store external initializers for the compiled
     * ONNX model. If not set, initializers are stored within the model.
     *
     * Only initializers for nodes that were not compiled are stored in the external initializers
     * file. Compiled nodes contain their initializer data within the `ep_cache_context` attribute
     * of EPContext nodes. Refer to ModelCompilationOptions_SetEpContextEmbedMode.
     *
     * \param[in] model_compile_options The OrtModelCompilationOptions instance.
     * \param[in] external_initializers_file_path Null terminated string of the path to the file.
     * \param[in] external_initializers_size_threshold Initializers larger than this threshold are
     * stored in the file.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
    ...
    ORT_API2_STATUS(ModelCompilationOptions_SetOutputModelBuffer,
                    _In_ OrtModelCompilationOptions* model_compile_options,
                    _Inout_ OrtAllocator* allocator, _Outptr_ void** output_model_buffer_ptr,
                    _Out_ size_t* output_model_buffer_size_ptr);

    /** \brief Enables or disables the embedding of EPContext binary data into the
     * `ep_cache_context` attribute of EPContext nodes. Defaults to false.
     *
     * If enabled, the `ep_cache_context` attribute of EPContext nodes will store the context binary
     * data, which may include weights for compiled subgraphs.
     *
     * If disabled, the `ep_cache_context` attribute of EPContext nodes will contain the path to the
     * file containing the context binary data. The path is set by the execution provider creating
     * the EPContext node.
     *
     ...
     *
     * \param[in] model_compile_options The OrtModelCompilationOptions instance.
     * \param[in] embed_ep_context_in_model True to embed EPContext binary data into the EPContext
     * node `ep_cache_context` attributes.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.22.
     ...
     *
     * \param[in] env OrtEnv object.
     * \param[in] model_options The compilation options that defines compilation options for a
     * model.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.22.
     */
    ORT_API2_STATUS(CompileModel, _In_ const OrtEnv* env,
                    _In_ const OrtModelCompilationOptions* model_options);

    /** \brief Sets flags from OrtCompileApiFlags that represent one or more boolean options to
     * enable.
     *
     * \param[in] model_compile_options The OrtModelCompilationOptions instance.
     ...
     * \since Version 1.23.
     */
    ORT_API2_STATUS(ModelCompilationOptions_SetFlags,
                    _In_ OrtModelCompilationOptions* model_compile_options, uint32_t flags);
     ...
     *
     * \param[in] model_compile_options The OrtModelCompilationOptions instance.
     * \param[in] output_directory Null terminated string of the path (wchar on Windows, char
     * otherwise). \param[in] model_name Null terminated string of the model name (wchar on Windows,
     * char otherwise).
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
    ...
    ORT_API2_STATUS(ModelCompilationOptions_SetEpContextBinaryInformation,
                    _In_ OrtModelCompilationOptions* model_compile_options,
                    _In_ const ORTCHAR_T* output_directory, _In_ const ORTCHAR_T* model_name);
    ...
    /** \brief Sets a OrtWriteBufferFunc function that is called by ORT to write out the output
     * model's serialized ONNX bytes.
     *
     * The provided write function may be called repeatedly until then entire output model has been
     * written out. Each call to the write function is expected to consume the entire input buffer.
     *
     * The output model's destination (e.g., file path, memory buffer, or stream) can be set with
     * any of the functions that begin with ModelCompilationOptions_SetOutputModel____.
     *
     * \param[in] model_compile_options The OrtModelCompilationOptions instance.
     * \param[in] write_func The OrtWriteBufferFunc function called by ORT when writing out the
     * model. \param[in] state Opaque state passed as the first argument to OrtWriteBufferFunc. Can
     * be NULL.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
    ...
    /** \brief Sets a OrtGetInitializerLocationFunc function that is called by ORT for every
     * initializer in the generated model. Allows implementer to specify whether initializers should
     * be stored within the model or externally.
     *
     * \param[in] model_compile_options The OrtModelCompilationOptions instance.
     * \param[in] get_initializer_location_func The OrtGetInitializerLocationFunc function called by
     * ORT when to determine the location of the initializer. \param[in] state Opaque state passed
     * as the first argument to OrtGetInitializerLocationFunc. Can be NULL.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.23.
    ...
    ORT_API2_STATUS(ModelCompilationOptions_SetOutputModelGetInitializerLocationFunc,
                    _In_ OrtModelCompilationOptions* model_compile_options,
                    _In_ OrtGetInitializerLocationFunc get_initializer_location_func,
                    _In_ void* state);
};

/**
 * \brief The OrtInteropApi struct provides functions for external resource interop with execution
 * providers.
 *
 ...
 *
 * The API is designed to be EP-agnostic and can be extended to support various GPU interop
 * mechanisms (D3D12 shared handles, CUDA external memory, Vulkan, etc.).
 *
 ...
 *   status = interop_api->CanImportMemory(importer, ORT_EXTERNAL_MEMORY_HANDLE_TYPE_D3D12_RESOURCE,
 * &can_import); if (can_import) { OrtExternalMemoryHandle* mem_handle = NULL; status =
...
struct OrtInteropApi {
     ...
     * \param[out] out_importer Output parameter set to the created OrtExternalResourceImporter
     * instance, or nullptr if the EP does not support external resource import.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.24.
     ...
     * ReleaseExternalMemoryHandle to free it.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.24.
     ...
     *                        The caller owns the returned tensor and must call ReleaseValue to free
     * it.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
    ...
    ORT_API2_STATUS(WaitSemaphore, _In_ OrtExternalResourceImporter* importer,
                    _In_ OrtExternalSemaphoreHandle* semaphore_handle, _In_ OrtSyncStream* stream,
                    _In_ uint64_t value);
