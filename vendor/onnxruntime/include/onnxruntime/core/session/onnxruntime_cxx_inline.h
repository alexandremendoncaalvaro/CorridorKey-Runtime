...
inline MemoryInfo::MemoryInfo(const char* name, OrtMemoryInfoDeviceType device_type,
                              uint32_t vendor_id, uint32_t device_id, OrtDeviceMemoryType mem_type,
                              size_t alignment, OrtAllocatorType allocator_type) {
    ThrowOnError(GetApi().CreateMemoryInfo_V2(name, device_type, vendor_id, device_id, mem_type,
                                              alignment, allocator_type, &this->p_));
}
...
inline void TensorRTProviderOptions::Update(
    ...
    ThrowOnError(
        GetApi().UpdateTensorRTProviderOptions(p_, keys.data(), values.data(), options.size()));
}
...
inline std::string TensorRTProviderOptions::GetTensorRTProviderOptionsAsString() const {
    ...
    std::unique_ptr<void, detail::AllocatedFree> options_str_g(options_str,
                                                               detail::AllocatedFree(allocator));
...
inline void CUDAProviderOptions::Update(
    const std::unordered_map<std::string, std::string>& options) {
    std::vector<const char*> keys;
    std::vector<const char*> values;
    keys.reserve(options.size());
    ...
    ThrowOnError(
        GetApi().UpdateCUDAProviderOptions(p_, keys.data(), values.data(), options.size()));
}
...
inline ExternalInitializerInfo::ExternalInitializerInfo(const ORTCHAR_T* filepath,
                                                        int64_t file_offset, size_t byte_size) {
    ThrowOnError(
        GetApi().CreateExternalInitializerInfo(filepath, file_offset, byte_size, &this->p_));
}

inline Status ExternalInitializerInfo::Create(const ORTCHAR_T* filepath, int64_t file_offset,
                                              size_t byte_size,
                                              /*out*/ ExternalInitializerInfo& out) {
    OrtExternalInitializerInfo* info = nullptr;
    OrtStatus* status =
        GetApi().CreateExternalInitializerInfo(filepath, file_offset, byte_size, &info);
    if (status != nullptr) {
...
inline EpDevice::EpDevice(OrtEpFactory& ep_factory, ConstHardwareDevice& hardware_device,
    ...
    ThrowOnError(
        GetEpApi().CreateEpDevice(&ep_factory, hardware_device, ep_metadata, ep_options, &p_));
}
...
inline Status Env::CopyTensors(const std::vector<Value>& src_tensors,
                               const std::vector<Value>& dst_tensors, OrtSyncStream* stream) const {
    if (src_tensors.size() != dst_tensors.size()) {
        return Status("Source and destination tensor vectors must have the same size",
    ...
    const OrtValue* const* src_tensors_ptr =
        reinterpret_cast<const OrtValue* const*>(src_tensors.data());
    OrtValue* const* dst_tensors_ptr = reinterpret_cast<OrtValue* const*>(dst_tensors.data());
    OrtStatus* status =
        GetApi().CopyTensors(p_, src_tensors_ptr, dst_tensors_ptr, stream, src_tensors.size());
    return Status(status);
}

inline UnownedAllocator Env::CreateSharedAllocator(const OrtEpDevice* ep_device,
                                                   OrtDeviceMemoryType mem_type,
                                                   OrtAllocatorType allocator_type,
                                                   const OrtKeyValuePairs* allocator_options) {
    OrtAllocator* p;
    ThrowOnError(GetApi().CreateSharedAllocator(p_, ep_device, mem_type, allocator_type,
                                                allocator_options, &p));
...
inline OrtCompiledModelCompatibility GetModelCompatibilityForEpDevices(
    const std::vector<ConstEpDevice>& ep_devices, const char* compatibility_info) {
    if (ep_devices.empty()) {
    ...
    ThrowOnError(GetApi().GetModelCompatibilityForEpDevices(
        reinterpret_cast<const OrtEpDevice* const*>(ptrs.data()), ptrs.size(), compatibility_info,
        &status));
    return status;
}

inline AllocatedStringPtr GetCompatibilityInfoFromModelAllocated(const ORTCHAR_T* model_path,
                                                                 const char* ep_type,
                                                                 OrtAllocator* allocator) {
    char* compat_info = nullptr;
    ThrowOnError(
        GetApi().GetCompatibilityInfoFromModel(model_path, ep_type, allocator, &compat_info));
    return AllocatedStringPtr(compat_info, detail::AllocatedFree(allocator));
}

inline AllocatedStringPtr GetCompatibilityInfoFromModelBytesAllocated(const void* model_data,
                                                                      size_t model_data_length,
                                                                      const char* ep_type,
                                                                      OrtAllocator* allocator) {
    char* compat_info = nullptr;
    ThrowOnError(GetApi().GetCompatibilityInfoFromModelBytes(model_data, model_data_length, ep_type,
                                                             allocator, &compat_info));
    return AllocatedStringPtr(compat_info, detail::AllocatedFree(allocator));
}
...
inline ModelCompilationOptions::ModelCompilationOptions(const Env& env,
    ...
    ThrowOnError(GetCompileApi().CreateModelCompilationOptionsFromSessionOptions(
        env, session_options, &this->p_));
}
...
inline Status CompileModel(const Env& env,
                           const ModelCompilationOptions& model_compilation_options) {
...
inline ModelCompilationOptions& ModelCompilationOptions::SetInputModelPath(
    ...
    Ort::ThrowOnError(
        GetCompileApi().ModelCompilationOptions_SetInputModelPath(this->p_, input_model_path));
    return *this;
}
...
inline ModelCompilationOptions& ModelCompilationOptions::SetInputModelFromBuffer(
    ...
    Ort::ThrowOnError(GetCompileApi().ModelCompilationOptions_SetInputModelFromBuffer(
        this->p_, input_model_data, input_model_data_size));
    return *this;
}
...
inline ModelCompilationOptions& ModelCompilationOptions::SetOutputModelPath(
    ...
    Ort::ThrowOnError(
        GetCompileApi().ModelCompilationOptions_SetOutputModelPath(this->p_, output_model_path));
    return *this;
}
...
inline ModelCompilationOptions& ModelCompilationOptions::SetEpContextBinaryInformation(
    ...
    Ort::ThrowOnError(GetCompileApi().ModelCompilationOptions_SetEpContextBinaryInformation(
        this->p_, output_directory, model_name));
    return *this;
}
...
inline ModelCompilationOptions& ModelCompilationOptions::SetOutputModelExternalInitializersFile(
    ...
    Ort::ThrowOnError(
        GetCompileApi().ModelCompilationOptions_SetOutputModelExternalInitializersFile(
            this->p_, file_path, initializer_size_threshold));
    return *this;
}

inline ModelCompilationOptions& ModelCompilationOptions::SetOutputModelGetInitializerLocationFunc(
    OrtGetInitializerLocationFunc get_initializer_location_func, void* state) {
    Ort::ThrowOnError(
        GetCompileApi().ModelCompilationOptions_SetOutputModelGetInitializerLocationFunc(
            this->p_, get_initializer_location_func, state));
    return *this;
}
...
inline ModelCompilationOptions& ModelCompilationOptions::SetOutputModelBuffer(
    ...
    Ort::ThrowOnError(GetCompileApi().ModelCompilationOptions_SetOutputModelBuffer(
        this->p_, allocator, output_model_buffer_ptr, output_model_buffer_size_ptr));
    return *this;
}

inline ModelCompilationOptions& ModelCompilationOptions::SetOutputModelWriteFunc(
    OrtWriteBufferFunc write_func, void* state) {
    Ort::ThrowOnError(GetCompileApi().ModelCompilationOptions_SetOutputModelWriteFunc(
        this->p_, write_func, state));
    return *this;
}
...
inline ModelCompilationOptions& ModelCompilationOptions::SetEpContextEmbedMode(
    ...
    Ort::ThrowOnError(GetCompileApi().ModelCompilationOptions_SetEpContextEmbedMode(
        this->p_, embed_ep_context_in_model));
    return *this;
}
...
inline ModelCompilationOptions& ModelCompilationOptions::SetGraphOptimizationLevel(
    ...
    Ort::ThrowOnError(GetCompileApi().ModelCompilationOptions_SetGraphOptimizationLevel(
        this->p_, graph_optimization_level));
    return *this;
}
...
template <typename T>
inline std::string ConstSessionOptionsImpl<T>::GetConfigEntryOrDefault(
    const char* config_key, const std::string& def) const {
...
namespace {
template <typename T>
void SessionOptionsAppendEP(detail::SessionOptionsImpl<T>& session_options, Env& env,
...
template <typename T>
inline SessionOptionsImpl<T>& SessionOptionsImpl<T>::SetEpSelectionPolicy(
...
template <typename T>
inline SessionOptionsImpl<T>& SessionOptionsImpl<T>::SetEpSelectionPolicy(
    EpSelectionDelegate delegate, void* state) {
...
template <typename T>
inline SessionOptionsImpl<T>& SessionOptionsImpl<T>::AddFreeDimensionOverride(
...
template <typename T>
inline SessionOptionsImpl<T>& SessionOptionsImpl<T>::AddFreeDimensionOverrideByName(
...
inline std::vector<ConstMemoryInfo> ConstSessionImpl<T>::GetMemoryInfoForInputs() const {
    ...
    if (num_inputs > 0) {
        ...
        ThrowOnError(GetApi().SessionGetMemoryInfoForInputs(
            this->p_, reinterpret_cast<const OrtMemoryInfo**>(mem_infos.data()), num_inputs));
    }
...
inline std::vector<ConstMemoryInfo> ConstSessionImpl<T>::GetMemoryInfoForOutputs() const {
    ...
    if (num_outputs > 0) {
        ...
        ThrowOnError(GetApi().SessionGetMemoryInfoForOutputs(
            this->p_, reinterpret_cast<const OrtMemoryInfo**>(mem_infos.data()), num_outputs));
    }
    return mem_infos;
}
...
inline std::vector<ConstEpDevice> ConstSessionImpl<T>::GetEpDeviceForInputs() const {
    ...
    if (num_inputs > 0) {
        ...
        ThrowOnError(GetApi().SessionGetEpDeviceForInputs(
            this->p_, reinterpret_cast<const OrtEpDevice**>(input_devices.data()), num_inputs));
    }
...
inline std::vector<ConstEpDevice> ConstSessionImpl<T>::GetEpDeviceForOutputs() const {
    ...
    if (num_outputs > 0) {
        ...
        ThrowOnError(GetApi().SessionGetEpDeviceForOutputs(
            this->p_, reinterpret_cast<const OrtEpDevice**>(output_devices.data()), num_outputs));
    }
...
inline std::vector<ConstEpAssignedSubgraph> ConstSessionImpl<T>::GetEpGraphAssignmentInfo() const {
    ...
    ThrowOnError(
        GetApi().Session_GetEpGraphAssignmentInfo(this->p_, &ep_subgraph_ptrs, &num_ep_subgraphs));
...
inline std::vector<Value> SessionImpl<T>::Run(const RunOptions& run_options,
    ...
    for (size_t i = 0; i < output_count; i++) output_values.emplace_back(nullptr);
...
template <typename T>
inline void SessionImpl<T>::SetEpDynamicOptions(const char* const* keys, const char* const* values,
                                                size_t kv_len) {
...
#if !defined(ORT_MINIMAL_BUILD)
template <typename T>
inline void SessionImpl<T>::FinalizeModelEditorSession(
    const Model& model, const SessionOptions& options,
    OrtPrepackedWeightsContainer* prepacked_weights_container) {
    ThrowOnError(GetModelEditorApi().ApplyModelToModelEditorSession(this->p_, model));
    ThrowOnError(GetModelEditorApi().FinalizeModelEditorSession(this->p_, options,
...
// static
inline Session Session::CreateModelEditorSession(const Env& env, const void* model_data,
                                                 size_t model_data_length,
                                                 const SessionOptions& options) {
    OrtSession* session = nullptr;
    ThrowOnError(GetModelEditorApi().CreateModelEditorSessionFromArray(
        env, model_data, model_data_length, options, &session));
    return Session(session);
}
...
inline TensorTypeAndShapeInfo::TensorTypeAndShapeInfo(
    ONNXTensorElementDataType element_type, const std::vector<int64_t>& dims,
    ...
    if (symbolic_dims) {
        ...
        std::transform(symbolic_dims->begin(), symbolic_dims->end(),
                       std::back_inserter(symbolic_dims_cstr),
                       [](const std::string& s) { return s.c_str(); });
        ThrowOnError(GetApi().SetSymbolicDimensions(p_, symbolic_dims_cstr.data(),
                                                    symbolic_dims_cstr.size()));
    }
}
...
// static
inline TypeInfo TypeInfo::CreateSparseTensorInfo(
    ConstTensorTypeAndShapeInfo sparse_tensor_type_and_shape_info) {
    OrtTypeInfo* output = nullptr;
    ThrowOnError(
        GetModelEditorApi().CreateSparseTensorTypeInfo(sparse_tensor_type_and_shape_info, &output));
    return TypeInfo{output};
}
...
// static
inline TypeInfo TypeInfo::CreateMapTypeInfo(ONNXTensorElementDataType key_type,
...
template <typename T>
void ConstValueImpl<T>::GetTensorElementTypeAndShapeDataReference(
    ONNXTensorElementDataType& elem_type, Shape& shape) const {
    ThrowOnError(GetApi().GetTensorElementTypeAndShapeDataReference(
        this->p_, &elem_type, &shape.shape, &shape.shape_len));
}
...
inline Value Value::CreateTensor(OrtAllocator* deleter, void* p_data, size_t p_data_byte_count,
                                 ...
                                 ONNXTensorElementDataType type) {
    OrtValue* out;
    ThrowOnError(GetApi().CreateTensorWithDataAndDeleterAsOrtValue(
        deleter, p_data, p_data_byte_count, shape, shape_len, type, &out));
    return Value{out};
}
...
struct GetValueImpl {
    ...
    static Status GetValues(const OrtOpAttr* attr, std::vector<T>& out) {
        ...
        if (size > 0) {
            ...
            status = Status{GetApi().ReadOpAttr(attr, deduced_type, result.data(), size, &size)};
            if (!status.IsOK()) return status;
        }
        out.swap(result);
        return status;
...
struct GetValueImpl<std::string> {
    static Status GetValue(const OrtOpAttr* attr, std::string& out) {
        ...
        if (size > 0) {
            ...
            status = Status{
                GetApi().ReadOpAttr(attr, OrtOpAttrType::ORT_OP_ATTR_STRING, buffer, size, &size)};
            if (!status.IsOK()) return status;
        }
        out.swap(result);
        return status;
    ...
    static Status GetValues(const OrtOpAttr* attr, std::vector<std::string>& out) {
        ...
        if (total_buffer_size > 0) {
            ...
            status =
                Status{GetApi().ReadOpAttr(attr, OrtOpAttrType::ORT_OP_ATTR_STRINGS, buffer.data(),
...
inline int64_t ShapeInferContext::GetAttrInt(const char* attr_name) {
    ...
    if (!status.IsOK()) {
        ORT_CXX_API_THROW("Getting int attribute failed: " + status.GetErrorMessage(),
                          status.GetErrorCode());
    }
    return value;
}
...
inline ShapeInferContext::Ints ShapeInferContext::GetAttrInts(const char* attr_name) {
    ...
    if (!status.IsOK()) {
        ORT_CXX_API_THROW("Getting ints attribute failed: " + status.GetErrorMessage(),
                          status.GetErrorCode());
    }
    return result;
}
...
inline float ShapeInferContext::GetAttrFloat(const char* attr_name) {
    ...
    if (!status.IsOK()) {
        ORT_CXX_API_THROW("Getting float attribute failed: " + status.GetErrorMessage(),
                          status.GetErrorCode());
    }
    return value;
}
...
inline ShapeInferContext::Floats ShapeInferContext::GetAttrFloats(const char* attr_name) {
    ...
    if (!status.IsOK()) {
        ORT_CXX_API_THROW("Getting floats attribute failed: " + status.GetErrorMessage(),
                          status.GetErrorCode());
    }
    return result;
}
...
inline std::string ShapeInferContext::GetAttrString(const char* attr_name) {
    ...
    if (!status.IsOK()) {
        ORT_CXX_API_THROW("Getting string attribute failed: " + status.GetErrorMessage(),
                          status.GetErrorCode());
    }
    return value;
}
...
inline ShapeInferContext::Strings ShapeInferContext::GetAttrStrings(const char* attr_name) {
    ...
    if (!status.IsOK()) {
        ORT_CXX_API_THROW("Getting strings attribute failed: " + status.GetErrorMessage(),
                          status.GetErrorCode());
    }
    return result;
}
...
inline std::vector<ConstValueInfo> ConstNodeImpl<T>::GetInputs() const {
    ...
    if (num_vi > 0) {
        ...
        ThrowOnError(GetApi().Node_GetInputs(
            this->p_, reinterpret_cast<const OrtValueInfo**>(result.data()), num_vi));
    }
    return result;
}
...
inline std::vector<ConstValueInfo> ConstNodeImpl<T>::GetOutputs() const {
    ...
    if (num_vi > 0) {
        result.resize(num_vi);
        ThrowOnError(GetApi().Node_GetOutputs(
            this->p_, reinterpret_cast<const OrtValueInfo**>(result.data()), num_vi));
    }
    return result;
}
...
inline std::vector<ConstValueInfo> ConstNodeImpl<T>::GetImplicitInputs() const {
    ...
    if (num_vi > 0) {
        result.resize(num_vi);
        ThrowOnError(GetApi().Node_GetImplicitInputs(
            this->p_, reinterpret_cast<const OrtValueInfo**>(result.data()), num_vi));
    }
    return result;
}
...
inline std::vector<ConstOpAttr> ConstNodeImpl<T>::GetAttributes() const {
    ...
    if (num_attrs > 0) {
        ...
        ThrowOnError(GetApi().Node_GetAttributes(
            this->p_, reinterpret_cast<const OrtOpAttr**>(attrs.data()), num_attrs));
    }
    return attrs;
}

template <typename T>
inline Status ConstNodeImpl<T>::GetAttributeByName(const std::string& name,
                                                   ConstOpAttr& out) const {
...
inline std::vector<AttrNameSubgraph> ConstNodeImpl<T>::GetSubgraphs() const {
    ...
    if (num_graphs > 0) {
        ...
        ThrowOnError(
            GetApi().Node_GetSubgraphs(this->p_, sub_graphs.data(), num_graphs, attr_names.data()));
...
inline void Node::Init(const std::string& operator_name, const std::string& operator_domain,
                       const std::string& node_name, const std::vector<std::string>& input_names,
                       const std::vector<std::string>& output_names,
                       std::vector<OpAttr>& attributes, OrtNode*& node) {
    ...
    ThrowOnError(GetModelEditorApi().CreateNode(
        operator_name.c_str(), operator_domain.c_str(), node_name.c_str(), inputs.data(),
        inputs.size(), outputs.data(), outputs.size(), attributes_ptrs.data(),
        attributes_ptrs.size(), &node));
...
inline Node::Node(const std::string& operator_name, const std::string& operator_domain,
                  const std::string& node_name, const std::vector<std::string>& input_names,
                  const std::vector<std::string>& output_names, std::vector<OpAttr>& attributes) {
...
inline Node::Node(const std::string& operator_name, const std::string& operator_domain,
    ...
    Init(operator_name, operator_domain, node_name, input_names, output_names, empty_attributes,
         p_);
}
...
inline std::vector<ValueInfoConsumerProducerInfo> ConstValueInfoImpl<T>::GetConsumers() const {
    ...
    if (num > 0) {
        ...
        ThrowOnError(
            GetApi().ValueInfo_GetValueConsumers(this->p_, nodes.data(), indices.data(), num));
        out.reserve(num);
...
template <typename T>
inline Status ConstValueInfoImpl<T>::GetExternalInitializerInfo(
    ExternalInitializerInfo& info) const {
...
inline std::vector<OperatorSet> ConstGraphImpl<T>::GetOperatorSets() const {
    ...
    if (num_opsets > 0) {
        ...
        ThrowOnError(
            GetApi().Graph_GetOperatorSets(this->p_, domains.data(), versions.data(), num_opsets));
...
inline std::vector<ConstValueInfo> ConstGraphImpl<T>::GetInputs() const {
    ...
    if (num_vi > 0) {
        result.resize(num_vi);
        ThrowOnError(GetApi().Graph_GetInputs(
            this->p_, reinterpret_cast<const OrtValueInfo**>(result.data()), num_vi));
    }
    return result;
}
...
inline std::vector<ConstValueInfo> ConstGraphImpl<T>::GetOutputs() const {
    ...
    if (num_vi > 0) {
        result.resize(num_vi);
        ThrowOnError(GetApi().Graph_GetOutputs(
            this->p_, reinterpret_cast<const OrtValueInfo**>(result.data()), num_vi));
    }
    return result;
}
...
inline std::vector<ConstValueInfo> ConstGraphImpl<T>::GetInitializers() const {
    ...
    if (num_vi > 0) {
        result.resize(num_vi);
        ThrowOnError(GetApi().Graph_GetInitializers(
            this->p_, reinterpret_cast<const OrtValueInfo**>(result.data()), num_vi));
    }
    return result;
}
...
inline std::vector<ConstNode> ConstGraphImpl<T>::GetNodes() const {
    ...
    if (num_nodes > 0) {
        ...
        ThrowOnError(GetApi().Graph_GetNodes(
            this->p_, reinterpret_cast<const OrtNode**>(result.data()), num_nodes));
    }
    return result;
}
...
inline Graph ConstGraphImpl<T>::GetGraphView(const std::vector<ConstNode>& nodes) const {
    ...
    ThrowOnError(
        GetApi().Graph_GetGraphView(this->p_, inputs_ptrs.data(), nodes.size(), &graph_viewer));
...
inline void GraphImpl<T>::SetInputs(std::vector<ValueInfo>& inputs) {
    ...
    ThrowOnError(
        GetModelEditorApi().SetGraphInputs(this->p_, inputs_ptrs.data(), inputs_ptrs.size()));
...
inline void GraphImpl<T>::SetOutputs(std::vector<ValueInfo>& outputs) {
    ...
    ThrowOnError(
        GetModelEditorApi().SetGraphOutputs(this->p_, outputs_ptrs.data(), outputs_ptrs.size()));
...
template <typename T>
inline void GraphImpl<T>::AddInitializer(const std::string& name, Value& initializer,
                                         bool data_is_external) {
    // Graph takes ownership of `initializer`
    // On error the ownership is not transferred.
    ThrowOnError(GetModelEditorApi().AddInitializerToGraph(this->p_, name.c_str(), initializer,
                                                           data_is_external));
    initializer.release();
}
...
inline Model::Model(const std::vector<DomainOpsetPair>& opsets) {
    ...
    ThrowOnError(
        GetModelEditorApi().CreateModel(domains.data(), versions.data(), opsets.size(), &p_));
}
#endif
...
inline KernelDefBuilder::KernelDefBuilder(OrtKernelDefBuilder* p)
    : detail::Base<OrtKernelDefBuilder>{p} {}
...
inline KernelDefBuilder& KernelDefBuilder::SetSinceVersion(int since_version_start,
                                                           int since_version_end) {
    ThrowOnError(
        GetEpApi().KernelDefBuilder_SetSinceVersion(p_, since_version_start, since_version_end));
    return *this;
}
...
inline KernelDefBuilder& KernelDefBuilder::SetInputMemType(size_t input_index,
                                                           OrtMemType mem_type) {
    ThrowOnError(GetEpApi().KernelDefBuilder_SetInputMemType(p_, input_index, mem_type));
    return *this;
}

inline KernelDefBuilder& KernelDefBuilder::SetOutputMemType(size_t output_index,
                                                            OrtMemType mem_type) {
...
inline KernelDefBuilder& KernelDefBuilder::AddTypeConstraint(
    const char* arg_name, const std::vector<const OrtDataType*>& data_types) {
    ThrowOnError(GetEpApi().KernelDefBuilder_AddTypeConstraint(p_, arg_name, data_types.data(),
                                                               data_types.size()));
    return *this;
}
...
inline KernelDefBuilder& KernelDefBuilder::AddInputOutputAlias(int input_index, int output_index) {
    ThrowOnError(
        GetEpApi().KernelDefBuilder_AddInputOutputAliases(p_, &input_index, &output_index, 1));
    return *this;
}

inline KernelDefBuilder& KernelDefBuilder::AddInputOutputAliases(
    const std::vector<int>& input_indices, const std::vector<int>& output_indices) {
    if (input_indices.size() != output_indices.size()) {
        ORT_CXX_API_THROW("Expecting input and output indices to have the same element count",
                          ORT_INVALID_ARGUMENT);
    }

    ThrowOnError(GetEpApi().KernelDefBuilder_AddInputOutputAliases(
        p_, input_indices.data(), output_indices.data(), input_indices.size()));
    return *this;
}

inline KernelDefBuilder& KernelDefBuilder::AddInputOutputMutableAlias(int input_index,
                                                                      int output_index) {
    ThrowOnError(GetEpApi().KernelDefBuilder_AddInputOutputMutableAliases(p_, &input_index,
                                                                          &output_index, 1));
    return *this;
}

inline KernelDefBuilder& KernelDefBuilder::AddInputOutputMutableAliases(
    const std::vector<int>& input_indices, const std::vector<int>& output_indices) {
    if (input_indices.size() != output_indices.size()) {
        ORT_CXX_API_THROW("Expecting input and output indices to have the same element count",
                          ORT_INVALID_ARGUMENT);
    ...
    ThrowOnError(GetEpApi().KernelDefBuilder_AddInputOutputMutableAliases(
        p_, input_indices.data(), output_indices.data(), input_indices.size()));
    return *this;
}
...
inline Status KernelRegistry::AddKernel(const OrtKernelDef* kernel_def,
                                        OrtKernelCreateFunc kernel_create_func,
                                        void* kernel_create_func_state) {
    return Status{GetEpApi().KernelRegistry_AddKernel(p_, kernel_def, kernel_create_func,
                                                      kernel_create_func_state)};
}

namespace detail {
template <typename T>
inline Status SharedPrePackedWeightCacheImpl<T>::StoreWeightData(void** buffer_data_ptrs,
                                                                 size_t* buffer_sizes,
                                                                 size_t num_buffers) {
    return Status{GetEpApi().SharedPrePackedWeightCache_StoreWeightData(this->p_, buffer_data_ptrs,
