...
inline void InitApi(const OrtApi* api) noexcept {
    detail::Global::Api(api);
}
inline void InitApi() noexcept {
    InitApi(OrtGetApiBase()->GetApi(ORT_API_VERSION));
}
...
/// <summary>
/// This returns a reference to the ORT C Model Editor API. Used if building or augmenting a model
/// at runtime.
/// </summary>
...
struct Float16_t : onnxruntime_float16::Float16Impl<Float16_t> {
   private:
    ...
    constexpr explicit Float16_t(uint16_t v) noexcept {
        val = v;
    }
   ...
   public:
    ...
    constexpr static Float16_t FromBits(uint16_t v) noexcept {
        return Float16_t(v);
    }
    ...
    explicit Float16_t(float v) noexcept {
        val = Base::ToUint16Impl(v);
    }
...
struct BFloat16_t : onnxruntime_float16::BFloat16Impl<BFloat16_t> {
   private:
    ...
    constexpr explicit BFloat16_t(uint16_t v) noexcept {
        val = v;
    }
...
inline void OrtRelease(OrtTensorRTProviderOptionsV2* ptr) {
    GetApi().ReleaseTensorRTProviderOptions(ptr);
}
inline void OrtRelease(OrtCUDAProviderOptionsV2* ptr) {
    GetApi().ReleaseCUDAProviderOptions(ptr);
}
...
struct Status : detail::Base<OrtStatus> {
    Status() = default;  // Same as with std::nullptr_t. But can be used in re-sizable containers
                         // and represent success.
    ...
    Status(const char* message,
           OrtErrorCode code);  ///< Creates status instance out of null-terminated string message.
 ...
 */
struct ExternalInitializerInfo
    : detail::ConstExternalInitializerInfoImpl<OrtExternalInitializerInfo> {
...
struct KeyValuePairsImpl : Ort::detail::Base<T> {
    ...
    // get the pairs in two vectors. entries will be 1:1 between keys and values. avoids copying to
    // std::string
...
struct KeyValuePairs : detail::KeyValuePairsImpl<OrtKeyValuePairs> {
    ...
    ConstKeyValuePairs GetConst() const {
        return ConstKeyValuePairs{this->p_};
    }
};
...
struct MemoryInfo : detail::MemoryInfoImpl<OrtMemoryInfo> {
    ...
    MemoryInfo(const char* name, OrtMemoryInfoDeviceType device_type, uint32_t vendor_id,
               uint32_t device_id, OrtDeviceMemoryType mem_type, size_t alignment,
               OrtAllocatorType allocator_type);  ///< Wrapper around CreateMemoryInfo_V2
    ConstMemoryInfo GetConst() const {
        return ConstMemoryInfo{this->p_};
    }
};
...
struct AllocatorImpl : Base<T> {
     ...
     *
     * \return A pointer to a KeyValuePairs object that will be filled with the allocator
     * statistics.
     */
 ...
 */
using ConstHardwareDevice =
    detail::HardwareDeviceImpl<Ort::detail::Unowned<const OrtHardwareDevice>>;
...
struct EpDeviceImpl : Ort::detail::Base<T> {
    ...
    ConstMemoryInfo GetMemoryInfo(
        OrtDeviceMemoryType memory_type) const;  ///< Wraps EpDevice_MemoryInfo
    SyncStream CreateSyncStream(
        ConstKeyValuePairs stream_options = {}) const;  /// Wraps EpDevice_CreateSyncStream
};
}  // namespace detail
...
struct EpDevice : detail::EpDeviceImpl<OrtEpDevice> {
    ...
    explicit EpDevice(OrtEpDevice* p)
        : EpDeviceImpl<OrtEpDevice>{p} {}  ///< Take ownership of a pointer created by C API
 ...
 * from the model's metadata properties. This is only applicable to models that have been
 * precompiled for an EP. Standard ONNX models do not contain this information.
 *
 ...
 * \return The compatibility info string, or nullptr if not found for this EP. Caller must free via
 * allocator. \throws Ort::Exception on error.
 */
AllocatedStringPtr GetCompatibilityInfoFromModelAllocated(const ORTCHAR_T* model_path,
                                                          const char* ep_type,
 ...
 * allocator. \throws Ort::Exception on error.
 */
AllocatedStringPtr GetCompatibilityInfoFromModelBytesAllocated(const void* model_data,
 ...
 */
using ConstEpAssignedNode =
    detail::EpAssignedNodeImpl<Ort::detail::Unowned<const OrtEpAssignedNode>>;
 ...
 */
using ConstEpAssignedSubgraph =
    detail::EpAssignedSubgraphImpl<Ort::detail::Unowned<const OrtEpAssignedSubgraph>>;
...
struct Env : detail::Base<OrtEnv> {
    ...
    UnownedAllocator CreateSharedAllocator(
        const OrtEpDevice* ep_device, OrtDeviceMemoryType mem_type, OrtAllocatorType allocator_type,
        const OrtKeyValuePairs* allocator_options);  ///< Wraps OrtApi::CreateSharedAllocator

    // Result may be nullptr
    UnownedAllocator GetSharedAllocator(
        const OrtMemoryInfo* mem_info);  ///< Wraps OrtApi::GetSharedAllocator
    ...
    Env& RegisterExecutionProviderLibrary(
        ...
        const std::basic_string<ORTCHAR_T>&
            path);  ///< Wraps OrtApi::RegisterExecutionProviderLibrary
    Env& UnregisterExecutionProviderLibrary(
        const char* registration_name);  ///< Wraps OrtApi::UnregisterExecutionProviderLibrary

    std::vector<ConstEpDevice> GetEpDevices() const;

    Status CopyTensors(const std::vector<Value>& src_tensors, const std::vector<Value>& dst_tensors,
...
struct LoraAdapter : detail::Base<OrtLoraAdapter> {
    ...
    explicit LoraAdapter(std::nullptr_t) {
    ...
    /// \param allocator optional pointer to a device allocator. If nullptr, the data stays on CPU.
    /// It would still
...
struct SessionOptionsImpl : ConstSessionOptionsImpl<T> {
    ...
    SessionOptionsImpl& SetLoadCancellationFlag(
        bool value);  ///< Wraps OrtApi::SessionOptionsSetLoadCancellationFlag
    ...
    SessionOptionsImpl& AppendExecutionProvider_CPU(
        int use_arena);  ///< Wraps OrtApi::SessionOptionsAppendExecutionProvider_CPU
    ...
    SessionOptionsImpl& AppendExecutionProvider_V2(Env& env,
                                                   const std::vector<ConstEpDevice>& ep_devices,
                                                   const KeyValuePairs& ep_options);
    /// Append EPs that have been registered previously with the OrtEnv.
    /// Wraps OrtApi::SessionOptionsAppendExecutionProvider_V2
    SessionOptionsImpl& AppendExecutionProvider_V2(
...
struct SessionOptions : detail::SessionOptionsImpl<OrtSessionOptions> {
    ...
    UnownedSessionOptions GetUnowned() const {
        return UnownedSessionOptions{this->p_};
    }
    ConstSessionOptions GetConst() const {
        return ConstSessionOptions{this->p_};
    }
};
...
struct ModelCompilationOptions : detail::Base<OrtModelCompilationOptions> {
    ...
    explicit ModelCompilationOptions(std::nullptr_t) {
    }  ///< Create an empty ModelCompilationOptions object, must be assigned a valid one to be used.

    ModelCompilationOptions(
        const Env& env,
        const SessionOptions&
            session_options);  ///< Wraps OrtApi::CreateModelCompilationOptionsFromSessionOptions
    ModelCompilationOptions(
        const Env& env,
        ConstSessionOptions
            session_options);  ///< Wraps OrtApi::CreateModelCompilationOptionsFromSessionOptions
    ...
    ModelCompilationOptions& SetInputModelPath(
        const ORTCHAR_T*
            input_model_path);  ///< Wraps OrtApi::ModelCompilationOptions_SetInputModelPath
    ModelCompilationOptions& SetInputModelFromBuffer(
        const void* input_model_data,
        size_t input_model_data_size);  ///< Wraps
                                        ///< OrtApi::ModelCompilationOptions_SetInputModelFromBuffer
    ModelCompilationOptions& SetEpContextEmbedMode(
        bool embed_ep_context_in_model);  ///< Wraps
                                          ///< OrtApi::ModelCompilationOptions_SetEpContextEmbedMode
    ModelCompilationOptions& SetOutputModelPath(
        const ORTCHAR_T*
            output_model_path);  ///< Wraps OrtApi::ModelCompilationOptions_SetOutputModelPath
    ModelCompilationOptions& SetOutputModelExternalInitializersFile(
        const ORTCHAR_T* file_path,
        size_t
            initializer_size_threshold);  ///< Wraps
                                          ///< OrtApi::ModelCompilationOptions_SetOutputModelExternalInitializersFile
    ...
    ModelCompilationOptions& SetOutputModelBuffer(
        OrtAllocator* allocator, void** output_model_buffer_ptr,
        size_t*
            output_model_buffer_size_ptr);  ///< Wraps
                                            ///< OrtApi::ModelCompilationOptions_SetOutputModelBuffer
    ...
    ModelCompilationOptions& SetEpContextBinaryInformation(
        const ORTCHAR_T* output_directory,
        const ORTCHAR_T*
            model_name);  ///< Wraps OrtApi::ModelCompilationOptions_SetEpContextBinaryInformation
    ModelCompilationOptions& SetFlags(
        uint32_t flags);  ///< Wraps OrtApi::ModelCompilationOptions_SetFlags

    ModelCompilationOptions& SetGraphOptimizationLevel(
        GraphOptimizationLevel
            graph_optimization_level);  ///< Wraps
                                        ///< OrtApi::ModelCompilationOptions_SetGraphOptimizationLevel
};

/** \brief Compiles an input model to generate a model with EPContext nodes that execute EP-specific
 * kernels. Wraps OrtApi::CompileModels.
 *
...
struct ConstSessionImpl : Base<T> {
    ...
    std::vector<ConstMemoryInfo> GetMemoryInfoForInputs()
        const;  ///< Wrapper for OrtApi::SessionGetMemoryInfoForInputs
    std::vector<ConstMemoryInfo> GetMemoryInfoForOutputs()
        const;  ///< Wrapper for OrtApi::SessionGetMemoryInfoForOutputs
    std::vector<ConstEpDevice> GetEpDeviceForInputs()
        const;  ///< Wrapper for OrtApi::SessionGetEpDeviceForInputs
    std::vector<ConstEpDevice> GetEpDeviceForOutputs()
        const;  ///< Wrapper for OrtApi::SessionGetEpDeviceForOutputs
    ...
    /** \brief Returns information on the subgraph/nodes assigned to execution providers in the
     * session.
     *
...
struct SessionImpl : ConstSessionImpl<T> {
     ...
     * the output_names array (eg. output_name[0] is the first entry of the returned vector)
     */
     ...
     * member (Ort::Value) is allocated by onnxruntime or preallocated by the customer. \param[in]
     * output_count Number of elements in the output_names and outputs array \param[in] callback
     * Callback function on model run completion \param[in] user_data User data that pass back to
     * the callback
     */
...
struct Session : detail::SessionImpl<OrtSession> {
    /// Create an empty Session object, must be assigned a valid one to be used. Wraps
    ...
    static Session CreateModelEditorSession(const Env& env, const ORTCHAR_T* model_path,
                                            const SessionOptions& options);

    /// Wraps OrtModelEditorApi::CreateModelEditorSession
    static Session CreateModelEditorSession(const Env& env, const void* model_data,
                                            size_t model_data_length,
                                            const SessionOptions& options);
#endif  // !defined(ORT_MINIMAL_BUILD)

    ConstSession GetConst() const {
        return ConstSession{this->p_};
    }
    UnownedSession GetUnowned() const {
        return UnownedSession{this->p_};
    }
};
...
struct SequenceTypeInfo : detail::SequenceTypeInfoImpl<OrtSequenceTypeInfo> {
    ...
    ConstSequenceTypeInfo GetConst() const {
        return ConstSequenceTypeInfo{this->p_};
    }
};
...
struct MapTypeInfo : detail::MapTypeInfoImpl<OrtMapTypeInfo> {
    ...
    ConstMapTypeInfo GetConst() const {
        return ConstMapTypeInfo{this->p_};
    }
};
...
struct ConstValueImpl : Base<T> {
    ...
    /// <summary>
    /// Returns the tensor's element type and a reference to the tensor's internal shape data. The
    /// shape data is owned by the Ort::Value and becomes invalid when the Ort::Value is destroyed
    /// or if the underlying shape data is updated or reallocated.
    ///
    ...
    /// <param name="shape">Output parameter set to the OrtValue instance's shape data and number of
    /// elements.</param>
    void GetTensorElementTypeAndShapeDataReference(ONNXTensorElementDataType& elem_type,
                                                   Shape& shape) const;
};
...
struct Value : detail::ValueImpl<OrtValue> {
    ...
    ConstValue GetConst() const {
        return ConstValue{this->p_};
    }
    UnownedValue GetUnowned() const {
        return UnownedValue{this->p_};
    }
    ...
    /** \brief Creates a tensor with a user supplied buffer. Wraps
     * OrtApi::CreateTensorWithDataAndDeleterAsOrtValue.
     *
...
struct IoBinding : detail::IoBindingImpl<OrtIoBinding> {
    ...
    ConstIoBinding GetConst() const {
        return ConstIoBinding{this->p_};
    }
    UnownedIoBinding GetUnowned() const {
        return UnownedIoBinding{this->p_};
    }
};
 ...
 * formats. \param ... Zero or more variadic arguments referenced by the format string.
 */
...
struct Logger {
     ...
     * of ORT_FILE. \param line_number The file line number in which the message is logged. Usually
     * the value of __LINE__. \param func_name The name of the function in which the message is
     * logged. Usually the value of __FUNCTION__. \param message The message to log. \return A
     * Ort::Status value to indicate error or success.
     */
     ...
     * of ORT_FILE. \param line_number The file line number in which the message is logged. Usually
     * the value of __LINE__. \param func_name The name of the function in which the message is
     * logged. Usually the value of __FUNCTION__. \param format A null-terminated UTF-8 format
     * string forwarded to a printf-like function. Refer to
     * https://en.cppreference.com/w/cpp/io/c/fprintf for information on valid formats. \param args
     * Zero or more variadic arguments referenced by the format string. \return A Ort::Status value
     * to indicate error or success.
     */
...
struct KernelInfo : detail::KernelInfoImpl<OrtKernelInfo> {
    ...
    ConstKernelInfo GetConst() const {
        return ConstKernelInfo{this->p_};
    }
};
...
#pragma warning(pop)
    ...
    // Ort::CustomOpBase derived class should provide the following static method with the
    // type/shape inferencing implementation if needed:
...
struct Node : detail::ConstNodeImpl<OrtNode> {
    ...
    explicit Node(OrtNode* p)
        : ConstNodeImpl<OrtNode>{p} {}  ///< Take ownership of a pointer created by C API
    ...
    /// <summary>
    /// Wraps CreateNode. Node takes ownership of attributes on success and updates the OpAttr in
    /// `attributes` to do so.
    /// </summary>
    Node(const std::string& operator_name, const std::string& operator_domain,
         const std::string& node_name, const std::vector<std::string>& input_names,
...
struct GraphImpl : ConstGraphImpl<T> {
    ...
    void AddInitializer(const std::string& name, Value& initializer,
                        bool data_is_external);  // Graph takes ownership of Value
...
struct Graph : detail::GraphImpl<OrtGraph> {
    ...
    explicit Graph(OrtGraph* p)
        : GraphImpl<OrtGraph>{p} {}  ///< Take ownership of a pointer created by C API
#if !defined(ORT_MINIMAL_BUILD)
...
struct Model : detail::ModelImpl<OrtModel> {
    ...
    explicit Model(OrtModel* p)
        : ModelImpl<OrtModel>{p} {}  ///< Take ownership of a pointer created by C API
...
struct KernelDefBuilder : detail::Base<OrtKernelDefBuilder> {
    ...
    explicit KernelDefBuilder(std::nullptr_t) {
    }  ///< Create an empty object, must be assigned a valid one to be used
    ...
    KernelDefBuilder& AddTypeConstraint(const char* arg_name,
...
/** \brief Convenience C++ wrapper class around a ::OrtSharedPrePackedWeightCache instance owned by
 * ORT.
 *
 * An `OrtSharedPrePackedWeightCache*` instance is passed as an argument to
 * OrtKernelImpl::PrePackWeight. Example use: OrtStatus* MyKernel::PrePackWeightImpl(OrtKernelImpl*,
 * ..., OrtSharedPrePackedWeightCache* c_cache, ...) {
 *     ...
 *     if (c_cache != nullptr) {
