...
class Environment {
    ...
    */
    ...
    /**
     * Set the global threading options for the environment, if no global thread pools have been
     created yet.
     *
     * This function is not safe to call simultaneously from multiple threads, and will return a
     FAIL status on all calls
     * after the first.
     * @param tp_options set of parameters controlling the number of intra and inter op threads for
    ...
    /**
     * Returns an AllocatorPtr for a shared IAllocator based allocator if it matches the memory
     * info. The OrtMemoryInfo name and whether it's an arena or device allocator is ignored in the
     * lookup, as is the alignment. The user calling this function is not expected to know the
     * alignment, and we expect the allocator instance to be created with a valid alignment for the
     * device.
     */
    ...
    /**
     * Create and register an allocator, specified by provider_type, for sharing between multiple
     * sessions. Return an error if an allocator with the same OrtMemoryInfo is already registered.
     * For provider_type please refer core/graph/constants.h
     */
    Status CreateAndRegisterAllocatorV2(const std::string& provider_type,
    ...
    Status GetHardwareDeviceEpIncompatibilityDetails(
        const std::string& ep_name, const OrtHardwareDevice* hw,
    ...
    ///
    /// Primarily used by EP libraries to retrieve environment-level configurations, but could be
    /// used more generally to specify global settings.
    ///
   ...
   private:
    ...
    Status CreateSharedAllocatorImpl(const OrtEpDevice& ep_device, const OrtMemoryInfo& memory_info,
                                     OrtAllocatorType allocator_type,
                                     const OrtKeyValuePairs* allocator_options,
                                     OrtAllocator** allocator, bool replace_existing);
    ...
    // RegisterAllocator: IAllocatorImplWrappingOrtAllocator custom allocators registered by the
    // user.
    //                    TODO: How can we detect registration of an allocator from an
    //                    InferenceSession?
    // OrtEpDevice: We create a default shared IAllocatorImplWrappingOrtAllocator for each
    // OrtEpDevice memory info.
    ...
    // OrtAllocator wrapped CPUAllocator::DefaultInstance that is returned by GetSharedAllocator
    // when no plugin EP is providing a CPU allocator.
    ...
    Status RegisterExecutionProviderLibrary(
        const std::string& registration_name, std::unique_ptr<EpLibrary> ep_library,
    ...
    struct EpInfo {
        ...
        std::vector<EpFactoryInternal*>
            internal_factories;  // factories that can create IExecutionProvider instances
        std::vector<plugin_ep::DataTransfer*>
            data_transfers;  // data transfer instances for this EP.
    ...
    // The environment may add or remove an entry on EP library registration and unregistration,
    // respectively.
    OrtKeyValuePairs config_entries_;
    mutable std::shared_mutex
        config_entries_mutex_;  // Should be locked when accessing config_entries_
