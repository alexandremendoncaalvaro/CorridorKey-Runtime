...
/**
@class NodeArg
Class representing a data type that is input or output for a Node, including the shape if it is a
Tensor.
*/
class NodeArg {
   public:
    ...
    */
    ...
    /** Return an indicator.
    @returns true if NodeArg is a normal tensor with a non-empty shape or a scalar with an empty
    shape. Otherwise, returns false. */
    bool HasTensorOrScalarShape() const;

#if !defined(ORT_MINIMAL_BUILD) || defined(ORT_EXTENDED_MINIMAL_BUILD)

    /** Sets the shape.
    @remarks Shape can only be set if the TypeProto was provided to the ctor, or #SetType has been
    called, as the shape information is stored as part of TypeProto. */
    void SetShape(const ONNX_NAMESPACE::TensorShapeProto& shape);

    /** Clears shape info.
    @remarks If there is a mismatch during shape inferencing that can't be resolved the shape info
    may be removed. */
    void ClearShape();
    ...
    /** Override current type from input_type if override_types is set to true, return failure
    status otherwise.
  ...
  */
    common::Status OverrideTypesHelper(const ONNX_NAMESPACE::TypeProto& input_type,
                                       ...
                                       int32_t current_tensor_elem_type, bool override_types);
    ...
    @returns Success unless there is existing type or shape info that can't be successfully updated.
  */
    common::Status UpdateTypeAndShape(const ONNX_NAMESPACE::TypeProto& input_type, bool strict,
  ...
  */
    ...
    const std::optional<ONNX_NAMESPACE::TensorShapeProto>& GetInferredShapeValues() const noexcept {
        return inferred_shape_values_;
    }

    /** Gets mutable inferred shape values as a TensorShapeProto. */
    std::optional<ONNX_NAMESPACE::TensorShapeProto>& GetMutableInferredShapeValues() noexcept {
        return inferred_shape_values_;
    }

    /** Gets the inferred shape scalar value */
    const std::optional<int64_t> GetInferredShapeScalarValue() const noexcept {
        return inferred_scalar_value_;
    }

    /** Sets the inferred shape scalar value */
    void SetInferredShapeScalarValue(int64_t value) noexcept {
        inferred_scalar_value_ = value;
    }
  ...
  */
   ...
   private:
    ...
    // This variable stores the actual tensor data of the shape as a TensorShapeProto after
    // executing the ONNX operator's PartialDataPropagationFunction(). It's used for shape inference
    // purpose.
    //
    // Calling an operator's TypeAndShapeInferenceFunction() alone is sometimes insufficient
    // for complete shape inference. For example, the Shape operator's
    // TypeAndShapeInferenceFunction() only provides the output's rank which is 1 but not its actual
    // shape values.
    //
