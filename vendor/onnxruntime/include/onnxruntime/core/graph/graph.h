...
class Node {
...
#if !defined(ORT_MINIMAL_BUILD) || defined(ORT_EXTENDED_MINIMAL_BUILD) || \
    defined(ORT_MINIMAL_BUILD_CUSTOM_OPS)
    Node(std::string_view name, std::string_view op_type, std::string_view description,
         gsl::span<NodeArg* const> input_args, gsl::span<NodeArg* const> output_args,
         const NodeAttributes* attributes, std::string_view domain) {
    ...
    */
    ...
    /** Sets the since version (opset version that the Node's operator was first defined in.) for
    this node.
    @remarks Used during layout transformation for setting since version for layout transformed
    nodes with domain kMSNHWC.
    */
    ...
    /**
    Helper to iterate through the container returned by #InputDefs() or #OutputDefs() and call the
    provided function.
    @param node_args Collection of NodeArgs returned by #InputDefs() or #OutputDefs()
    @param func Function to call for each valid NodeArg in the node_args. The function is called
    with the NodeArg and the index number in the container.
    ...
    */
    static common::Status ForEachWithIndex(
        const ConstPointerContainer<std::vector<NodeArg*>>& node_args,
    ...
    @remarks requires ConstPointerContainer wrapper to apply const to the NodeArg pointers so access
    is read-only. */
    ...
    If this Node contains a subgraph, these are the NodeArg's that are implicitly consumed by Nodes
    within that subgraph. e.g. If and Loop operators.*/
...
#if !defined(ORT_MINIMAL_BUILD)
    /**
    Helper to iterate through the container returned by #MutableInputDefs() or #MutableOutputDefs()
    ...
    @remarks Returns immediately on error.
    */
    static common::Status ForEachMutableWithIndex(
    ...
    /** Struct to provide sorting between EdgeEnd instances based on NodeIndex first, and
     * NodeArg::Name second. */
...
#undef ADD_ATTR_INTERFACES
    ...
    /** Gets the Graph instance that is instantiated from a GraphProto attribute during
    Graph::Resolve.
    @param attr_name Attribute name for the GraphProto attribute.
    @returns nullptr if the Graph instance has not been instantiated or attribute does not contain a
    GraphProto.
    */
    const Graph* GetGraphAttribute(const std::string& attr_name) const;

    /** Gets the mutable Graph instance that is instantiated from a GraphProto attribute during
    Graph::Resolve.
    @param attr_name Attribute name for the GraphProto attribute.
    @returns nullptr if the Graph instance has not been instantiated or attribute does not contain a
    GraphProto.
    */
    ...
    /** Checks if the Node contains at least one subgraph (this is the case for control flow
    operators, such as If, Scan, Loop).
    ...
    */
    const std::unordered_map<std::string, gsl::not_null<Graph*>>&
     ...
     */
    std::unordered_map<std::string, gsl::not_null<Graph*>>&
    ...
    @returns Map of the attribute name that defines the subgraph to the subgraph's Graph instance.
             nullptr if the Node has no subgraphs.
    */
    std::unordered_map<std::string, gsl::not_null<const Graph*>> GetAttributeNameToSubgraphMap()
        const;
    ...
    /** Call the provided function for all explicit inputs, implicit inputs, and outputs of this
       Node. If the NodeArg is an explicit or implicit input, is_input will be true when func is
       called.
        @param include_missing_optional_defs Include NodeArgs that are optional and were not
       provided i.e. NodeArg::Exists() == false.
        */
    ...
    */
    ...
    @param update_subgraphs Update the GraphProto values for any subgraphs in the returned
    NodeProto. If graph optimization has been run this is most likely required to ensure the
    complete Graph is valid.
    */
    ...
    flatbuffers::Offset<onnxruntime::fbs::NodeEdge> SaveEdgesToOrtFormat(
    ...
    Status LoadEdgesFromOrtFormat(const onnxruntime::fbs::NodeEdge& fbs_node_edgs,
                                  const Graph& graph);
    ...
    */
    class Definitions {
       public:
        ...
        This means that 4 elements (inputs) of input_defs map to the first argument of the operator
        or function, and the other 6 map to the second argument.
        */
        ...
        @remarks For example, a subgraph in an 'If' node gets all its input values via this
        mechanism rather than there being explicit inputs to the 'If' node that are passed to the
        subgraph. They are pseudo-inputs to this Node as it has an implicit dependency on them. */
    ...
    */
    ...
    // NOTE: This friendship relationship should ONLY be used for calling methods of the Node class
    // and not accessing the data members directly, so that the Node can maintain its internal
    // invariants.
    friend class Graph;
   ...
   protected:
#if !defined(ORT_MINIMAL_BUILD) || defined(ORT_EXTENDED_MINIMAL_BUILD)
    // internal only method to allow selected classes to directly alter the input/output definitions
    // and arg counts made protected to facilitate testing
   ...
   private:
...
#if !defined(ORT_MINIMAL_BUILD) || defined(ORT_EXTENDED_MINIMAL_BUILD) || \
    defined(ORT_MINIMAL_BUILD_CUSTOM_OPS)
    void Init(std::string_view name, std::string_view op_type, std::string_view description,
              gsl::span<NodeArg* const> input_args, gsl::span<NodeArg* const> output_args,
              const NodeAttributes* attributes, std::string_view domain);
    void Init(std::string_view name, std::string_view op_type, std::string_view description,
              gsl::span<NodeArg* const> input_args, gsl::span<NodeArg* const> output_args,
              NodeAttributes&& attributes, std::string_view domain);
#endif
    ...
    std::vector<std::unique_ptr<Graph>>& MutableSubgraphs() noexcept {
        return subgraphs_;
    }
    ...
    // Cleared after first call to onnx::check_node in VerifyNodeAndOpMatch when the first
    // Graph::Resolve runs.
    ...
    // NodeProto that the Node was created from. We temporarily set this as a performance
    // optimization to avoid calling Node::ToProto when running onnx::check_node in the first
    // Graph::Resolve. At that point we know all the nodes are unchanged from the original model.
...
*/
class Graph {  // NOLINT(clang-analyzer-optin.performance.Padding): preserve existing data member
   ...
   public:
    ...
    bool StrictShapeTypeInference() const {
        return strict_shape_type_inference_;
    }
    ...
    The replacement initializer tensor must have the same type and shape as the existing initializer
    tensor. The new_initializer is expected to be either small or have external data reference
    stored in OrtValue.

    Note: This currently has linear time complexity. There is room for improvement but it would
    likely require changes to how initializer tensors are stored and tracked.
    */
     ...
     */
    common::Status InjectExternalInitializedTensors(
        const InlinedHashMap<std::string, OrtValue>& external_initializers);
    ...
    /// a data pointer to ort_value. For small tensors (LT
    /// utils::kSmallTensorExternalDataThreshold), the data would still be contained within
    /// tensor_proto, and OrtValue would be unallocated in this case, and not added to
    ...
    */
    ...
    /** Populate `value` if an externally allocated OrtValue exists for an initializer with the
     * given name.
     */
    bool GetOrtValueInitializer(const std::string& name, OrtValue& value,
                                bool check_outer_scope = false) const;

    /// <summary>
    /// Loads an initializer with data in an external file into an OrtValue. Does NOT cache the
    /// OrtValue in this Graph.
    /// </summary>
    /// <param name="name">The name of the initializer.</param>
    /// <param name="value">Output parameter set to the loaded OrtValue. Set to an existing OrtValue
    /// if it is already loaded.</param> <returns>A status indicating an error or success. An error
    /// occurs if `name` is not an initializer with external data.</returns>
    Status LoadExternalInitializerAsOrtValue(const std::string& name, OrtValue& value) const;

    /// <summary>
    /// Gets information (external filepath, file offset, num bytes) for an initializer with data in
    /// an external file.
    /// </summary>
    /// <param name="name">The initializer's name.</param>
    /// <param name="ext_info">Output parameter set to the location information of the external
    /// data.</param> <param name="check_outer_scope">Set to true if parent graphs should be
    /// checked.</param> <returns>True if `name` refers to an initializer with data in an external
    /// file. Otherwise, returns false</returns>
    bool GetExternalInitializerInfo(const std::string& name,
                                    std::unique_ptr<ExternalDataInfo>& ext_info,
                                    bool check_outer_scope = false) const;
    ...
    cannot be overridden at runtime. If the initializer is not found or is not constant, a nullptr
    is returned.
    ...
    */
    ...
    /** returns the initializer's TensorProto if 'name' is an initializer (both constant and
    overridable). If the initializer is not found, a nullptr is returned.
    @param check_outer_scope If true and the graph is a subgraph,
           check ancestor graph/s for 'name' if not found in 'graph'.
    @remarks check_outer_scope of true is not supported in a minimal build
    */
    const ONNX_NAMESPACE::TensorProto* GetInitializer(const std::string& name,
                                                      bool check_outer_scope) const;

    /// <summary>
    /// Returns the initializer's TensorProto if 'name' is an initializer (either constant or
    /// overridable). If the initializer is not found, a nullptr is returned. Also returns, via
    /// output parameters, the OrtValue that holds the actual data (if any) and a boolean that
    /// indicates if the initializer is constant.
    /// </summary>
    /// <param name="name">The initializer's name.</param>
    /// <param name="value">Output OrtValue that is populated if the initializer tensor proto has
    /// been configured
    ///                     to use external data that points to an OrtValue. Is set to an
    ///                     unallocated OrtValue for a tensor proto that holds the weight data
    ///                     (e.g., small initializers).</param>
    /// <param name="is_constant">Output parameter set to true if the initializer is a
    /// constant.</param> <param name="check_outer_scope">Checks outer scope if set to true and the
    /// graph is a subgraph.</param> <returns>The initializer's TensorProto or nullptr.</returns>
    ...
    These are the required inputs to the Graph as the initializers can be optionally overridden via
    graph inputs.
    ...
    bool IsOutput(const NodeArg* node_arg) const noexcept {
        return std::find(graph_outputs_.begin(), graph_outputs_.end(), node_arg) !=
               graph_outputs_.end();
    }
    ...
    */
    ...
    /** Returns a vector with the indexes of the outputs of the given Node that are also Graph
     * outputs. */
    std::vector<int> GetNodeOutputsInGraphOutputs(const Node& node) const {
        int output_idx = 0;
        std::vector<int> indexes;
        for (auto output_def : node.OutputDefs()) {
            if (std::find(GetOutputs().cbegin(), GetOutputs().cend(), output_def) !=
                GetOutputs().cend()) {
    ...
    */
    ...
    @returns Mutable Node instance if found. nullptr if node_index is invalid or node has been
    freed.
    */
    ...
    /** Get a ConstGraphNodes instance that provides access to a filtered set of valid Nodes in the
    Graph.
    @remarks We can't use GraphNodes as that would provide mutable access to the nodes by default,
    and we can't prevent that by returning a const instance of GraphNodes as we're creating a new
    instance here due to the filter being something we don't control (i.e. we have to return a new
    instance so it can't be const).
    */
    ...
    */
    ...
    }  // assume the casting won't overflow
    ...
    */
    ...
    // Searches for a NodeArg in the current graph and its parent graphs, and returns the
    // corresponding mutable NodeArg
    NodeArg* GetNodeArgIncludingParentGraphs(const std::string& node_arg_name);

    // Searches for a NodeArg in the current graph and its parent graphs, and returns the
    // corresponding const NodeArg
    const NodeArg* GetNodeArgIncludingParentGraphs(const std::string& node_arg_name) const;

    /** Gets a mutable NodeArg by name. Creates a new NodeArg that is owned by this Graph if not
    found.
    ...
    */
    NodeArg& GetOrCreateNodeArg(const std::string& name,
    ...
    */
    Node& AddNode(const std::string& name, const std::string& op_type,
                  const std::string& description, gsl::span<NodeArg* const> input_args,
                  gsl::span<NodeArg* const> output_args, const NodeAttributes* attributes = nullptr,
                  const std::string& domain = kOnnxDomain);

    Node& AddNode(const std::string& name, const std::string& op_type,
                  const std::string& description, gsl::span<NodeArg* const> input_args,
                  gsl::span<NodeArg* const> output_args, NodeAttributes&& attributes,
                  const std::string& domain = kOnnxDomain);
    Node& AddNode(const std::string& name, const std::string& op_type,
                  const std::string& description, std::initializer_list<NodeArg*> input_args,
                  std::initializer_list<NodeArg*> output_args,
                  const NodeAttributes* attributes = nullptr,
                  const std::string& domain = kOnnxDomain) {
        return AddNode(name, op_type, description, AsSpan(input_args), AsSpan(output_args),
                       attributes, domain);
    }
    ...
    Node& AddNode(const std::string& name, const std::string& op_type,
                  ...
                  const NodeAttributes* attributes = nullptr,
                  const std::string& domain = kOnnxDomain) {
        return AddNode(name, op_type, description, input_args, AsSpan(output_args), attributes,
                       domain);
    }
    ...
    Node& AddNode(const std::string& name, const std::string& op_type,
                  ...
                  gsl::span<NodeArg* const> output_args, const NodeAttributes* attributes = nullptr,
                  const std::string& domain = kOnnxDomain) {
        return AddNode(name, op_type, description, AsSpan(input_args), output_args, attributes,
                       domain);
    }
    ...
    */
    ...
    @param dst_node_index NodeIndex of destination Node that is receiving input from the source
    Node.
    @param src_arg_index node arg index of source node.
    @param dst_arg_index node arg index of destination node.
    */
    void AddEdge(NodeIndex src_node_index, NodeIndex dst_node_index, int src_arg_index,
                 int dst_arg_index);
    ...
    @param dst_arg_index node arg index of destination node.
    */
    void RemoveEdge(NodeIndex src_node_index, NodeIndex dst_node_index, int src_arg_index,
                    int dst_arg_index);
#endif
    ...
    The source Node does not produce output that is directly consumed by the destination Node,
    however the destination Node must execute after the source node. The control edge allows this
    ordering to occur.
    */
    ...
    This should be done after modifying any aspect of the Graph that changes the Nodes or
    ...
    /** Gets flag indicating whether Graph::graph_proto_ needs to be synchronized with this Graph
     * instance. */
    ...
    @param enter Visit function that will be invoked on a node when it is visited but its parents
    haven't been.
    @param leave Visit function invoked on the node after its parents have all been visited.
    @param comp Comparison function to stabilize the traversal order by making Node ordering
    deterministic.
    */
    ...
    */
     ...
     */
    void MemoryEfficientTopologicalSort(
        const Node* yield_op,
    ...
    @remarks As a new Graph instance for the fused nodes is not created, a GraphViewer can be
    constructed with the IndexedSubGraph information to provide a view of the subgraph. The original
    nodes are left in place while this is in use. Call FinalizeFuseSubGraph to remove them once the
    fused replacement node is fully created.
    */
    ...
    @param initializer_size_threshold initializers larger or equal to this threshold (in bytes) are
    saved in the external file. Initializer smaller than this threshold are included in the onnx
    file.
    @param align_info offset alignment info.
    @returns GraphProto serialization of the graph.
    */
    ONNX_NAMESPACE::GraphProto ToGraphProtoWithExternalInitializers(
        const std::filesystem::path& external_file_path,
        const std::filesystem::path& model_file_path,
        const ModelSavingOptions& model_saving_options) const;

    /// <summary>
    /// Serialize the Graph to a onnx::GraphProto. Caller provides a function that determines where
    /// each initializer is stored (i.e., either in an external file or within the model).
    /// </summary>
    ...
    common::Status ToGraphProtoWithCustomInitializerHandling(
        OrtGetInitializerLocationFunc handle_initializer_func, void* state,
    ...
    */
    ...
    /**
    Create a single Function based Node that is the result of the a fusion of multiple nodes in this
    Graph. A new Graph instance will be created for the fused nodes.
    @param sub_graph A IndexSubGraph instance with details of the nodes to fuse. Ownership is
    transferred to the new Node
    @param fused_node_name The name for the new Node.
    @returns Function based Node with fused subgraph. The Node body will contain a Function
    instance.
    */
    ...
    */
    ...
    /** Mark a NodeArg name as coming from the outer scope when programmatically constructing a
    Graph that will be used as a GraphProto attribute in another Node. e.g. when creating a Graph
    instance that will be used as a subgraph in a control flow operator, it is necessary to define
    placeholder NodeArgs for outer scope values. This prevents these values from becoming explicit
    graph inputs when the Graph is resolved.
    */
    ...
    */
    ...
    @param outputs NodeArgs that represent complete graph outputs which need to be explicitly
    ordered.
    ...
    */
    ...
    /** During constant folding it may become possible to infer the shape for a node.
        To avoid running a full Resolve allow an individual node to have the shape inferencing
       re-run.
    */
    ...
    */
    ...
    /**
     * @brief This function examines the specified initializers in the graph and converts them
     * inline if any has external data in memory.
     *
     * @param iterators Span of iterators pointing to the initializers and the order that should be
     * processed
     * @param output_graph_proto The GraphProto to be updated with the modified initializers
     * @return Status Returns a Status object indicating success or any errors that occurred during
     * conversion
     */
    Status RegenerateInitializersAndReplaceInMemory(
        gsl::span<const InitializedTensorSet::const_iterator> iterators,
    ...
    bool IsOuterScopeValue(const std::string& name) const {
        ...
        return std::any_of(
            implicit_input_defs.cbegin(), implicit_input_defs.cend(),
            [&name](const NodeArg* implicit_input) { return implicit_input->Name() == name; });
    }

#if !defined(ORT_MINIMAL_BUILD)
    /** Construct a Graph instance for a subgraph that is created from a GraphProto attribute in a
    Node. Inherits some properties from the parent graph.
    ...
    */
    ...
    Graph(const Model& owning_model, IOnnxRuntimeOpSchemaCollectionPtr schema_registry,
          ONNX_NAMESPACE::GraphProto& subgraph_proto,
          const std::unordered_map<std::string, int>& domain_version_map,
          const logging::Logger& logger, bool strict_shape_type_inference);
#endif
    ...
    static Status LoadFromOrtFormat(const onnxruntime::fbs::Graph& fbs_graph, Graph& parent_graph,
                                    const Node& parent_node,
                                    const OrtFormatLoadOptions& load_options,
                                    const logging::Logger& logger, std::unique_ptr<Graph>& graph);

    static Status LoadFromModelEditorApiModel(
        const OrtGraph& api_graph, const Model& owning_model,
        const std::unordered_map<std::string, int>& domain_to_version,
        IOnnxRuntimeOpSchemaCollectionPtr schema_registry, bool strict_shape_type_inference,
        const logging::Logger& logger, std::unique_ptr<Graph>& graph);
    ...
    // We don't run Graph::Resolve() on an ORT format model, but a compiling EP may copy
    // initializers to its compiled model during partitioning, leaving them unused in the ORT Graph.
    // To allow the memory to be freed we need to manually run the cleanup that would usually happen
    // as part of Graph::Resolve.
    ...
    Graph(const Model& owning_model, const std::unordered_map<std::string, int>& domain_to_version,
#if !defined(ORT_MINIMAL_BUILD)
          IOnnxRuntimeOpSchemaCollectionPtr schema_registry,
#endif
    ...
    Graph(const Model& owning_model, ONNX_NAMESPACE::GraphProto* graph_proto,
          const std::unordered_map<std::string, int>& domain_to_version, Version ir_version,
          IOnnxRuntimeOpSchemaCollectionPtr schema_registry, const logging::Logger& logger,
          bool strict_shape_type_inference);
    ...
    Graph(const Model& owning_model, ONNX_NAMESPACE::GraphProto* graph_proto,
          ...
          const Node* parent_node, const logging::Logger& logger, bool strict_shape_type_inference);
    ...
    */
    ...
    /// <param name="model_external_file_path">model file folder path with external file path
    /// appended</param> <param name="model_saving_options">model saving options including alignment
    /// and pre-packs</param> <param name="output_graph_proto">The graph proto to be
    /// modified</param> <param name="external_stream">external file stream</param> <param
    /// name="external_offset">current external file offset updated with each write</param>
    /// <returns>Status instance</returns>
    Status AddExternalInitializersToGraphProtoImpl(
        const std::filesystem::path& model_path, const std::filesystem::path& external_file_path,
        const std::filesystem::path& model_external_file_path,
        const ModelSavingOptions& model_saving_options,
        ONNX_NAMESPACE::GraphProto& output_graph_proto, std::ostream& external_stream,
        int64_t& external_offset) const;

    Status ToGraphProtoWithCustomInitializerHandlingImpl(
        OrtGetInitializerLocationFunc handle_initializer_func, void* state,
    ...
    // During the Resolve of a Graph it is necessary to recursively descend into subgraphs (created
    // from GraphProto Node attributes in the Graph) if present. The ResolveContext holds the
    // collection of values for the current Graph instance, be it the main graph or a subgraph, so
    // that the various operations that are part of the Resolve can work iteratively or recursively
    // as needed.
    struct ResolveContext {
        ...
        // check if the provided name is an input/initialize/node output of this Graph instance
        // during Graph::Resolve. Graph::node_args_ can have stale entries so we can't rely on that.
        bool IsLocalValue(const std::string& name) const;

        // check if an ancestor graph has a valid value with the provided name during
        // Graph::Resolve. Once Graph::Resolve completes Graph::IsOuterScopeValue can be used and is
        // more efficient.
        bool IsOuterScopeValue(const std::string& name) const;
    ...
    // Depth-first going from bottom up through the graph and checking whether there are any back
    // edges. NodesInTopologicalOrder is updated with the nodes' indexes in topological order if
    // <Status> returned is "OK", otherwise it's undefined.
    ...
    static common::Status InferAndVerifySubgraphTypes(
        const Node& node, Graph& subgraph,
        const std::vector<const ONNX_NAMESPACE::TypeProto*>& input_types,
        std::vector<const ONNX_NAMESPACE::TypeProto*>& output_types,
        const Graph::ResolveOptions& options);

    // If ONNX operator's PartialDataPropagationFunction() infers concrete shape values in the
    // output save them to the output NodeArg as a TensorShapeProto or a scalar value so that
    // downstream (consumer) nodes can use them later for their TypeAndShapeInferenceFunction() and
    // PartialDataPropagationFunction().
    common::Status SaveShapeValuesFromDataPropagation(
        const Node& node, NodeArg& output_def,
    ...
    // recursively accumulate and set the outer scope node args in the resolve context for all
    // subgraphs so they can be used to resolve outer scope dependencies when running
    // BuildConnections for the subgraphs.
    common::Status SetOuterScopeNodeArgs(
    ...
    /// <param name="must_replace_external">This is true when we replace the initializer with
    /// external data with OrtValue from the customer, in which case we enforce that the original
    /// initializer must have external data</param> <returns></returns>
    ...
    common::Status ForThisAndAllSubgraphs(const std::vector<Graph*>& subgraphs,
                                          std::function<Status(Graph&)> func);

    // Clear all unused initializers and NodeArgs
    void CleanUnusedInitializersAndNodeArgs(
        const std::unordered_set<std::string>* initializer_names_to_preserve = nullptr);

    Status PopulateNodeArgToProducerConsumerLookupsFromNodes();

    template <typename TInstance>
    static auto GetConsumerNodesImpl(TInstance& instance, const std::string& node_arg_name)
    ...
    template <typename TInstance>
    static auto GetProducerNodeImpl(TInstance& instance, const std::string& node_arg_name)
    ...
    Node& CreateFusedSubGraphNode(const IndexedSubGraph& sub_graph,
    ...
    Node* NodeAtIndexImpl(NodeIndex node_index) const {
        ...
        ORT_ENFORCE(node_index < nodes_.size(),
                    "Validating no unexpected access using an invalid node_index. Got:", node_index,
                    " Max:", nodes_.size());
        return nodes_[node_index].get();
    }

    Status LoadFromModelEditorApiModel(const OrtGraph& api_graph,
                                       bool updating_existing_graph = false);
    ...
    // GraphProto that provides storage for the ONNX proto types deserialized from a
    // flexbuffer/flatbuffer
    ONNX_NAMESPACE::GraphProto deserialized_proto_data_;

    InitializedTensorSet name_to_initial_tensor_;

    // Initializers that are external to the Graph.
    // e.g. created from existing memory using CreateTensorWithDataAndDeleterAsOrtValue in the ORT
    // API. As we need to convert to TensorProto for the optimizers to work and keep the deleter
    // information we store them in the Graph instance and retrieve during session state
    // finalization.
    ...
    InlinedHashMap<std::string, std::reference_wrapper<ONNX_NAMESPACE::OpSchema>>
...
// Print Node as,
//  (operator's name, operator's type, domain, version) : (input0, input1, ...) -> (output0,
//  output1, ...)
// For example,
...
//    "Input": tensor(float)
// Nodes:
//    ("add0", Add, "", 7) : ("Input": tensor(float),"Bias": tensor(float),) -> ("add0_out":
//    tensor(float),)
//    ("matmul", MatMul, "", 9) : ("add0_out": tensor(float),"matmul_weight": tensor(float),) ->
//    ("matmul_out": tensor(float),)
//    ("add1", Add, "", 7) : ("matmul_out": tensor(float),"add_weight": tensor(float),) ->
//    ("add1_out": tensor(float),)
//    ("reshape", Reshape, "", 5) : ("add1_out": tensor(float),"concat_out": tensor(int64),) ->
//    ("Result": tensor(float),)
// Outputs:
//    "Result": tensor(float)
