...
@remarks If the underlying Graph is changed, GetNodesInTopologicalOrder and GetRootNodes may become
invalid.
*/
class GraphViewer {
   public:
    ...
    */
    ...
    /**
    Construct a GraphViewer from the provided Graph instance, filtering to the nodes specified in
    the IndexedSubGraph
    */
    ...
    */
    ...
    @returns Collection of NodeArg pointers for the graph inputs, excluding inputs that have
    matching initializers.
    @remarks No nullptr values in the returned collection. The order will be the same as in the
    GraphProto. Inputs are for filter_info_ if set.
    */
    ...
    */
    ...
    @remarks No nullptr values in the returned collection. The order will be the same as in the
    GraphProto. Outputs are for filter_info_ if set.
    */
    ...
    */
    ...
    @remarks May return nullptr if index no longer points to a valid node due to the node being
    freed, or if node is excluded by filter_info_.
    */
    ...
    */
    const std::vector<NodeIndex>& GetNodesInTopologicalOrder(
        ExecutionOrder order = ExecutionOrder::DEFAULT) const;
    ...
    */
    ...
    @remarks This function will return the result from GetConstantInitializer of the underlying
    Graph, if a const initializer is part of the underlying Graph but not part of this GraphViewer,
    ...
    */
    ...
    IOnnxRuntimeOpSchemaCollectionPtr GetSchemaRegistry() const {
        return graph_->GetSchemaRegistry();
    }
#endif
   ...
   private:
    ...
    // if we're limiting the view to an IndexedSubGraph we need to create a few pieces of
    // infrastructure that would usually come from the full graph
