...
/**
Class to filter out null entries from either a vector of unique_ptr<Node> or a vector of [const]
Node* and provide an iterator interface that returns [const] Node& for the valid entries.
*/
...
class ValidNodes {
   public:
    ...
    /**
    Construct a ValidNodes instance to provide iteration over all valid nodes in the
    TNodesCollection
    ...
    */
    ...
    // we need to templatize the functions for enable_if to work at this level, but mandate T2 being
    // TNodesContainer
    template <typename T2 = TNodesContainer>
    typename std::enable_if<!std::is_const<T2>::value, MutableNodeIterator>::type begin() noexcept {
        static_assert(std::is_same<T2, TNodesContainer>::value,
                      "Explicit specialization is not allowed");
    ...
    */
    template <typename TIterator>
    class NodeIterator {
        // get the type being returned by the iterator. can't use TIterator::value_type as that is
        // always non-const
        using IterType = typename std::remove_reference<
            typename std::iterator_traits<TIterator>::reference>::type;
        // and determine what we will return based on its constness
        using T =
            typename std::conditional<std::is_const<IterType>::value,
       ...
       public:
        ...
        NodeIterator(const TIterator current, const TIterator end,
                     const NodeFilterFunc& filter_fn) noexcept
            : current_{current},
              end_{end},
              apply_filter_{filter_fn != nullptr},
              filter_func_{&filter_fn} {
        ...
        NodeIterator<TIterator>& operator++() {
            if (current_ < end_) {
                while (++current_ != end_) {
                    if (*current_ != nullptr &&
                        (!apply_filter_ || (*filter_func_)((*current_)->Index()) == false))
                        break;
                }
            }
            return *this;
        ...
        /** Return the current Node&. This will be const if the iterator was returned from a const
         * GraphNodes instance. */
