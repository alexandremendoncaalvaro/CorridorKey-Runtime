...
struct IndexedSubGraph {
    struct MetaDef {
        ...
        ONNX_NAMESPACE::OperatorStatus status{
            ONNX_NAMESPACE::OperatorStatus::STABLE};  ///< Status of customized
                                                      ///< SubGraph/FunctionProto.
        ...
        std::vector<std::string>
            constant_initializers;  ///< Constant initializers of customized SubGraph/FunctionProto.
    ...
    // Either using an existing schema or generating reusable one when fusing nodes using the
    // MetaDef. MetaDef.domain + MetaDef.name => the domain.op_type that a schema must exist for
    // with a valid since_version.
    ...
    bool IsAccountingEnabled() const {
        return resource_accountant != nullptr && nodes_costs.size() == nodes.size();
    }
