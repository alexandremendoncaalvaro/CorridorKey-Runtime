...
struct Int2x4Base {
    ...
    Int2x4Base(UnpackedType val0, UnpackedType val1, UnpackedType val2, UnpackedType val3) {
        bits_ = static_cast<std::byte>((val0 & 0x3) | ((val1 & 0x3) << 2) | ((val2 & 0x3) << 4) |
                                       ((val3 & 0x3) << 6));
    }
    ...
    /// <summary>
    /// Calculates the number of packed byte units needed to store the given number of 2-bit
    /// elements. Each byte stores 4 x 2-bit elements.
    /// </summary>
    ...
    /// <summary>
    /// Copy a source buffer of 2-bit elements (packed) into a destination buffer of 8-bit elements
    /// (unpacked).
    /// </summary>
    ...
    /// <summary>
    /// Copy a source buffer of 8-bit elements (unpacked) into a destination buffer of 2-bit
    /// elements (packed).
    /// </summary>
...
#endif
            dst[dst_i] =
                Int2x4Base<Signed>(src[src_i], src[src_i + 1], src[src_i + 2], src[src_i + 3]);
#if defined(__GNUC__) && defined(HAS_ARRAY_BOUNDS)
