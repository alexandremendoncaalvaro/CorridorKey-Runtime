...
class Tensor final {
   public:
    ...
    // Strive not to allocate Tensor with new/delete as it is a shallow class and using it by value
    // is just fine. Use InitOrtValue() methods to allocate for OrtValue.
     ...
     * This function does not check if the preallocated buffer(p_data) has enough room for the
     * shape. \param elt_type Data type of the tensor elements. \param shape Shape of the tensor
     ...
     */
    Tensor(MLDataType elt_type, const TensorShape& shape, void* p_data,
           const OrtMemoryInfo& location, ptrdiff_t offset = 0,
           gsl::span<const int64_t> strides = {});

    /**
     * Create tensor with given type, shape, pre-allocated memory and allocator which will be used
     * to free the pre-allocated memory. The Tensor will take over ownership of p_data. This
     * function does not check if the preallocated buffer(p_data) has enough room for the shape.
     * \param elt_type Data type of the tensor elements.
     * \param shape Shape of the tensor
     * \param p_data A preallocated buffer. Can be NULL if the shape is empty.
     *               Tensor will own the memory and will delete it when the tensor instance is
     * destructed. \param deleter Allocator used to free the pre-allocated memory \param offset
     * Offset in bytes to start of Tensor within p_data. \param strides Strides span. Can be empty
     * if the tensor is contiguous.
     */
    Tensor(MLDataType elt_type, const TensorShape& shape, void* p_data,
           std::shared_ptr<IAllocator> deleter, ptrdiff_t offset = 0,
           gsl::span<const int64_t> strides = {});
    ...
    /// <summary>
    /// Creates an instance of Tensor on the heap which will take over ownership of the
    /// pre-allocated buffer.
    /// </summary>
    /// <param name="elt_type">Data type of the tensor elements.</param>
    /// <param name="shape"Tensor shape.</param>
    /// <param name="p_data">Tensor data.</param>
    /// <param name="allocator">Allocator that was used to create p_data and will be used to free
    /// it.</param> <param name="ort_value">OrtValue to populate with Tensor.</param> <param
    /// name="offset">Optional offset if Tensor refers to a subset of p_data.</param> <param
    /// name="strides">Optional strides if Tensor refers to a subset of p_data.</param>
    static void InitOrtValue(MLDataType elt_type, const TensorShape& shape, void* p_data,
    ...
    /// <param name="shape">Tensor shape.</param>
    /// <param name="allocator">Allocator that was used to create p_data and will be used to free
    /// it.</param> <param name="ort_value">OrtValue to populate with Tensor.</param>
    static void InitOrtValue(MLDataType elt_type, const TensorShape& shape,
                             std::shared_ptr<IAllocator> allocator, OrtValue& ort_value);
    ...
    static Status CalculateTensorStorageSize(MLDataType elt_type, const TensorShape& shape,
                                             size_t alignment, size_t& storage_size);
    /**
    ...
    */
    ...
    T* MutableData() {
        // Type check
        ORT_ENFORCE(utils::IsPrimitiveDataType<T>(dtype_), "Tensor type mismatch. ", "T ",
                    "!=", dtype_);
    ...
    */
    ...
    gsl::span<const T> DataAsSpan() const {
        ...
        return gsl::make_span(data,
                              static_cast<typename gsl::span<T>::size_type>(NumStorageElements()));
    }
    ...
    inline void Reshape(const TensorShape& new_shape) {
        ORT_ENFORCE(shape_.Size() == new_shape.Size(),
                    "Tensor size (" + std::to_string(shape_.Size()) + ") != new size (" +
     ...
     * @warning use with caution - make sure you do boundary check before calling this method (see
     * view.cc)
     */
    ...
    /// <summary>
    /// The number of Tensor "storage" elements. A single storage element may contain multiple
    /// sub-elements for sub-byte data types (e.g., int4/float4).
    ///
    /// For element types smaller than 1 byte (e.g., int4/float4), a single storage element stores
    /// multiple sub-byte elements. Example: Tensor<int4> of shape (4,) has 2 storage elements.
    ///
    ...
    */
    ...
    // More API methods.
   private:
    void Init(MLDataType elt_type, const TensorShape& shape, void* p_raw_data, AllocatorPtr deleter,
              ptrdiff_t offset = 0, gsl::span<const int64_t> strides = {});
