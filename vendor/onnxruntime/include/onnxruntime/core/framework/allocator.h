...
struct OrtArenaCfg {
    ...
    // Bytes to keep on shrink for CudaMemPool, 0 is to attempt to release all, allocated space not
    // affected.
    ...
    bool IsValid() {
        return arena_extend_strategy >= -1 && arena_extend_strategy <= 1 &&
               ...
               initial_growth_chunk_size_bytes >= -1 && max_power_of_two_extend_bytes >= -1;
    }
    ...
    struct ConfigKeyNames {
        ...
        static constexpr const char* CudaMempoolReleaseThreshold =
            "arena.cuda_mempool_release_threshold";
        static constexpr const char* CudaMempoolBytesToKeepOnShrink =
            "arena.cuda_mempool_bytes_to_keep_on_shrink";
    };

    static onnxruntime::common::Status FromKeyValuePairs(const OrtKeyValuePairs& kvps,
                                                         OrtArenaCfg& cfg);
};
...
void* AllocateBufferWithOptions(IAllocator& allocator, size_t size, bool use_reserve,
                                Stream* stream,
...
class IAllocator {
   public:
     ...
     *
     * A device Stream may be available when executing a model on non-CPU devices. In this case
     * operations are queued asynchronously and the allocation/free call is made when the operation
     * is queued rather than executed. Due to this it is not safe to use the memory on another
     * stream or with no stream unless synchronization has occurred.
     *
     * ORT currently handles the synchronization when executing the model using streams.
     *
     * When two streams are synchronized the event used is identified by the producer stream's
     * latest sync id. This <producer stream:sync id> pair is copied into the sync information of
     * the consumer stream. Each new event creates a new sync id.
     *
     * It is safe to re-use an allocated piece of memory if:
     *   - the stream that currently owns the memory and the stream that wants to use the memory
     * have been synchronized,
     *   - and the sync id from when the memory was assigned to the stream that currently owns it is
     * less than the sync id from the last synchronization between the two streams.
     *     - e.g. stream0 is assigned the memory when its sync id is 1.
     *            stream0 (producer) and stream1 (consumer) are synchronized.
     *              stream0 sync id will be incremented to 2 when creating the event used in the
     * synchronization. stream1 will copy this information into its sync info and now contains an
     * entry for stream0 with a sync id of 2. stream0 frees the memory the memory is marked as not
     * in use, but still assigned to stream0 stream1 is now able to use the memory as it is not in
     * use, and the sync id from the allocation (1) is less than the sync id (2) that is has for
     * stream0. or
     *   - the inference session that owned the Stream has completed inferencing
     *     - Stream::CleanUpOnRunEnd is called when this occurs
     *     - any memory assigned to the Stream is now able to be used by other streams when it is
     * not longer in use.
     */
     ...
     */
    [[nodiscard]] static bool CalcMemSizeForArrayWithAlignment(size_t nmemb, size_t size,
                                                               size_t alignment,
                                                               size_t* out) noexcept;
    ...
    /**
       ...
       @param wait_fn If the allocator want to dynamic reuse a chunk from another stream, use this
       wait_fn to sync on the target stream to make the reuse safe.
       @returns std::unique_ptr with allocated memory and deleter. Throws if it cannot allocate
       memory.
    */
    template <typename T>
    static IAllocatorUniquePtr<T> MakeUniquePtr(std::shared_ptr<IAllocator> allocator,
                                                size_t count_or_bytes, bool use_reserve = false,
                                                Stream* stream = nullptr) {
        ValidateAllocator(allocator);

        // for now limit to fundamental types. we could support others, but to do so either we or
        // the caller needs to call the dtor for the objects, for buffers allocated on device we
        // don't have destructor static_assert(std::is_fundamental<T>::value, "Fundamental type
        // required as no destructors are called.");

        size_t alloc_size = count_or_bytes;

        // if T is not void, 'count_or_bytes' == number of items so allow for that
        if constexpr (!std::is_void<T>::value) {
            // sizeof(void) isn't valid, but the compiler isn't smart enough to ignore that this
            // line isn't reachable if T is void. use std::conditional to 'use' void* in the sizeof
            // call
            constexpr auto size =
        ...
        // allocate
        T* p = static_cast<T*>(
            AllocateBufferWithOptions(*allocator, alloc_size, use_reserve, stream, nullptr));
        ValidateAllocation(p, alloc_size);

        return IAllocatorUniquePtr<T>{
            p, [allocator = std::move(allocator)](T* p) { allocator->Free(p); }};
    }
    ...
    */
    template <typename T>
    static IAllocatorUniquePtr<T> MakeUniquePtrFromOrtAllocator(OrtAllocator* ort_allocator,
        ...
        return IAllocatorUniquePtr<T>{
            p, [ort_allocator](T* p) { ort_allocator->Free(ort_allocator, p); }};
    }

   private:
    //
    // validation functions. split out from methods that are templatized on the data type to
    ...
    static void ValidateAllocation(void* p, size_t size) {
        // allocator should throw directly but in case it didn't ensure we do here so that calling
        // code doesn't need to check for nullptr when an actual allocation was expected.
