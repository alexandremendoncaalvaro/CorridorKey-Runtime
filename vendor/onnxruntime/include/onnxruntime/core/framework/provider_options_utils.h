...
Status EnumToName(const EnumNameMapping<TEnum>& mapping, TEnum value, std::string& name) {
    const auto it = std::find_if(
        ...
        [&value](const std::pair<TEnum, std::string>& entry) { return entry.first == value; });
    ORT_RETURN_IF(it == mapping.end(), "Failed to map enum value to name: ",
...
Status NameToEnum(const EnumNameMapping<TEnum>& mapping, const std::string& name, TEnum& value) {
    const auto it = std::find_if(
        mapping.begin(), mapping.end(),
        [&name](const std::pair<TEnum, std::string>& entry) { return entry.second == name; });
    ORT_RETURN_IF(it == mapping.end(), "Failed to map enum name to value: ", name);
    value = it->first;
    return Status::OK();
}
...
class ProviderOptionsParser {
   public:
    ...
    template <typename ValueParserType>
    ProviderOptionsParser& AddValueParser(std::string_view name, ValueParserType value_parser) {
        ORT_ENFORCE(value_parsers_.emplace(name, ValueParser{value_parser}).second,
                    "Provider option \"", name, "\" already has a value parser.");
        return *this;
    }

    template <typename ValueParserType>
    ProviderOptionsParser& AddValueParser(const char* name, ValueParserType value_parser) {
    ...
    template <typename ValueType>
    ProviderOptionsParser& AddAssignmentToReference(std::string_view name, ValueType& dest) {
        return AddValueParser(name, [&dest](std::string_view value_str) -> Status {
            return ParseStringWithClassicLocale(value_str, dest);
        });
    }

    template <typename ValueType>
    ProviderOptionsParser& AddAssignmentToReference(const char* name, ValueType& dest) {
    ...
    template <typename EnumType>
    ProviderOptionsParser& AddAssignmentToEnumReference(std::string_view name,
                                                        const EnumNameMapping<EnumType>& mapping,
                                                        EnumType& dest) {
        return AddValueParser(name, [&mapping, &dest](const std::string& value_str) -> Status {
            return NameToEnum(mapping, value_str, dest);
        });
    }

    template <typename EnumType>
    ProviderOptionsParser& AddAssignmentToEnumReference(const char* name,
                                                        const EnumNameMapping<EnumType>& mapping,
                                                        EnumType& dest) {
    ...
    Status Parse(const ProviderOptions& options) const {
        for (const auto& option : options) {
            ...
            ORT_RETURN_IF(value_parser_it == value_parsers_.end(), "Unknown provider option: \"",
                          name, "\".");

            const auto parse_status = value_parser_it->second(value_str);
            ORT_RETURN_IF_NOT(parse_status.IsOK(), "Failed to parse provider option \"", name,
