...
// if we are export the fused function to dll, the function will still in the same binary as
// onnxruntime use std function to give execution provider some chance to capture some state.
...
class IExecutionProvider {
   protected:
    IExecutionProvider(const std::string& type) : IExecutionProvider(type, OrtDevice()) {}
   ...
   public:
    ...
    /**
     * Returns an external data loader object that implements methods to load data from external
     * sources.
     *
     * By default, framework will handle external data loading by loading the data into CPU memory
     * and then copying it to the target device if required. So in most cases, it's not necessary to
     * override this method. Specifically, in WebAssembly build, because the memory is limited and
     * Web platform supports loading data from external sources directly into GPU memory, this
     * method is overridden to provide a custom external data loader to avoid the extra CPU memory
     * usage.
     */
    ...
    /**
       ...
       The graph_optimizer_registry is designed for enabling L2+ graph optimizations tailored for
       EPs. These optimizations are applied after the graph partitioner assigns ComputeCapability to
       the EP and before EP's "Compile" or fusion.

       Steps to use graph_optimizer_registry and create the optimization ComputeCapability:
       1. Lookup Optimizer: The EP calls provider bridge API to lookup pre-defined optimizer by name
       and get selection function.
          - Example: g_host->GetOptimizerByName(optimizer_name, graph_optimizer_registry,
       selection_func)
       2. Run Selection Function: The EP executes the selection function to obtain the selection
       ComputeCapability.
          - ComputeCapability.optimize_func would be set by the optimizer to the function that does
       the optimization.
       3. Create Optimization ComputeCapability: The EP uses the selection ComputeCapability to
       create the optimization ComputeCapability.
       4. Return ComputeCapability: The EP returns the final ComputeCapability, with
       nodes_to_optimize set to the optimization ComputeCapability.

       Note: For more detailed implementations of using graph_optimizer_registry, please refer to
       TensorRT EP.
    */
    virtual std::vector<std::unique_ptr<ComputeCapability>> GetCapability(
        const onnxruntime::GraphViewer& graph_viewer, const IKernelLookup& kernel_lookup,
    ...
    */
    ...
    /**
       ...
       NOTE: In the case of ONNX model having EP specific custom nodes and don't want to ask user to
       register those nodes, EP might need to a way to register those custom nodes. This API is
       added for the purpose where EP can use it to leverage ORT custom op to register those custom
       nodes with one or more custom op domains.

       For example, TensorRT EP uses this API to support TRT plugins where each custom op is mapped
       to TRT plugin and no kernel implementation is needed for custom op since the real
       implementation is inside TRT. This custom op acts as a role to help pass ONNX model
       validation.
     */
    virtual void GetCustomOpDomainList(
        std::vector<OrtCustomOpDomain*>& /*provider custom op domain list*/) const {};
    ...
    */
    virtual common::Status OnRunStart(const onnxruntime::RunOptions& /*run_options*/) {
        return Status::OK();
    }
    ...
    */
     ...
     */
    virtual bool IsGraphCaptured(int /*graph_annotation_id*/) const {
        return false;
    }
    ...
    */
    ...
    enum class FusionStyle {
        // The node fusion will create an onnxruntime::Function based Node that contains a
        // completely new Graph instance
        // in the Node body. The original nodes and initializers are copied to the new Graph
        // instance in Function::Body().
        // A GraphProto can be produced from the Node body.
        Function,

        // The node fusion will create a new Node that defines the inputs and outputs using the
        // IndexedSubGraph
        // that GetCapability returned. The Node will not be onnxruntime::Function based so will
        // have no Body().
        // Instead a GraphViewer that filters the full Graph to the fused Nodes will be created.
        // This is significantly cheaper as it doesn't incur the cost of creating a new Graph
        // instance,
    ...
    /**
    Given a collection of fused Nodes and the respective GraphViewer instance for the nodes that
    were fused, return create_state/compute/release_state func for each node.
    @remarks This is now the default interface when execution provider wants to compile nodes
             ...
             Do NOT cache the GraphViewer in FusedNodeAndGraph.filtered_graph in any of the
    NodeComputeInfo functions as it is only valid for the duration of the call to Compile.
    */
     ...
     *
     * The execution provider determines this value, which denotes the compatibility of the compiled
     * model with the EP. This is stored in the model metadata under a key associated with the EP
     * type.
     */
    virtual std::string GetCompiledModelCompatibilityInfo(
    ...
    /**
      Given an op with domain `domain` and type `op_type`, determine whether an associated node's
      data layout should be converted to `target_data_layout`. If the EP prefers a non-default data
      layout (see `GetPreferredLayout()`), this function will be called during layout transformation
      with `target_data_layout` set to the EP's preferred data layout. A return value of
      `std::nullopt` indicates that this decision is left to ORT.
    */
    virtual std::optional<bool> ShouldConvertDataLayoutForOp(
        std::string_view /*domain*/, std::string_view /*op_type*/,
        DataLayout /*target_data_layout*/) const {
        return std::nullopt;
    }

    virtual void RegisterStreamHandlers(IStreamCommandHandleRegistry& /*stream_handle_registry*/,
                                        AllocatorMap&) const {}
     ...
     * This function is a stateless function which creates new instances of Allocator, without
     * storing them in EP.
     */
     ...
     * EP needs to implement this if has the requirement to generate the context cache model.
     * Otherwise leave it. Default return an empty vector if not provided by the Execution Provider
     */
     ...
     * This is used to retrieve the OrtEp instance from a OrtKernelInfo instance in a plugin EP's
