...
class OpNodeProtoHelper {
   public:
    ...
    /**
       ...
       Call this function for a required attribute or when a default value for an optional attribute
       is specified in the op schema
    */
    template <typename T>
    ...
    /**
       Get a single attribute
       Call this function for a required attribute or when a default value for an optional attribute
       is specified in the op schema Throws if an attribute with the specified type doesn't exist
    */
    template <typename T>
    ...
    /**
       Get a single attribute
       Call this function only when a default value for an optional attribute isn't specified in the
       op schema
    */
    template <typename T>
    ...
    */
    ...
    void GetAttrOrDefault(const std::string& name, T* value, const T& default_value) const {
        if (!GetAttr<T>(name, value).IsOK()) *value = default_value;
    }
    ...
    */
    ...
    [[nodiscard]] TensorShapeVector GetAttrsOrDefault(
        const std::string& name, const TensorShapeVector& default_value = {}) const {
    ...
    */
    ...
    [[nodiscard]] uint32_t GetPrimitiveAttrElementCount(
        ONNX_NAMESPACE::AttributeProto_AttributeType type, const std::string& name) const noexcept;
    ...
    [[nodiscard]] const ONNX_NAMESPACE::AttributeProto* TryGetAttribute(
        const std::string& name) const {
        return impl_->getAttribute(name);
    }

    [[nodiscard]] const ONNX_NAMESPACE::AttributeProto* GetAttribute(
        const std::string& name) const {
