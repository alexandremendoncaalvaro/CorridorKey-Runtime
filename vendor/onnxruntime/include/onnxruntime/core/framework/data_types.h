...
class DataTypeImpl {
   ...
   public:
    ...
    // Returns this if this is one of the primitive data types (specialization of
    // PrimitiveDataTypeBase) and null otherwise
     ...
     * However, this conversion is lossy. Don't try to use 'this->GetTypeProto()' converting it
     * back. Even though GetTypeProto() will not have the original information, it will still have
     * enough to correctly map to MLDataType. \param proto
     */
    ...
    static const std::vector<MLDataType>&
    AllFixedSizeTensorTypes();  // up to IR4 (no float 8), deprecated
    ...
    static const std::vector<MLDataType>&
    AllSequenceTensorTypes();  // up to IR4 (no float 8), deprecated
    ...
    static const std::vector<MLDataType>&
    AllFixedSizeSequenceTensorTypes();  // up to IR4 (no float 8), deprecated
    ...
    static const std::vector<MLDataType>&
    AllNumericTensorTypes();  // up to IR4 (no float 8), deprecated
    ...
    static const std::vector<MLDataType>&
    AllTensorAndSequenceTensorTypes();  // up to IR4 (no float 8), deprecated
    ...
    static const std::vector<MLDataType>&
    AllOptionalAndTensorAndSequenceTensorTypes();  // up to IR4 (no float 8), deprecated
    ...
    static const std::vector<MLDataType>&
    AllFixedSizeTensorAndSequenceTensorTypes();  // up to IR4 (no float 8), deprecated
    ...
    static const std::vector<MLDataType>&
    AllTensorAndSequenceTensorAndOptionalTypes();  // up to IR4 (no float 8), deprecated
...
struct IsAnyOf<T, H, Tail...> {
    static constexpr bool value = (std::is_same<T, H>::value || IsAnyOf<T, Tail...>::value);
};
...
struct IsTensorContainedType
    : public IsAnyOf<T, float, uint8_t, int8_t, uint16_t, int16_t, int32_t, int64_t, std::string,
                     bool, MLFloat16, double, uint32_t, uint64_t, BFloat16, Int4x2, UInt4x2, Int2x4,
                     UInt2x4
...
struct MapTypeHelper {
    ...
    static void Set(ONNX_NAMESPACE::TensorProto_DataType key_type,
...
struct OptionalTypeHelper {
    ...
    static MLDataType GetElemType() {
        if constexpr (std::is_same<T, Tensor>::value) {
            ...
            static_assert(std::is_same<T, TensorSeq>::value,
                          "Unsupported element type for optional type");
...
// The suppressed warning is: "The type with a virtual function needs either public virtual or
// protected nonvirtual destructor." However, we do not allocate this type on heap.
...
/// Common base-class for all disabled types. We need DataTypeImpl::ToString to work in a minimal
/// build with disabled types to keep the ORT format model kernel hashes stable.
...
#else
    OptionalType()
        : DisabledTypeBase{DataTypeImpl::GeneralType::kOptional, 0}
#endif
    {
        using namespace data_types_internal;
        OptionalTypeHelper::Set(OptionalTypeHelper::GetElemType<T, elemT>()->GetTypeProto(),
                                MutableTypeProto());
    }
};  // namespace onnxruntime
...
struct NonTensorTypeConverter {
    static void FromContainer(MLDataType /*dtype*/, const void* /*data*/, size_t /*data_size*/,
                              OrtValue& /*output*/) {
...
class NonTensorTypeBase : public DataTypeImpl {
   public:
    ...
    // The caller of the interface should have a shared definition of the data which is used to
    // initialize CPP data representation. This is used from C API.
    //
...
class MapType : public NonTensorType<CPPType> {
   ...
   private:
    MapType() {
        using namespace data_types_internal;
        MapTypeHelper::Set(
            utils::ToTensorProtoElementType<typename CPPType::key_type>(),
...
class SequenceType : public NonTensorType<CPPType> {
   ...
   private:
    SequenceType() {
        using namespace data_types_internal;
        SequenceTypeHelper::Set(
            SequenceTypeHelper::GetElemType<typename CPPType::value_type>()->GetTypeProto(),
            this->MutableTypeProto());
    }
};
 ...
 *          used for input data type dispatching. This class also stores the number of subelements
 * per size units. Example: For float4/int4, the size unit is 1 byte and the number of subelements
 * is 2.
 *
 */
class PrimitiveDataTypeBase : public DataTypeImpl {
   ...
   protected:
    PrimitiveDataTypeBase(size_t size, int32_t data_type, int32_t num_sub_elems)
        : DataTypeImpl{GeneralType::kPrimitive, size},
          data_type_{data_type},
          num_sub_elems_{num_sub_elems} {}
...
class PrimitiveDataType : public PrimitiveDataTypeBase {
   ...
   private:
    explicit PrimitiveDataType(int32_t num_sub_elems)
        : PrimitiveDataTypeBase{sizeof(T), utils::ToTensorProtoElementType<T>(), num_sub_elems} {}
};
...
//   - [not supported] Int3x8 could store 8 packed 3-bit elements in 3 bytes:
//   ORT_*_SUBBYTE_TYPE(Int3x8, 8)
