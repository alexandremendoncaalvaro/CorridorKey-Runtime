...
/**
 * Each provider has a KernelRegistry. Often, the KernelRegistry only belongs to that specific
 * provider.
 */
class KernelRegistry {
   public:
    ...
    // TODO(edgchen1) for TryFindKernel(), consider using `out` != nullptr as indicator of whether
    // kernel was found and Status as an indication of failure
    ...
    Status TryFindKernel(const Node& node, ProviderType exec_provider,
                         ...
                         const logging::Logger& logger, const KernelCreateInfo** out) const;

    // map of type constraint name to required type
    using TypeConstraintMap = InlinedHashMap<std::string, MLDataType>;

    // Check if an execution provider can create kernel for a node and return the kernel if so.
    // Kernel matching uses the explicit type constraint name to required type map in
    // type_constraints.
    Status TryFindKernel(const Node& node, ProviderType exec_provider,
                         const TypeConstraintMap& type_constraints, const logging::Logger& logger,
     ...
     * @return
     */
    Status TryFindKernel(ProviderType exec_provider, std::string_view op_type,
                         std::string_view domain, int version,
    ...
    // This is used by the opkernel doc generator to enlist all registered operators for a given
    // provider's opkernel
   ...
   private:
    // TryFindKernel implementation. Either kernel_type_str_resolver or type_constraints is
    // provided.
    ...
    //
    // In typical usage kernel_type_str_resolver is provided and type information from the node is
    // used with kernel_type_str_resolver.
    //
    // There is also usage from a node dynamically created within a custom op via OrtApi CreateOp
    // where an explicit type value for each type constraint is provided in type_constraints.
    //
    ...
    static bool VerifyKernelDef(const Node& node, const KernelDef& kernel_def,
                                const IKernelTypeStrResolver* kernel_type_str_resolver,
                                const TypeConstraintMap* type_constraints, std::string& error_str);

    static std::string GetMapKey(std::string_view op_name, std::string_view domain,
                                 std::string_view provider) {
        std::string key(op_name);
        // use the kOnnxDomainAlias of 'ai.onnx' instead of kOnnxDomain's empty string
        key.append(1, ' ')
            .append(domain.empty() ? kOnnxDomainAlias : domain)
            .append(1, ' ')
            .append(provider);
        return key;
    }
