...
//
//  1) If you need to handle all of the primitive tensor contained datatypes, the best choice would
//  be macros
//     DispatchOnTensorType or DispatchOnTensorTypeWithReturn. Use inline wrappers so your function
//     can be invoked as function<T>().
//  2) if you have a few types, use Tensor.IsDataType<T>()/IsDataTypeString() or use
//  utils::IsPrimitiveDataType<T>()
...
//
// Invoking DataTypeImpl::GetType<T>() for switching on input types is discouraged and should be
// avoided. Every primitive type carries with it an integer constant that can be used for quick
// switching on types.
...
inline bool IsDataTypeString(MLDataType dt_type) {
    ...
    return (prim_type != nullptr &&
            prim_type->GetDataType() == ONNX_NAMESPACE::TensorProto_DataType_STRING);
}
...
// This implementation contains a workaround for GCC bug
// https://gcc.gnu.org/bugzilla/show_bug.cgi?id=47226 GCC until very recently does not support
// template parameter pack expansion within lambda context.
...
class CallableDispatchableRetHelper {
   ...
   public:
    explicit CallableDispatchableRetHelper(int32_t dt_type) noexcept
        : dt_type_(dt_type), called_(0), result_() {}
...
using UndefinedTensorProtoElementTypeConstant =
    std::integral_constant<ONNX_NAMESPACE::TensorProto_DataType,
...
class MLTypeCallDispatcher {
    ...
    using SupportedTensorProtoElementTypeList =
        boost::mp11::mp_transform<mltype_dispatcher_internal::TensorProtoElementTypeConstant,
                                  SupportedTypeList>;

    static_assert(
        boost::mp11::mp_and<
            boost::mp11::mp_is_set<SupportedTensorProtoElementTypeList>,
            boost::mp11::mp_not<boost::mp11::mp_set_contains<
   ...
   public:
     ...
     * @tparam Args The argument types.
     */
    template <template <typename...> class Fn, typename LeadingTemplateArgTypeList,
              typename... Args>
    void InvokeWithLeadingTemplateArgs(Args&&... args) const {
        static_assert(boost::mp11::mp_is_list<LeadingTemplateArgTypeList>::value,
                      "LeadingTemplateArgTypeList must be a type list (e.g., "
                      "onnxruntime::TypeList<T1, T2, ...>).");
        ...
        static_cast<void>(std::array<int, sizeof...(Types)>{helper.template Invoke<Types>(
            boost::mp11::mp_apply<Fn,
                                  boost::mp11::mp_push_back<LeadingTemplateArgTypeList, Types>>(),
    ...
    Ret InvokeRetWithUnsupportedPolicy(Args&&... args) const {
        return InvokeRetWithUnsupportedPolicyAndLeadingTemplateArgs<Ret, Fn, UnsupportedPolicy,
                                                                    TypeList<>>(
            std::forward<Args>(args)...);
    }
     ...
     * @tparam Args The argument types.
     */
    template <class Ret, template <typename...> class Fn, typename LeadingTemplateArgTypeList,
              typename... Args>
    Ret InvokeRetWithLeadingTemplateArgs(Args&&... args) const {
        return InvokeRetWithUnsupportedPolicyAndLeadingTemplateArgs<
            Ret, Fn, mltype_dispatcher_internal::UnsupportedTypeDefaultPolicy<Ret>,
            LeadingTemplateArgTypeList>(std::forward<Args>(args)...);
    }
     ...
     * @tparam Args The argument types.
     */
    template <class Ret, template <typename...> class Fn, class UnsupportedPolicy,
              typename LeadingTemplateArgTypeList, typename... Args>
    Ret InvokeRetWithUnsupportedPolicyAndLeadingTemplateArgs(Args&&... args) const {
        mltype_dispatcher_internal::CallableDispatchableRetHelper<Ret, UnsupportedPolicy> helper(
            dt_type_);
...
class ContainerChecker {
    ...
    struct IsContainerOfType<std::map<K, V>> {
        static bool check(const Cont& c, size_t index) {
            static_assert(
                ToTensorProtoElementType<K>() != ONNX_NAMESPACE::TensorProto_DataType_UNDEFINED,
