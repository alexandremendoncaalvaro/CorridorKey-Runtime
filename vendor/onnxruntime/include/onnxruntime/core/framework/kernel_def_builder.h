...
class KernelDef {
   private:
    // note that input/output might be on CPU implicitly when the node is from CPU execution
    // provider
   ...
   public:
    ...
    OrtMemType InputMemoryType(size_t input_index) const {
        ...
        if (it == input_memory_type_args_.end()) return default_inputs_mem_type_;
        return it->second;
    }

    bool IsInputOnCpu(size_t input_index) const {
        return MemTypeOnCpuExplicitly(InputMemoryType(input_index));
    }

    bool IsOutputOnCpu(size_t output_index) const {
        return MemTypeOnCpuExplicitly(OutputMemoryType(output_index));
    }

    bool AllocateInputsContiguously() const {
        return allocate_inputs_contiguously_;
    }
    ...
    OrtMemType OutputMemoryType(size_t output_index) const {
        ...
        if (it == output_memory_type_args_.end()) return default_outputs_mem_type_;
        return it->second;
    }
   ...
   private:
    ...
    // An element <i, j> means j-th output can be a strided tensor, which share the data from i-th
    // input.
...
class KernelDefBuilder {
   public:
    ...
    explicit KernelDefBuilder() : kernel_def_(std::make_unique<KernelDef>()) {}
    ...
    */
    ...
    /**
       ...
       The OrtValue set as external outputs, must be safe to release as long as the OrtValue's
       reference count reaches zero in ORT's allocation/deallocation plan. We usually create such an
       OrtValue following flows: torch tensors --> to dlpack tensors (destructor will release a view
       of original torch tensor, instead of releasing original torch tensor) --> to OrtValue.

       When the OrtValue is not needed in the graph, then it will be released after calling the
       attached destructor. The destructor will release the view of the original torch tensor,
       instead of releasing the original torch tensor. This is to make sure the original torch
       tensor can still be okay to use externally, even after OrtValue is released in the graph.
       (Recalled this OrtValue is also not reused by ORT).
    */
