...
class OpKernel {
   public:
    ...
    // @param alloc: The kernel's PrePack() method MUST use this allocator for allocating the
    // pre-packed
    //               weights' buffers. The alloc that the PrePack() method will receive will be
    //               either the allocator tied to the session if the kernel owns the pre-packed
    //               buffer or an allocator shared between sessions if the pre-packed buffer is to
    //               be shared across sessions (i.e.) the kernel does not own the buffer.
    // @param is_packed: Set it to true if the kernel packed the tensor or to false
    //                   The kernel is responsible for keeping the packed data and related metadata
    //                   if is_packed is true, and the original initialized constant tensor will be
    //                   released and not accessible anymore in the Compute function.
    // @param prepacked_weights: A PrePackedWeights instance will be provided to the kernel IF the
    // pre-packed weights
    //                          are meant to be stored in a shared container.

    virtual Status PrePack(const Tensor& /*tensor*/, int /*input_idx*/, AllocatorPtr /*alloc*/,
                           /*out*/ bool& is_packed,
    ...
    //   }
    // Please refer to MatMulIntegerToFloatBase for a complete example
    // @param prepacked_buffers: The pre-packed buffers to be used by this kernel for the provided
    // input index
    //                           (Sometimes a single constant initializer may have multiple
    //                           pre-packed buffers associated
    //                            with it and it upto the kernel developer to store it in any order
    //                            of their choice in PrePack() and must use the same order for
    //                            retrieval in UseSharedPrePackedBuffers(). Though each element
    //                           of this vector is a BufferUniquePtr, the deleter of the
    //                           BufferUniquePtr is NULL. So actually they are raw pointers.
    // @param input_idx: The input index of the tensor in this kernel
    ...
    /// <summary>
    /// Version 2 of OpKernel::UseSharedPrePackedBuffers() that additionally accepts the buffer
    /// sizes as a parameter. The default implementation of this function just calls directly to
    /// OpKernel::UseSharedPrePackedBuffers() to avoid the need to update all existing kernel-based
    /// provider-bridge EPs.
    ///
    /// TODO: Consolidate UseSharedPrePackedBuffers and UseSharedPrePackedBuffers_V2 into a single
    /// function, which will require updating kernel-based provider-bridge EPs (cpu, cuda, webgpu).
    ///
    /// </summary>
...
class FuncManager;
using KernelCreateFn = std::function<Status(FuncManager& func_mgr, const OpKernelInfo& info,
                                            std::unique_ptr<OpKernel>& out)>;
using KernelCreatePtrFn = std::add_pointer<Status(FuncManager& func_mgr, const OpKernelInfo& info,
                                                  std::unique_ptr<OpKernel>& out)>::type;
...
struct KernelCreateInfo {
    ...
    KernelCreateInfo(std::unique_ptr<KernelDef> definition, KernelCreateFn create_func)
        : kernel_def(std::move(definition)), kernel_create_func(create_func) {
...
#define ONNX_OPERATOR_KERNEL_EX(name, domain, ver, provider, builder, ...)                         \
    ...
    BuildKernelCreateInfo<ONNX_OPERATOR_KERNEL_CLASS_NAME(provider, domain, ver, name)>() {        \
        return KernelCreateInfo(                                                                   \
            ...
            static_cast<KernelCreatePtrFn>([](FuncManager&, const OpKernelInfo& info,              \
...
#define ONNX_CPU_OPERATOR_VERSIONED_KERNEL(name, startver, endver, builder, ...)                  \
    ONNX_OPERATOR_VERSIONED_KERNEL_EX(name, kOnnxDomain, startver, endver, kCpuExecutionProvider, \
                                      builder, __VA_ARGS__)

#define ONNX_CPU_OPERATOR_VERSIONED_ML_KERNEL(name, startver, endver, builder, ...)             \
    ONNX_OPERATOR_VERSIONED_KERNEL_EX(name, kMLDomain, startver, endver, kCpuExecutionProvider, \
                                      builder, __VA_ARGS__)
...
#define ONNX_OPERATOR_VERSIONED_KERNEL_EX(name, domain, startver, endver, provider, builder, ...) \
    ...
    template <>                                                                                   \
    KernelCreateInfo BuildKernelCreateInfo<ONNX_OPERATOR_VERSIONED_KERNEL_CLASS_NAME(             \
...
#define ONNX_CPU_OPERATOR_TYPED_KERNEL(name, ver, type, builder, ...)                           \
    ONNX_OPERATOR_TYPED_KERNEL_EX(name, kOnnxDomain, ver, type, kCpuExecutionProvider, builder, \
                                  __VA_ARGS__)

#define ONNX_CPU_OPERATOR_TYPED_ML_KERNEL(name, ver, type, builder, ...)                      \
    ONNX_OPERATOR_TYPED_KERNEL_EX(name, kMLDomain, ver, type, kCpuExecutionProvider, builder, \
                                  __VA_ARGS__)

#define ONNX_CPU_OPERATOR_TYPED_MS_KERNEL(name, ver, type, builder, ...)                      \
    ONNX_OPERATOR_TYPED_KERNEL_EX(name, kMSDomain, ver, type, kCpuExecutionProvider, builder, \
                                  __VA_ARGS__)
...
#define ONNX_OPERATOR_TYPED_KERNEL_EX(name, domain, ver, type, provider, builder, ...)             \
    ...
    template <>                                                                                    \
    KernelCreateInfo BuildKernelCreateInfo<ONNX_OPERATOR_TYPED_KERNEL_CLASS_NAME(                  \
...
#define ONNX_OPERATOR_TWO_TYPED_KERNEL_EX(name, domain, ver, type1, type2, provider, builder, ...) \
    ...
    template <>                                                                                    \
    KernelCreateInfo BuildKernelCreateInfo<ONNX_OPERATOR_TWO_TYPED_KERNEL_CLASS_NAME(              \
        provider, domain, ver, type1, type2, name)>() {                                            \
        return KernelCreateInfo(                                                                   \
            builder.SetName(#name).SetDomain(domain).SinceVersion(ver).Provider(provider).Build(), \
            static_cast<KernelCreatePtrFn>([](FuncManager&, const OpKernelInfo& info,              \
                                              std::unique_ptr<OpKernel>& out) -> Status {          \
...
#define ONNX_OPERATOR_THREE_TYPED_KERNEL_CLASS_NAME(provider, domain, ver, type1, type2, type3, \
                                                    name)                                       \
    provider##_##name##_##domain##_ver##ver##_##type1##_##type2##_##type3

#define ONNX_OPERATOR_THREE_TYPED_KERNEL_EX(name, domain, ver, type1, type2, type3, provider,      \
                                            builder, ...)                                          \
    class ONNX_OPERATOR_THREE_TYPED_KERNEL_CLASS_NAME(provider, domain, ver, type1, type2, type3,  \
                                                      name);                                       \
    template <>                                                                                    \
    KernelCreateInfo BuildKernelCreateInfo<ONNX_OPERATOR_THREE_TYPED_KERNEL_CLASS_NAME(            \
...
#define ONNX_OPERATOR_VERSIONED_TYPED_KERNEL_CLASS_NAME(provider, domain, startver, endver, type, \
                                                        name)                                     \
...
#define ONNX_CPU_OPERATOR_VERSIONED_TYPED_KERNEL(name, startver, endver, type, builder, ...) \
    ONNX_OPERATOR_VERSIONED_TYPED_KERNEL_EX(name, kOnnxDomain, startver, endver, type,       \
                                            kCpuExecutionProvider, builder, __VA_ARGS__)

#define ONNX_CPU_OPERATOR_VERSIONED_TYPED_ML_KERNEL(name, startver, endver, type, builder, ...) \
    ONNX_OPERATOR_VERSIONED_TYPED_KERNEL_EX(name, kMLDomain, startver, endver, type,            \
                                            kCpuExecutionProvider, builder, __VA_ARGS__)

#define ONNX_CPU_OPERATOR_VERSIONED_TYPED_MS_KERNEL(name, startver, endver, type, builder, ...) \
    ONNX_OPERATOR_VERSIONED_TYPED_KERNEL_EX(name, kMSDomain, startver, endver, type,            \
                                            kCpuExecutionProvider, builder, __VA_ARGS__)

#define ONNX_OPERATOR_VERSIONED_TYPED_KERNEL_EX(name, domain, startver, endver, type, provider, \
                                                builder, ...)                                   \
    class ONNX_OPERATOR_VERSIONED_TYPED_KERNEL_CLASS_NAME(provider, domain, startver, endver,   \
                                                          type, name);                          \
    template <>                                                                                 \
    KernelCreateInfo BuildKernelCreateInfo<ONNX_OPERATOR_VERSIONED_TYPED_KERNEL_CLASS_NAME(     \
...
#define ONNX_OPERATOR_VERSIONED_TWO_TYPED_KERNEL_CLASS_NAME(provider, domain, startver, endver, \
                                                            type1, type2, name)                 \
...
#define ONNX_OPERATOR_VERSIONED_TWO_TYPED_KERNEL_EX(name, domain, startver, endver, type1, type2, \
                                                    provider, builder, ...)                       \
    class ONNX_OPERATOR_VERSIONED_TWO_TYPED_KERNEL_CLASS_NAME(provider, domain, startver, endver, \
                                                              type1, type2, name);                \
    template <>                                                                                   \
    KernelCreateInfo BuildKernelCreateInfo<ONNX_OPERATOR_VERSIONED_TWO_TYPED_KERNEL_CLASS_NAME(   \
