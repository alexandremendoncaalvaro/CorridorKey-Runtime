...
struct OrtMemoryInfo {
    ...
    OrtMemoryInfo(std::string name_, OrtAllocatorType type_, OrtDevice device_ = OrtDevice(),
        ...
        : name(std::move(name_)), mem_type(mem_type_), alloc_type(type_), device(device_) {}

    // To make OrtMemoryInfo become a valid key in std map
    bool operator<(const OrtMemoryInfo& other) const {
        if (alloc_type != other.alloc_type) return alloc_type < other.alloc_type;
        if (mem_type != other.mem_type) return mem_type < other.mem_type;
    ...
    std::string ToString() const {
        ...
        ostr << "OrtMemoryInfo:[" << "name:" << name << " OrtMemType:" << mem_type
             << " OrtAllocatorType:" << alloc_type << " " << device.ToString() << "]";
        return ostr.str();
    }
};
...
inline bool operator==(const OrtMemoryInfo& left, const OrtMemoryInfo& other) {
    return left.mem_type == other.mem_type && left.alloc_type == other.alloc_type &&
           left.device == other.device && left.name == other.name;
}

inline bool operator!=(const OrtMemoryInfo& lhs, const OrtMemoryInfo& rhs) {
    return !(lhs == rhs);
}
