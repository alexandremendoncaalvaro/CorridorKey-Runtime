...
// A memory allocation and allocator have a specific OrtDevice associated with them, and this
// information is used to determine when data transfer is required.
struct OrtDevice {
    ...
    struct MemType {
        ...
        //   - For memory that is only accessible by a specific device and CPU, use the specific
        //   device type and id.
        //   - When creating an OrtDevice for an EP allocator, you would typically use the same
        //   device type and id
        //     that the EP is registered with (i.e. the OrtDevice passed to the base
        //     IExecutionProvider constructor).
    ...
    enum VendorIds : VendorId {
        // No vendor ID. Valid for DeviceType::CPU + MemType::DEFAULT or for generic allocators like
        // WebGPU.
        NONE = 0x0000,
    ...
    constexpr OrtDevice(DeviceType device_type_, MemoryType memory_type_, VendorId vendor_id_,
                        DeviceId device_id_, Alignment alignment) /*noexcept*/
    ...
    std::string ToString() const {
        ...
        ostr << "Device:[" << "DeviceType:" << static_cast<int>(device_type)
             << " MemoryType:" << static_cast<int>(memory_type) << " VendorId:" << vendor_id
             << " DeviceId:" << device_id << " Alignment:" << alignment << "]";
        return ostr.str();
    }
    ...
    bool operator<(const OrtDevice& other) const {
        if (device_type != other.device_type) return device_type < other.device_type;
        if (memory_type != other.memory_type) return memory_type < other.memory_type;
    ...
    bool EqualIgnoringAlignment(const OrtDevice& other) const {
        return device_type == other.device_type && memory_type == other.memory_type &&
...
inline bool operator==(const OrtDevice& left, const OrtDevice& other) {
    return left.Type() == other.Type() && left.MemType() == other.MemType() &&
           left.Vendor() == other.Vendor() && left.Id() == other.Id() &&
