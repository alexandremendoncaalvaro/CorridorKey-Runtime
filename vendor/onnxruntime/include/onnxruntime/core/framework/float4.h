...
struct Float4E2M1x2 {
   ...
   private:
    ORT_HOST_DEVICE UnpackedType Fp4ToFloatConversionCpuHelper(uint8_t fp4x2, size_t shift) const {
        ...
        return (exponent == 0)
                   ? (sign * (mantissa / 2.f))
                   : (sign * (1.f + mantissa / 2.f) * static_cast<float>(1 << (exponent - 1)));
    }
    ...
    ORT_HOST_DEVICE uint8_t FloatToFp4ConversionCpuHelper(float f, size_t shift) const {
        ...
        if (((float_bits & exponent_bitmask) == exponent_bitmask) &&
            (float_bits & mantissa_bitmask)) {
            return (0x07 << shift);
        }
        ...
        if (((float_bits & exponent_bitmask) == exponent_bitmask) &&
            ((float_bits & mantissa_bitmask) == zero)) {
   ...
   public:
    ...
    inline ORT_HOST_DEVICE float2 ToCudaFloat2() const {
        return __half22float2(
            __nv_cvt_fp4x2_to_halfraw2(static_cast<PackedCudaStorageType>(val_), __NV_E2M1));
    }
#endif
...
#else
        return std::make_pair(Fp4ToFloatConversionCpuHelper(val_, 0),
                              Fp4ToFloatConversionCpuHelper(val_, 4));
#endif
    }
    ...
    static void UnpackFloat4E2M1ToFloat(const Float4E2M1x2* fp4x2_arr, UnpackedType* flt_arr,
                                        size_t size) {
        auto src = fp4x2_arr;
        auto dst = flt_arr;
    ...
    static void PackFloatToFloat4E2M1(const UnpackedType* flt_arr, Float4E2M1x2* fp4x2_arr,
                                      size_t size) {
        auto src = flt_arr;
        auto dst = fp4x2_arr;
...
inline ORT_HOST_DEVICE bool operator==(const Float4E2M1x2& left, const Float4E2M1x2& right) {
...
class numeric_limits<onnxruntime::Float4E2M1x2> {
   public:
    ...
    static constexpr int min_exponent =
        1;  // 2 ^ (1-1) = 1 is the valid normalized value min ceiling we can reach
    static constexpr int min_exponent10 =
        0;  // 10 ^ 0 is the valid normalized value min ceiling we can reach
    static constexpr int max_exponent =
        3;  // 2 ^ (3-1) = 4 is valid normalized value max ceiling we can reach
    static constexpr int max_exponent10 =
        0;  // 10 ^ 0 is the valid normalized value max ceiling we can reach
