 ...
 *
 * - Represent sparse data that is a result of format conversion or a computation result. Use second
 constructor
 *   to supply a desired allocator. Use Make*() format specific interfaces to supply values and
 format
...
class SparseTensor final {
   public:
    ...
    /// <param name="values_shape">shape for user supplied values. Use {0} shape for fully sparse
    /// tensors.</param> <param name="values_data">a pointer to values. Use nullptr for fully sparse
    /// tensors.</param> <param name="location">description of the user allocated memory</param>
    SparseTensor(MLDataType elt_type, const TensorShape& dense_shape,
                 const TensorShape& values_shape, void* values_data, const OrtMemoryInfo& location);
    ...
    /// <param name="values_data">pointer to a user allocated buffer. Use nullptr for fully sparse
    /// tensors.</param> <param name="location">description of the user allocated buffer</param>
    /// <param name="ort_value">default constructed input/output ort_value</param>
    static void InitOrtValue(MLDataType elt_type, const TensorShape& dense_shape,
    ...
    size_t NumValues() const {
        return static_cast<size_t>(values_.Shape().Size());
    }
    ...
    class CooView {
       public:
        explicit CooView(const Tensor& indices) noexcept : indices_(indices) {}
    ...
    /// to the constructor. The buffer is used as is and its lifespan must eclipse the lifespan of
    /// the sparse tensor instance. The OrtMemoryInfo (location) of the index is assumed to be the
    /// same as values.
    ///
    ...
    /// </summary>
    /// <param name="indices">user allocated buffer span. Use empty span for fully sparse
    /// tensors.</param> <returns>Status</returns>
    ...
    /// <param name="values_data">pointer to a buffer to be copied. Use nullptr for fully sparse
    /// tensors.</param> <param name="indices"></param> <returns></returns>
    ...
    /// with copies of the supplied zero-terminated strings followed by COO indices. All data is
    /// assumed to be on CPU and the allocator supplied must be a CPU based allocator.
    /// </summary>
    ...
    Status MakeCooStrings(size_t string_count, const char* const* strings,
                          gsl::span<const int64_t> indices);
    ...
    class CsrView {
       public:
        CsrView(const Tensor& inner, const Tensor& outer) noexcept : inner_(inner), outer_(outer) {}
    ...
    /// </summary>
    /// <param name="inner_index">User allocated buffer span. use empty span for fully sparse
    /// tensors</param> <param name="outer_index">User allocated buffer span. Use empty span for
    ...
    /// <param name="values_count">use 0 for fully sparse tensors</param>
    /// <param name="values_data">pointer to data to be copied. Use nullptr for fully sparse
    /// tensors.</param> <param name="inner_index">inner index to be copied. Use empty span for
    /// fully sparse tensors.</param> <param name="outer_index">outer index to be copied. Use empty
    /// span for fully sparse tensors.</param> <returns></returns>
    Status MakeCsrData(const IDataTransfer& data_transfer, const OrtMemoryInfo& data_location,
                       size_t values_count, const void* values_data,
    ...
    /// <param name="inner_index">inner index to be copied. Use empty span for fully sparse
    /// tensors.</param> <param name="outer_index">outer index to be copied. Use empty span for
    /// fully sparse tensors.</param> <returns></returns>
    ...
    class BlockSparseView {
       public:
        explicit BlockSparseView(const Tensor& indices) noexcept : indices_(indices) {}
        const Tensor& Indices() const noexcept {
            return indices_;
        }
    ...
    /// <param name="indices_data">Ptr to user allocated buffer. Use nullptr for fully spare
    ...
    /// <param name="data_transfer"></param>
    /// <param name="data_location"></param>
    /// <param name="values_shape">The shape is expected to be at least 3-D. However, use {0} for
    /// fully sparse tensors.</param> <param name="values_data">Pointer to a data to be copied. Use
    /// nullptr for fully sparse tensors.</param> <param name="indices_shape">The shape is expected
    /// to be 2-D. However, you can use {0} for fully sparse tensors.</param> <param
    /// name="indices_data">Pointer to index data to be copied. Use nullptr for fully sparse
    /// tensors.</param> <returns></returns>
    Status MakeBlockSparseData(const IDataTransfer& data_transfer,
                               const OrtMemoryInfo& data_location, const TensorShape& values_shape,
                               const void* values_data, const TensorShape& indices_shape,
    ...
    /// data can be populated
    /// </summary>
    /// <param name="values_shape">Shape is expected to be 3-D, use {0} for fully sparse
    /// tensors</param> <param name="indices_shape">Shape is expected to be 2-D, use {0} for fully
    /// sparse tensors </param> <returns></returns>
    BlockSparseMutator MakeBlockSparseData(const TensorShape& values_shape,
   ...
   private:
    ...
    Status ValidateBlockSparseShapes(const TensorShape& values_shape,
                                     const TensorShape& index_shape) const;
    ...
    void InitCsrIndices(size_t inner_size, const int64_t* inner, size_t outer_size,
                        const int64_t* outer);
    ...
    Tensor values_;  // Tensor instance that holds a values buffer information either user supplied
                     // or to a beginning of p_data_, before format specific indices.
    std::vector<Tensor>
        format_data_;  // A collection of format specific indices. They contain pointers to either a
