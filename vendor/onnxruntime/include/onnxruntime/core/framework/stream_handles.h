...
class Stream {
   public:
    ...
    virtual std::unique_ptr<synchronize::Notification> CreateNotification(
    ...
    // The sync id is incremented before each new event that is recorded in our stream via
    // Notification::Activate.
    uint64_t GetSyncId() const {
        return sync_id_;
    }

    // Return the sync id from when the last synchronization happened between producer_stream and
    // this stream. i.e. the id for the event that the producer stream recorded and we waited on
    //
   ...
   private:
    ...
    // current sync id. equivalent to a counter for the number of events we have recorded in the
    // underlying stream. 0 == no events recorded. sync_id_ is updated prior to recording a new
    // event.
    std::atomic<uint64_t> sync_id_{0};

    // This is a map to track synchronization points between streams. When we wait on another stream
    // (the producer) we add an entry to the map for that stream. The entry has the sync id from the
    // producer stream for the event we waited on.
    //
...
class Notification {
   public:
    ...
    // Activate the notification. This records an event in the Stream that created the Notification
    // that other streams can wait on.
...
// This SetDevice function is used by TRT EP or CUDA EP to handle the case where
// ExecutionMode::ORT_PARALLEL is enabled. In that case, ORT retrieves a thread from the thread pool
// to run kernels for a given session. Since new threads default to using device 0, but the session
// may be tightly bound to a device > 0, This SetDevice function will be called in RunSince to
// ensure running kernels on a correct GPU device.
...
class IStreamCommandHandleRegistry {
   public:
    ...
    // and the stream we are waiting. i.e., for an cuda event what notify the memory copy, it could
    // be wait on a CPU stream, or on another cuda stream.
    [[nodiscard]] virtual WaitNotificationFn GetWaitHandle(
        const OrtDevice& notification_owner_device, const OrtDevice& executor_device) const = 0;

    // Get the stream creation function registered for the given device type.
    [[nodiscard]] virtual CreateStreamFn GetCreateStreamFn(
        OrtDevice::DeviceType execution_device_type) const = 0;
    ...
    virtual void RegisterWaitFn(OrtDevice::DeviceType notification_device_type,
                                OrtDevice::DeviceType device_type, WaitNotificationFn fn) = 0;
    ...
    // This interface is currently used by TRT EP or CUDA EP only and is called in RunSince from
    // stream execution.
