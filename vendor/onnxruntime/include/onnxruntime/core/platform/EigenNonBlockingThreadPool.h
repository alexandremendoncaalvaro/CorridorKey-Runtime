...
// error: ignoring attributes on template argument "Eigen::PacketType<const float,
// Eigen::DefaultDevice>::type {aka
...
// error: ‘void* memcpy(void*, const void*, size_t)’ copying an object of non-trivial type
// ‘Eigen::internal::Packet4c’ {aka ‘struct Eigen::internal::eigen_packet_wrapper<int, 2>’} from an
// array of ‘const int8_t’ {aka ‘const signed char’} [-Werror=class-memaccess]
#ifdef HAS_CLASS_MEMACCESS
#pragma GCC diagnostic ignored "-Wclass-memaccess"
#endif
// eigen-src/unsupported/Eigen/CXX11/src/ThreadPool/EventCount.h:231:56: error: implicit conversion
// loses integer
//   precision: 'uint64_t' (aka 'unsigned long long') to 'size_t' (aka 'unsigned long')
...
enum class PushResult { REJECTED, ACCEPTED_IDLE, ACCEPTED_BUSY };
...
2. Inside thread pool, call LogStart() before interested section and LogEnd... after to log elapsed
time;
3. To extend, just add more events in enum Event before "All", and update GetEventName(...)
accordingly;
...
class ThreadPoolProfiler {
   public:
    ...
    void Start() {}
    std::string Stop() {
        return "not available for minimal build";
    }
    void LogStart() {}
...
class ThreadPoolProfiler {
   public:
    ...
    void LogEnd(ThreadPoolEvent);  // called in main thread to calculate and save the time elapsed
                                   // from last start point
    void LogEndAndStart(ThreadPoolEvent);
    void LogStartAndCoreAndBlock(std::ptrdiff_t block_size);
    void LogCoreAndBlock(std::ptrdiff_t block_size);  // called in main thread to log core and block
                                                      // size for task breakdown
...
class ExtendedThreadPoolInterface : public Eigen::ThreadPoolInterface {
   public:
    ...
    virtual void RunInParallelSection(ThreadPoolParallelSection& ps,
                                      std::function<void(unsigned idx)> fn, unsigned n,
...
class ThreadPoolLoop {
   public:
    ThreadPoolLoop(std::function<void(unsigned)> f, unsigned t)
        : fn(std::move(f)), threads_needed(t) {}
   ...
   public:
    RunQueue() : front_(0), back_(0) {
        ...
        for (unsigned i = 0; i < kSize; i++)
            array_[i].state.store(ElemState::kEmpty, std::memory_order_relaxed);
    }
...
class ThreadPoolTempl : public onnxruntime::concurrency::ExtendedThreadPoolInterface {
   ...
   public:
    ...
    ThreadPoolTempl(const CHAR_TYPE* name, int num_threads, bool allow_spinning, Environment& env,
        ...
        ORT_TRY {
            ...
            for (auto i = 0u; i < num_threads_; i++) {
                worker_data_[i].thread.reset(
                    env_.CreateThread(name, i, WorkerLoop, this, thread_options));
            }
        }
        ORT_CATCH(...) {
    ...
    void ScheduleOnPreferredWorkers(PerThread& pt, ThreadPoolParallelSection& ps,
                                    InlinedVector<int>& preferred_workers, unsigned par_idx_start,
                                    unsigned par_idx_end, std::function<void(unsigned)> worker_fn) {
        for (auto par_idx = par_idx_start; par_idx < par_idx_end; ++par_idx) {
            ...
            auto push_status = q.PushBackWithTag(
                [worker_fn, par_idx, &preferred_workers, &ps, this]() {
            ...
            if (push_status == PushResult::ACCEPTED_IDLE ||
                push_status == PushResult::ACCEPTED_BUSY) {
    ...
    void RunInParallelInternal(PerThread& pt, ThreadPoolParallelSection& ps, unsigned new_dop,
                               bool dispatch_async, std::function<void(unsigned)> worker_fn) {
        ...
        if (current_dop < new_dop) {
            ...
            if (dispatch_async && extra_needed > 1) {
                ...
                Task dispatch_task = [current_dop, new_dop, worker_fn, &preferred_workers, &ps, &pt,
                                      this]() {
                    ...
                    ScheduleOnPreferredWorkers(pt, ps, preferred_workers, current_dop + 1, new_dop,
                                               worker_fn);
                ...
                auto push_status =
                    dispatch_que.PushBackWithTag(dispatch_task, pt.tag, ps.dispatch_w_idx);
                // Queue accepted the task; wake the thread that owns the queue.
                // In addition, if the queue was non-empty, attempt to wake
                // another thread (which may then steal the task).
                if (push_status == PushResult::ACCEPTED_IDLE ||
                ...
                ScheduleOnPreferredWorkers(pt, ps, preferred_workers, current_dop, new_dop,
                                           std::move(worker_fn));
            }
    ...
    void RunInParallelSection(ThreadPoolParallelSection& ps, std::function<void(unsigned idx)> fn,
...
#pragma warning(pop)
    ...
    struct WorkerData {
        ...
        // While manipulating under the mutex, we employ relaxed semantics so the compiler is not
        // restricted any further.
        void EnsureAwake() {
            ThreadStatus seen = GetStatus();
            if (seen == ThreadStatus::Blocking || seen == ThreadStatus::Blocked) {
        ...
        bool SetBlocked(std::function<bool()> should_block, std::function<void()> post_block) {
            std::unique_lock<std::mutex> lk(mutex);
    ...
    // SpinLoopStatus indicates whether the main worker spinning (inner) loop should exit
    // immediately when there is no work available (kIdle) or whether it should follow the
    // configured spin-then-block policy (kBusy). This lets the ORT session layer hint to the thread
    // pool that it should stop spinning in between requests.
    enum class SpinLoopStatus { kIdle, kBusy };
                ...
                if (!t) {
                    if (!td.SetBlocked(  // Pre-block test
                            [&]() -> bool {
                                ...
                                // Check whether work was pushed to us while attempting to block. We
                                // make this test while holding the per-thread status lock, and
                                // after setting our status to ThreadStatus::Blocking.
                                //
                                // This synchronizes with ThreadPool::Schedule which pushes work to
                                // the queue and then tests for ThreadStatus::Blocking/Blocked (via
                                // EnsureAwake):
                                //
                                // Main thread:                    Worker:
                                ...
                                // No work pushed to us, continue attempting to block.  The
                                // remaining test  is to synchronize with termination requests.  If
                                // we are shutting down and all worker threads blocked without work,
                                // that's we are done.
                                if (should_block) {
                                    blocked_++;
                                    if (done_ && blocked_ == num_threads_) {
                                        ...
                                        // Consider that all queues are empty and all worker threads
                                        // are preempted right after incrementing blocked_ above.
                                        // Now a free-standing thread submits work and calls
                                        // destructor (which sets done_). If we don't re-check
                                        // queues, we will exit leaving the work unexecuted.
                                        if (NonEmptyQueueIndex() != -1) {
                                            // Note: we must not pop from queues before we decrement
                                            // blocked_, otherwise the following scenario is
                                            // possible. Consider that instead of checking for
                                            // emptiness we popped the only element from queues. Now
                                            // other worker threads can start exiting, which is bad
                                            // if the work item submits other work. So we just check
                                            // emptiness here, which ensures that all worker threads
