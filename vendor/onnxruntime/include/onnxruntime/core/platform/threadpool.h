...
class ThreadPool {
   public:
    ...
    ThreadPool(Env* env, const ThreadOptions& thread_options, const NAME_CHAR_TYPE* name,
               int degree_of_parallelism, bool low_latency_hint, bool force_hybrid = false);
    ...
    static void TryParallelFor(
        ThreadPool* tp, std::ptrdiff_t total, double cost_per_unit,
        const std::function<void(std::ptrdiff_t first, std::ptrdiff_t last)>& fn) {
        TryParallelFor(tp, total, TensorOpCost{0, 0, static_cast<double>(cost_per_unit)}, fn);
    }

    static void TryParallelFor(
        ThreadPool* tp, std::ptrdiff_t total, const TensorOpCost& cost_per_unit,
        const std::function<void(std::ptrdiff_t first, std::ptrdiff_t last)>& fn);
     ...
     * Pitfall: Caller should cap `num_batches` to a reasonable value based on the cost of `fn` and
     *the value of `total`. For example, if fn is as simple as: int sum=0; fn = [&](int i){sum +=i;}
     *and `total` is 100, then num_batches should be just 1.
     *
     * ```
     **/
    template <typename F>
    inline static void TryBatchParallelFor(ThreadPool* tp, std::ptrdiff_t total, F&& fn,
    ...
    */
    constexpr static WorkInfo PartitionWork(std::ptrdiff_t batch_idx, std::ptrdiff_t num_batches,
   ...
   private:
    ...
    void ParallelForFixedBlockSizeScheduling(
        std::ptrdiff_t total, std::ptrdiff_t block_size,
