...
The transformer will apply all the rewrite rules iteratively as determined by the underlying
rewriting strategy. Several rewriting-strategies are possible when traversing the graph and applying
rewrite rules, each with different trade offs. At the moment, we define one that performs top-down
traversal of nodes.
...
class RuleBasedGraphTransformer : public GraphTransformer {
   public:
    ...
    /** Gets the list of registered rewrite rules that will be triggered on nodes with the given op
       type by this rule-based transformer.
        @returns a pointer to the vector containing all the registered rewrite rules. */
    const InlinedVector<std::reference_wrapper<const RewriteRule>>* GetRewriteRulesForOpType(
    ...
    /** Gets the rewrite rules that are evaluated on all nodes irrespective of their op type.
        @returns a pointer to the vector containing all such rewrite rules or nullptr if no such
       rule. */
   ...
   protected:
    /** Applies the given set of rewrite rules on the Node of this Graph.
        ...
        @param[out] rule_effect Enum that indicates whether and how the graph was modified as a
       result of applying rules on this node.
        @returns Status indicating success or providing error information. */
    common::Status ApplyRulesOnNode(
        Graph& graph, Node& node, gsl::span<const std::reference_wrapper<const RewriteRule>> rules,
   ...
   private:
    ...
    // The list of unique pointers for all rules (so that rules can be registered for several op
    // types).
    InlinedVector<std::unique_ptr<RewriteRule>> rules_;
    // Map that associates a node's op type with the vector of rules that are registered to be
    // triggered for that node.
    InlinedHashMap<std::string, InlinedVector<std::reference_wrapper<const RewriteRule>>>
        op_type_to_rules_;
    ...
    common::Status ApplyImpl(Graph& graph, bool& modified, int graph_level,
