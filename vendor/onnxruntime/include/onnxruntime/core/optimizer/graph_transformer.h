...
class GraphTransformer {
   public:
    ...
    /** Apply the in-place transformation defined by this transformer to the provided Graph
    instance.
   ...
   protected:
    ...
    Status Recurse(Node& node, bool& modified, int graph_level,
   ...
   private:
    ...
    // graph_level is 0 for the main graph, and is incremented when descending into the subgraph of
    // a node. You MUST call Recurse for all valid Nodes in the graph to ensure any subgraphs in
    // control flow nodes (Scan/If/Loop) are processed as well. You should avoid calling
    // Graph::Resolve in ApplyImpl unless you are 100% sure it's required. In most cases the call to
    // Graph::Resolve in GraphTransformer::Apply after the call to ApplyImpl (if 'modified' is true)
    // should suffice.
    virtual Status ApplyImpl(Graph& graph, bool& modified, int graph_level,
...
struct OpKernelRegistryId {
    ...
    OpKernelRegistryId(
        const std::basic_string_view<char>& op, const std::basic_string_view<char>& domain,
        const int version,
