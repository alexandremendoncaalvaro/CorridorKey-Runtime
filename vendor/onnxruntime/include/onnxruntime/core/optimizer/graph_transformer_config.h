...
struct GraphTransformerConfiguration {
    struct PropagateCastOpsConfiguration {
         ...
         * the other is to propagate casts using flood-fill approach, expanding float16 regions in
         * the graph traversing the graph up/down.
         */
        enum class Strategy {
            None = 0,
            InsertAndReduce = 1,
        ...
        int level{1}; /* -1 => no cast propagation,
                           ...
                           1 => use ORT predefined list of level 1 opcodes in addition to the user
                         specified allow opcodes 2 => use ORT predefined list of level 2 opcodes in
                         addition to the user specified allow opcodes
                        */
        Strategy strategy = Strategy::FloodFill;
        // List of allowed opcodes to consider as safe to execute in float16, while moving cast
        // operations
