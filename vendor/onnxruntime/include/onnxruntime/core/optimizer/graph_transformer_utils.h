...
/** Generates all predefined rules for this level.
   If rules_to_enable is not empty, it returns the intersection of predefined rules and
   rules_to_enable.
...
InlinedVector<std::unique_ptr<RewriteRule>> GenerateRewriteRules(
    TransformerLevel level, const InlinedHashSet<std::string>& rules_to_disable = {},
    const bool enable_cast_chain_elimination = false);

/** Given a TransformerLevel, this method generates a name for the rule-based graph transformer of
 * that level. */
...
InlinedVector<std::unique_ptr<GraphTransformer>> GenerateTransformers(
    TransformerLevel level, const SessionOptions& session_options,
...
/** Generates all predefined transformers which can be used to provide runtime optimizations for
   this level in a minimal build. Any transformers or rewrite rules named in
   rules_and_transformers_to_disable will be excluded.

    This is a distinct function from GenerateTransformers() because:
    - An ORT format model used in a minimal build will have been pre-optimized to at least level 1
   when created, so level 1 transformers are not included.
    ...
    - Only a small subset of transformers support storing/replaying runtime optimizations with an
   ORT format model
      - this capability is provided by the SelectionActionTransformer infrastructure
      - the logic to determine the set of nodes a transformer should modify is captured during
   creation of the ORT format model
    ...
    - The QDQFinalCleanupTransformer and NhwcTransformer transformers are also supported in a
   minimal build
*/
InlinedVector<std::unique_ptr<GraphTransformer>> GenerateTransformersForMinimalBuild(
    TransformerLevel level, const SessionOptions& session_options,
    const SatApplyContextVariant& apply_context, const IExecutionProvider& cpu_execution_provider,
    const logging::Logger& logger,
    const InlinedHashSet<std::string>& rules_and_transformers_to_disable = {},
    concurrency::ThreadPool* intra_op_thread_pool = nullptr);
