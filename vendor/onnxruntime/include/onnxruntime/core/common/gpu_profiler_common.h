...
class ProfilerActivityBuffer {
   public:
    ProfilerActivityBuffer() noexcept : data_(nullptr), size_(0) {}
    ...
    ProfilerActivityBuffer(ProfilerActivityBuffer&& other) noexcept : ProfilerActivityBuffer() {
    ...
    static ProfilerActivityBuffer CreateFromPreallocatedBuffer(std::unique_ptr<char[]>&& buffer_ptr,
                                                               size_t size) {
...
class GPUTracerManager {
   public:
    ...
    void Consume(uint64_t client_handle, const TimePoint& start_time,
    ...
    void PushCorrelation(uint64_t client_handle, uint64_t external_correlation_id,
        ...
        //      std::chrono::duration_cast<std::chrono::microseconds>(event_start_time -
        //      profiling_start_time).count()
        //
        ...
        // makes it impossible to map events (which are tagged with external_correlation_id) to
        // clients.
        //
        // To address these difficulties, we construct a new correlation_id (let's call it
        // unique_cid) as follows: unique_cid =
        //    external_correlation_id +
        //    std::chrono::duration_cast<std::chrono::microseconds>(profiling_start_time.time_since_epoch()).count()
        // now, unique_cid is monotonically increasing with time, so it can be used to reliably map
        // events to clients.
        //
        // Of course, clients expect lists of events to be returned (on a call to Consume()), that
        // are still keyed on the external_correlation_id that they've specified here, so we need to
        // remember the offset to be subtracted
        uint64_t offset = std::chrono::duration_cast<std::chrono::microseconds>(
    ...
    void PopCorrelation(uint64_t& popped_external_correlation_id) {
        ...
        // lookup the offset and subtract it before returning popped_external_correlation_id to the
        // client
  ...
  uint64_t GetGPUTimestampInNanoseconds();
    ...
    void MapEventToClient(uint64_t tracer_correlation_id, EventRecord&& event) {
        ...
        if (it == tracer_correlation_to_unique_correlation_.end()) {
            // We're yet to receive a mapping to unique_correlation_id for this
            // tracer_correlation_id
   ...
   private:
    ...
    Events* GetEventListForUniqueCorrelationId(uint64_t unique_correlation_id) {
        ...
        auto& event_list =
            per_client_events_by_ext_correlation_[client_handle_offset.first][external_correlation];
        return &event_list;
    }
    ...
    void MapEventsToClient(uint64_t unique_correlation_id, std::vector<EventRecord>&& events) {
        auto p_event_list = GetEventListForUniqueCorrelationId(unique_correlation_id);
        if (p_event_list != nullptr) {
            p_event_list->insert(p_event_list->end(), std::make_move_iterator(events.begin()),
    ...
    void MergeEvents(std::map<uint64_t, Events>& events_to_merge, Events& events) {
        ...
        for (auto& map_iter : events_to_merge) {
            ...
            while (event_iter != event_end &&
                   (event_iter->ts < ts || (event_iter->ts == ts && (event_iter + 1) != event_end &&
                                            (event_iter + 1)->ts == ts))) {
