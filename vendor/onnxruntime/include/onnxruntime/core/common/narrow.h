...
// onnxruntime::narrow() is like gsl::narrow() but it is also available when exceptions are
// disabled.
...
constexpr T narrow(U u) noexcept {
    ...
    const T t = gsl::narrow_cast<T>(
        u);  // While this is technically undefined behavior in some cases (i.e., if the source
             // value is of floating-point type and cannot fit into the destination integral type),
             // the resultant behavior is benign on the platforms that we target (i.e., no hardware
             // trap representations are hit).
