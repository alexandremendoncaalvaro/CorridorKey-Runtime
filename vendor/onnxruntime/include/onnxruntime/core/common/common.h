...
// __PRETTY_FUNCTION__
#define ORT_WHERE \
    ::onnxruntime::CodeLocation(__FILE__, __LINE__, static_cast<const char*>(__FUNCTION__))

#define ORT_WHERE_WITH_STACK                                                                       \
    ::onnxruntime::CodeLocation(__FILE__, __LINE__, static_cast<const char*>(__PRETTY_FUNCTION__), \
...
#define ORT_THROW_FROM_STATUS(status)                                                             \
    do {                                                                                          \
        ::onnxruntime::PrintFinalMessage(                                                         \
            ::onnxruntime::OnnxRuntimeException(ORT_WHERE_WITH_STACK, status.ToString()).what()); \
        abort();                                                                                  \
    } while (false)
...
#define ORT_THROW_FROM_STATUS(status)                               \
    throw ::onnxruntime::OnnxRuntimeException(                      \
        ORT_WHERE_WITH_STACK, status.ToString(), status.Category(), \
...
#define ORT_MAKE_STATUS(category, code, ...)                                                    \
    ::onnxruntime::common::Status(::onnxruntime::common::category, ::onnxruntime::common::code, \
                                  ::onnxruntime::MakeString(__VA_ARGS__))
...
#define ORT_RETURN_IF_ERROR_SESSIONID(expr, session_id)                                       \
    do {                                                                                      \
        auto _status = (expr);                                                                \
        if ((!_status.IsOK())) {                                                              \
            ::onnxruntime::LogRuntimeError(session_id, _status, __FILE__,                     \
...
#define ORT_THROW_IF_ERROR(expr)                                                              \
    do {                                                                                      \
        auto _status = (expr);                                                                \
        if ((!_status.IsOK())) {                                                              \
            ::onnxruntime::LogRuntimeError(0, _status, __FILE__,                              \
                                           static_cast<const char*>(__FUNCTION__), __LINE__); \
...
struct null_type {};
inline std::string ToUTF8String(const std::string& s) {
    return s;
}
#ifdef _WIN32
/**
...
inline std::wstring ToWideString(std::wstring_view s) {
...
inline std::string ToWideString(const char* s) {
    return s;
}
inline std::string ToWideString(std::string_view s) {
    return std::string{s};
}
#endif
...
inline bool Contains(const AssociativeContainer<Key, OtherContainerArgs...>& container,
                     LookupKey&& key) {
