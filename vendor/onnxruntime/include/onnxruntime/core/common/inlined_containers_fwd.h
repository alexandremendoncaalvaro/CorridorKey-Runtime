...
struct CalculateInlinedVectorDefaultInlinedElements {
    ...
    // make sure that users are making an intentional decision if they request a lot of inline
    // storage.
    //
    ...
    // sizeof(absl::InlinedVector<T, 1>) is in general platform-dependent. However, we don't expect
    // this to be much of an issue, because most LLVM development happens on 64-bit hosts, and
    // therefore sizeof(T) is expected to *decrease* when compiled for 32-bit hosts, dodging the
    // issue. The reverse situation, where development happens on a 32-bit host and then fails due
    // to sizeof(T) *increasing* on a 64-bit host, is expected to be very rare.
    ...
    static constexpr size_t PreferredInlineBytes =
        kPreferredInlinedVectorSizeof - InlinedVectorHeaderSize;
