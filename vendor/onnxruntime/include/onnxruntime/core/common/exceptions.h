...
class OnnxRuntimeException : public std::exception {
    ...
    */
    ...
    OnnxRuntimeException(const CodeLocation& location, const std::string& message,
                         common::StatusCategory category, common::StatusCode code) noexcept
    ...
    /**
       ...
       @param msg Message containing additional information about the exception cause.
    */
    OnnxRuntimeException(const CodeLocation& location, const char* failed_condition,
                         const std::string& msg) noexcept
        : OnnxRuntimeException(location, failed_condition, msg, common::StatusCategory::ONNXRUNTIME,
    ...
    */
    OnnxRuntimeException(const CodeLocation& location, const char* failed_condition,
                         const std::string& msg, common::StatusCategory category,
                         common::StatusCode code)
        : location_{location}, category_(category), code_(code) {
        std::ostringstream ss;

        ss << location.ToString(CodeLocation::kFilenameAndPath);  // output full path in case just
        ...
        if (!location.stacktrace.empty()) {
            ...
            // location.ToString()
            std::copy(std::next(location.stacktrace.begin()), location.stacktrace.end(),
