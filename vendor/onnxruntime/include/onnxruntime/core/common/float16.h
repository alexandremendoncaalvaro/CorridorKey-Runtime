...
struct MLFloat16 : onnxruntime_float16::Float16Impl<MLFloat16> {
   private:
    explicit constexpr MLFloat16(uint16_t x) noexcept {
        val = x;
    }
   ...
   public:
    ...
    constexpr static MLFloat16 FromBits(uint16_t x) noexcept {
        return MLFloat16(x);
    }
    ...
    explicit MLFloat16(float v) noexcept {
...
struct BFloat16 : onnxruntime_float16::BFloat16Impl<BFloat16> {
    ...
    inline ORT_HOST_DEVICE BFloat16(float v) noexcept {
        ...
        // We should be using memcpy in order to respect the strict aliasing rule but it fails in
        // the HIP environment.
        if (v != v) {  // isnan
            val = UINT16_C(0x7FC0);
        } else {
    ...
    ORT_HOST_DEVICE static bool AreZeroHostDevice(const BFloat16Impl& lhs,
                                                  const BFloat16Impl& rhs) noexcept {
...
class numeric_limits<onnxruntime::MLFloat16> {
   public:
    static constexpr onnxruntime::MLFloat16 min() noexcept {
        return onnxruntime::MLFloat16::FromBits(
            0x0400U);  // Minimum positive normalized value: 0.00006103515625
    }
    ...
    static constexpr onnxruntime::MLFloat16 infinity() noexcept {
        return onnxruntime::MLFloat16::FromBits(
            0x7C00U);  // Bits: sign(0), exponent(111,11), fraction(00,0000,0000)
    }
    ...
    static constexpr onnxruntime::MLFloat16 quiet_NaN() noexcept {
        ...
        // Note that most frameworks use 0x7E00; while CUDA uses 0x7FFF; .Net System.Half.NaN uses
        // 0xFE00;
        return onnxruntime::MLFloat16::FromBits(
            0x7E00U);  // Bits: sign(0), exponent(111,11), fraction(10,0000,0000)
    }

    static constexpr onnxruntime::MLFloat16 signaling_NaN() noexcept {
        return onnxruntime::MLFloat16::FromBits(
            0x7D00U);  // Bits: sign(0), exponent(111,11), fraction(01,0000,0000)
    }

    static constexpr onnxruntime::MLFloat16 denorm_min() noexcept {
        return onnxruntime::MLFloat16::FromBits(
            0x0001U);  // Minimum subnormal value: 0.000000059604645
    }

    static constexpr onnxruntime::MLFloat16 epsilon() noexcept {
        return onnxruntime::MLFloat16::FromBits(
            0x1400U);  // Difference between 1.0 and the next value: 2^-10 = 0.0009765625
    }
...
class numeric_limits<onnxruntime::BFloat16> {
   public:
    static constexpr onnxruntime::BFloat16 min() noexcept {
        return onnxruntime::BFloat16::FromBits(
            0x0080U);  // Minimum positive normalized value: 1.175494e-38
    }

    static constexpr onnxruntime::BFloat16 max() noexcept {
        return onnxruntime::BFloat16::FromBits(
            0x7F7FU);  // Largest representable value: 3.38953139e38
    }

    static constexpr onnxruntime::BFloat16 lowest() noexcept {
        return onnxruntime::BFloat16::FromBits(
            0xFF7FU);  // Smallest representable value: -3.38953139e38
    }

    static constexpr onnxruntime::BFloat16 infinity() noexcept {
        return onnxruntime::BFloat16::FromBits(
            0x7F80U);  // Bits: sign(0), exponent(111,1111,1), fraction(000,0000)
    }

    static constexpr onnxruntime::BFloat16 quiet_NaN() noexcept {
        // The most significant fraction bit shall be 1, and no limitation on other fraction bits.
        // Note that Torch, Tensorflow, OpenVino, nGraph uses 0x7FC0; Paddle uses 0x7FC1; CUDA uses
        // 0x7FFF.
        return onnxruntime::BFloat16::FromBits(
            0x7FC1U);  // Bits: sign(0), exponent(111,1111,1), fraction(100,0001)
    }

    static constexpr onnxruntime::BFloat16 signaling_NaN() noexcept {
        // The most significant fraction bit shall be 0, and there is at least one 1 in other
        // fraction bits.
        return onnxruntime::BFloat16::FromBits(
            0x7F81U);  // Bits: sign(0), exponent(111,1111,1), fraction(000,0001)
    }
    ...
    static constexpr onnxruntime::BFloat16 epsilon() noexcept {
        return onnxruntime::BFloat16::FromBits(
            0x3C00U);  // Difference between 1.0 and the next value: 2^-7 = 0.0078125
    }
