...
// Note: Currently (e.g., in LLVM 19), libc++'s std::from_chars() doesn't support floating point
// types yet.
template <typename T>
constexpr bool ParseWithFromChars = !std::is_same_v<bool, T> && std::is_integral_v<T>;
#else
template <typename T>
constexpr bool ParseWithFromChars =
    !std::is_same_v<bool, T> && (std::is_integral_v<T> || std::is_floating_point_v<T>);
#endif
 ...
 * If successful, sets `value` and returns true. Otherwise, does not modify `value` and returns
 * false.
 */
template <typename T>
std::enable_if_t<detail::ParseWithFromChars<T>, bool> TryParseStringWithClassicLocale(
