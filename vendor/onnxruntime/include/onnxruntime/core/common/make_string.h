...
inline std::string MakeString(const Args&... args) {
    // We need to update the types from the MakeString template instantiation to decay any char[n]
    // to char*.
    //   e.g. MakeString("in", "out") goes from MakeString<char[2], char[3]> to
    //   MakeStringImpl<char*, char*>
    //        so that MakeString("out", "in") will also match MakeStringImpl<char*, char*> instead
    //        of requiring MakeStringImpl<char[3], char[2]>.
    //
    // We have to do the type processing before any actual work, so this function purely implements
    // the type processing. If we do not do it this way we do not get the full binary size
    // reduction.
    //
...
inline std::string MakeStringWithClassicLocale(const Args&... args) {
    return detail::MakeStringWithClassicLocaleImpl(
