...
static constexpr const char* event_category_names_[EVENT_CATEGORY_MAX] = {"Session", "Node",
                                                                          "Kernel", "Api"};

// Timing record for all events.
struct EventRecord {
    EventRecord() = default;
    EventRecord(EventCategory category, int process_id, int thread_id, std::string&& event_name,
                long long time_stamp, long long duration,
    ...
    EventRecord(EventCategory category, int process_id, int thread_id,
                const std::string& event_name, long long time_stamp, long long duration,
...
class EpProfiler {
   public:
    ...
    virtual bool StartProfiling(
        TimePoint profiling_start_time) = 0;  // called when profiling starts
    virtual void EndProfiling(
        TimePoint start_time,
        Events& events) = 0;  // called when profiling ends, save all captures numbers to "events"
    virtual void Start(uint64_t) {
    }  // called before op start, accept an id as argument to identify the op
    virtual void Stop(uint64_t) {
    }  // called after op stop, accept an id as argument to identify the op
};
