...
#define CREATE_MESSAGE(logger, severity, category, datatype)                               \
    ::onnxruntime::logging::Capture(logger, ::onnxruntime::logging::Severity::k##severity, \
                                    category, datatype, ORT_WHERE)
...
/*
  ...
  The logger instance provides an identifier so that log messages from different runs can be
  separated.

  Variants with DEFAULT in the macro name use the default logger provided by logging manager. This
  is static so accessible from any code, provided a LoggingManager instance created with
  InstanceType::Default exists somewhere. See logging.h for further explanation of the expected
  setup.

  DataType
  Default uses DataType::SYSTEM.

  Variants with USER in the macro name use DataType::USER. This is data that could be PII, and may
  need to be filtered from output. LoggingManager applies this filtering.
...
/**
 * Note:
 * The stream style logging macros (something like `LOGS() << message`) are designed to be appended
 * to. Normally, we can isolate macro code in a separate scope (e.g., `do {...} while(0)`), but here
 * we need the macro code to interact with subsequent code (i.e., the values to log).
 *
 * When an unisolated conditional is involved, extra care needs to be taken to avoid unexpected
 * parsing behavior. For example:
 *
 * if (enabled)
 *   Capture().Stream()
 *
...
#define LOGF(logger, severity, format_str, ...)                                                \
    LOGF_CATEGORY(logger, severity, ::onnxruntime::logging::Category::onnxruntime, format_str, \
                  ##__VA_ARGS__)

#define LOGF_USER(logger, severity, format_str, ...)                                    \
    LOGF_USER_CATEGORY(logger, severity, ::onnxruntime::logging::Category::onnxruntime, \
...
#define LOGF_DEFAULT_CATEGORY(severity, category, format_str, ...)                             \
    LOGF_CATEGORY(::onnxruntime::logging::LoggingManager::DefaultLogger(), severity, category, \
                  format_str, ##__VA_ARGS__)

#define LOGF_USER_DEFAULT_CATEGORY(severity, category, format_str, ...)                   \
    LOGF_USER_CATEGORY(::onnxruntime::logging::LoggingManager::DefaultLogger(), severity, \
...
#define LOGF_DEFAULT(severity, format_str, ...)                                                \
    LOGF_DEFAULT_CATEGORY(severity, ::onnxruntime::logging::Category::onnxruntime, format_str, \
                          ##__VA_ARGS__)

#define LOGF_USER_DEFAULT(severity, format_str, ...)                                    \
    LOGF_USER_DEFAULT_CATEGORY(severity, ::onnxruntime::logging::Category::onnxruntime, \
                               format_str, ##__VA_ARGS__)
...
#define LOGF_CATEGORY_IF(boolean_expression, logger, severity, category, format_str, ...) \
    do {                                                                                  \
        if ((boolean_expression) == true)                                                 \
            LOGF_CATEGORY(logger, severity, category, format_str, ##__VA_ARGS__);         \
    } while (0)

#define LOGF_DEFAULT_CATEGORY_IF(boolean_expression, severity, category, format_str, ...) \
    do {                                                                                  \
        if ((boolean_expression) == true)                                                 \
            LOGF_DEFAULT_CATEGORY(severity, category, format_str, ##__VA_ARGS__);         \
    } while (0)

#define LOGF_USER_CATEGORY_IF(boolean_expression, logger, severity, category, format_str, ...) \
    do {                                                                                       \
        if ((boolean_expression) == true)                                                      \
            LOGF_USER_CATEGORY(logger, severity, category, format_str, ##__VA_ARGS__);         \
    } while (0)

#define LOGF_USER_DEFAULT_CATEGORY_IF(boolean_expression, severity, category, format_str, ...) \
    do {                                                                                       \
        if ((boolean_expression) == true)                                                      \
            LOGF_USER_DEFAULT_CATEGORY(severity, category, format_str, ##__VA_ARGS__);         \
    } while (0)

// Logging with category of "onnxruntime"

#define LOGS_IF(boolean_expression, logger, severity)      \
    LOGS_CATEGORY_IF(boolean_expression, logger, severity, \
                     ::onnxruntime::logging::Category::onnxruntime)

#define LOGS_DEFAULT_IF(boolean_expression, severity)      \
    LOGS_DEFAULT_CATEGORY_IF(boolean_expression, severity, \
                             ::onnxruntime::logging::Category::onnxruntime)

#define LOGS_USER_IF(boolean_expression, logger, severity)      \
    LOGS_USER_CATEGORY_IF(boolean_expression, logger, severity, \
                          ::onnxruntime::logging::Category::onnxruntime)

#define LOGS_USER_DEFAULT_IF(boolean_expression, severity)      \
    LOGS_USER_DEFAULT_CATEGORY_IF(boolean_expression, severity, \
                                  ::onnxruntime::logging::Category::onnxruntime)

#define LOGF_IF(boolean_expression, logger, severity, format_str, ...) \
    LOGF_CATEGORY_IF(boolean_expression, logger, severity,             \
                     ::onnxruntime::logging::Category::onnxruntime, format_str, ##__VA_ARGS__)

#define LOGF_DEFAULT_IF(boolean_expression, severity, format_str, ...)                  \
    LOGF_DEFAULT_CATEGORY_IF(boolean_expression, severity,                              \
                             ::onnxruntime::logging::Category::onnxruntime, format_str, \
                             ##__VA_ARGS__)

#define LOGF_USER_IF(boolean_expression, logger, severity, format_str, ...)          \
    LOGF_USER_CATEGORY_IF(boolean_expression, logger, severity,                      \
                          ::onnxruntime::logging::Category::onnxruntime, format_str, \
                          ##__VA_ARGS__)

#define LOGF_USER_DEFAULT_IF(boolean_expression, severity, format_str, ...)                  \
    LOGF_USER_DEFAULT_CATEGORY_IF(boolean_expression, severity,                              \
                                  ::onnxruntime::logging::Category::onnxruntime, format_str, \
                                  ##__VA_ARGS__)
...
#define VLOGF_USER_DEFAULT(level, format_str, ...)                                         \
    VLOGF_USER(::onnxruntime::logging::LoggingManager::DefaultLogger(), level, format_str, \
               ##__VA_ARGS__)
