...
enum class ORTTraceLoggingKeyword : uint64_t {
    ...
    Reserved1 = 0x4,  // Reserved if we want to add some specific sub-categories instead of just
                      // LOGS() or other uses
    Reserved2 = 0x8,
    Reserved3 = 0x10,
    Reserved4 = 0x20,
...
/// Provides filtering based on a minimum LogSeverity level, and of messages with DataType::User if
/// enabled.
/// </summary>
class LoggingManager final {
   public:
    enum InstanceType {
        Default,  ///< Default instance of LoggingManager that should exist for the lifetime of the
                  ///< program
        Temporal  ///< Temporal instance. CreateLogger(...) should be used, however DefaultLogger()
                  ///< will NOT be provided via this instance.
    };
    ...
    /**
       ...
       @param default_min_severity The default minimum severity. Messages with lower severity will
       be ignored unless overridden in CreateLogger.
       @param default_filter_user_data If set to true ignore messages with DataType::USER unless
       overridden in CreateLogger.
       @param instance_type If InstanceType::Default, this is the default instance of the
       LoggingManager and is expected to exist for the lifetime of the program. It creates and owns
       the default logger that calls to the static DefaultLogger method return.
       @param default_logger_id Logger Id to use for the default logger. nullptr/ignored if
       instance_type == Temporal.
       @param default_max_vlog_level Default maximum level for VLOG messages to be created unless
       overridden in CreateLogger. Requires a severity of kVERBOSE for VLOG messages to be logged.
    */
    LoggingManager(std::unique_ptr<ISink> sink, Severity default_min_severity,
                   ...
                   const std::string* default_logger_id = nullptr, int default_max_vlog_level = -1);

    /**
       Creates a new logger instance which will use the provided logger_id and default severity and
       vlog levels.
    ...
    */
    ...
    /**
       Creates a new logger instance which will use the provided logger_id, severity and vlog
       levels.
       @param logger_id The log identifier.
       @param min_severity The minimum severity. Requests to create messages with lower severity
       will be ignored.
    ...
    */
    ...
    /**
       ...
       @remarks
       Creating a LoggingManager instance with is_default_instance == true registers a default
       logger. Note that the default logger is only valid until the LoggerManager that registered it
       is destroyed.
    ...
    */
    static bool HasDefaultLogger() {
        return nullptr != s_default_logger_;
    }
    ...
    */
    bool AddSinkOfType(SinkType sinkType, std::function<std::unique_ptr<ISink>()> sinkFactory,
    ...
    */
    ...
    /**
       Logs a FATAL level message and creates an exception that can be thrown with error
       information.
...
class Logger {
    ...
    */
    Logger(const LoggingManager& loggingManager, std::string id, Severity severity,
           bool filter_user_data, int vlog_level)
    ...
    */
    ...
    OrtLogger* ToExternal() {
        return reinterpret_cast<OrtLogger*>(this);
    }
    const OrtLogger* ToExternal() const {
        return reinterpret_cast<const OrtLogger*>(this);
    }
...
*/
std::unique_ptr<ISink> EnhanceSinkWithEtw(std::unique_ptr<ISink> existingSink,
                                          logging::Severity originalSeverity,
                                          logging::Severity etwSeverity);

/**
  If the ONNXRuntimeTraceLoggingProvider ETW Provider is enabled, then can override the logging
  level. But this overrided level only applies to the ETW sink. The original logger(s) retain their
  original logging level
*/
