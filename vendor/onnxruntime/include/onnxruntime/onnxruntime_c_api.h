...
struct OrtApi {
     ...
     * of ::OrtTensorRTProviderOptionsV2 and value should be its related range.
     *
     ...
     *   "qnn_context_cache_enable": 1 to enable QNN graph creation from cached QNN context file. If
     * it's enabled: QNN EP will load from cached QNN context binary if it exist. It will generate a
     * context binary file if it's not exist "qnn_context_cache_path": explicitly provide the QNN
     * context cache file. Default to model_file.onnx.bin if not provided. "profiling_level": QNN
     * profiling level, options: "off", "basic", "detailed". Default to off. "rpc_control_latency":
     * QNN RPC control latency. "htp_performance_mode": QNN performance mode, options: "burst",
     * "balanced", "default", "high_performance", "high_power_saver", "low_balanced",
     * "low_power_saver", "power_saver", "sustained_high_performance". Default to "default".
     *
     * SNPE supported keys:
     ...
     * types - "TF8", "TF16", "UINT8", "FLOAT". "ITENSOR" -- default, ITensor which is float only.
     ...
     * default. If SNPE is not available (due to a non Snpe enabled build or its dependencies not
     * being installed), this function will fail.
     *
