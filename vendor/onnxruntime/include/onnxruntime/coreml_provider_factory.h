...
// To generate an uint32_t coreml_flags for using with
// OrtSessionOptionsAppendExecutionProvider_CoreML below,
...
enum COREMLFlags {
    ...
    // Enable this option will only enable CoreML EP for Apple devices with ANE (Apple Neural
    // Engine) Please note, enable this option does not guarantee the entire model to be executed
    // using ANE only
    COREML_FLAG_ONLY_ENABLE_DEVICE_WITH_ANE = 0x004,

    // Only allow CoreML EP to take nodes with inputs with static shapes. By default it will also
    // allow inputs with dynamic shapes. However, the performance may be negatively impacted if
